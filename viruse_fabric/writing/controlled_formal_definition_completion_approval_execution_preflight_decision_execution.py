...
SOURCE_PATH = Path("outputs/controlled_formal_definition_completion_approval_execution_preflight_decision_plan_v8_42.md")
OUTPUT_PATH = Path("outputs/controlled_formal_definition_completion_approval_execution_preflight_decision_execution_v8_43.md")


@dataclass(frozen=True)
class PreflightDecisionExecutionRow:
    row_id: str
    execution_focus: str
    source_signal: str
...
@dataclass(frozen=True)
class PreflightDecisionExecutionReport:
    title: str
    output_path: Path
    source_path: Path
    ...
    preflight_decision_execution_row_count: int
    preflight_decision_plan_source_row_count: int
...
def _source_has_preflight_decision_plan_rows(text: str) -> bool:
    return (
        "Controlled formal definition completion approval execution preflight decision plan row count: 4" in text
        or _has_all_terms(text, ["controlled formal definition completion approval execution preflight decision plan row count", "4"])
    )
...
def _source_hard_zero_signal_count(text: str) -> int:
    zero_term_sets = [
        ["approval execution preflight decision execution", "0"],
...
def build_rows() -> List[PreflightDecisionExecutionRow]:
    return [
        PreflightDecisionExecutionRow(
            row_id="PDE-001",
            execution_focus="source preflight decision plan verification",
            source_signal="v8.42 preflight decision plan exists and preserves hard-zero boundaries",
            executed_decision="accept the v8.42 preflight decision plan as the valid source for decision execution",
            enforced_boundary="does not approve preflight",
        ),
        PreflightDecisionExecutionRow(
            row_id="PDE-002",
            execution_focus="preflight approval decision",
            source_signal="v8.42 plans preflight decision but keeps preflight approval absent",
            executed_decision="do not approve preflight in this milestone; require a separate preflight approval plan",
            enforced_boundary="preflight approval remains zero",
        ),
        PreflightDecisionExecutionRow(
            row_id="PDE-003",
            execution_focus="approval execution decision",
            source_signal="v8.42 keeps approval execution absent",
            executed_decision="do not authorize approval execution; require a separate approval gate after preflight approval planning",
            enforced_boundary="approval execution remains zero",
        ),
        PreflightDecisionExecutionRow(
            row_id="PDE-004",
            execution_focus="claim boundary decision",
            source_signal="v8.42 keeps completed definitions, proof execution, validation, citations, and readiness absent",
            executed_decision="block claim escalation until a separate audited milestone changes the relevant count",
            enforced_boundary="completion, proof, validation, citation, and readiness counts remain zero",
        ),
    ]
...
def render_report() -> str:
    ...
    preflight_decision_plan_source_row_count = 4 if _source_has_preflight_decision_plan_rows(source_text) else 0
    ...
    if missing_source_artifact_count != 0:
        errors.append("Missing required v8.42 preflight decision plan source artifact.")
    if preflight_decision_plan_source_row_count != 4:
        errors.append("Expected v8.42 preflight decision plan row count signal is absent.")
    if preflight_decision_plan_count != 1:
        errors.append("Expected v8.42 preflight decision plan count signal is absent.")
    ...
    lines.extend(
        [
            ...
            f"- Controlled formal definition completion approval execution preflight decision execution row count: {len(rows)}",
            f"- Controlled formal definition completion approval execution preflight decision plan source row count: {preflight_decision_plan_source_row_count}",
    ...
    if overclaim_count != 0:
        errors.append("Overclaim pattern detected in v8.43 preflight decision execution report.")
    if invented_citation_like_pattern_count != 0:
        errors.append("Invented citation-like pattern detected in v8.43 preflight decision execution report.")
...
def run() -> PreflightDecisionExecutionReport:
    text = render_report()
    source_text = _read_source()
    rows = build_rows()
    ...
    return PreflightDecisionExecutionReport(
        title="Controlled Formal Definition Completion Approval Execution Preflight Decision Execution v8.43",
        output_path=OUTPUT_PATH,
        source_path=SOURCE_PATH,
        source_artifact_count=1 if source_exists else 0,
        missing_source_artifact_count=missing_source_artifact_count,
        preflight_decision_execution_count=1,
        preflight_decision_execution_row_count=len(rows),
        preflight_decision_plan_source_row_count=preflight_decision_plan_source_row_count,
        ...
        approval_execution_preflight_decision_execution_count=1,
        approval_execution_preflight_approval_plan_required_count=1,
