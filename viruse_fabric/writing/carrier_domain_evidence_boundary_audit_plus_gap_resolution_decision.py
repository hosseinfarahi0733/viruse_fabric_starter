...
SOURCE_PATH = Path("outputs/stabilization_predicate_carrier_domain_exactness_evidence_execution_v8_88.md")
OUTPUT_PATH = Path("outputs/carrier_domain_evidence_boundary_audit_plus_gap_resolution_decision_v8_89.md")
...
def _carried_counts(source_text: str) -> dict[str, int]:
    return {
        "evidence_execution": 1 if _has_count(source_text, "Carrier domain exactness evidence execution", "1") else 0,
        "evidence_rows": 5 if _has_count(source_text, "Carrier domain evidence row", "5") else 0,
        "scope_rules": 3 if _has_count(source_text, "Carrier domain scope rule", "3") else 0,
        "acceptance_tests": 5 if _has_count(source_text, "Carrier domain acceptance test", "5") else 0,
        "blocked_overreach": 5 if _has_count(source_text, "Carrier domain blocked overreach", "5") else 0,
        ...
        "gap_resolution": 0 if _has_count(source_text, "Gap resolution", "0") else -1,
        "resolved_gap": 0 if _has_count(source_text, "Resolved gap", "0") else -1,
        "unresolved_gap": 7 if _has_count(source_text, "Unresolved gap", "7") else 0,
        "remaining_blocking_gap": 7 if _has_count(source_text, "Remaining blocking gap", "7") else 0,
    ...
    return [
        {
            ...
            "audit_focus": "carrier source",
            "accepted_claim": "Sigma_A carrier references are restricted to the finite R-path quotient carrier",
            "blocked_claim": "raw biological, clinical, continuous, or undefined domains may serve as Sigma_A carrier domains",
            "audit_status": "passed for targeted gap resolution decision",
        },
        {
            "audit_id": "CDEBA-002",
            "evidence_id": "SPCDEE-002",
            "audit_focus": "carrier element shape",
            "accepted_claim": "accepted elements must be quotient-class terms over the finite R-path kernel",
            "blocked_claim": "arbitrary nodes, phenotypes, interventions, or observations count as carrier elements",
            "audit_status": "passed for targeted gap resolution decision",
        },
        {
            "audit_id": "CDEBA-003",
            "evidence_id": "SPCDEE-003",
            "audit_focus": "no domain import",
            "accepted_claim": "constraint-region, causal-mass, and observer-projection dependencies stay outside the carrier",
            "blocked_claim": "dependency terms enlarge the Sigma_A carrier",
            "audit_status": "passed for targeted gap resolution decision",
        },
        {
            "audit_id": "CDEBA-004",
            "evidence_id": "SPCDEE-004",
            "audit_focus": "finite closure guard",
            "accepted_claim": "carrier finiteness is inherited from the finite R-path quotient construction",
            "blocked_claim": "open-ended semantic, empirical, clinical, or biological universes close the carrier",
            "audit_status": "passed for targeted gap resolution decision",
        },
        {
            "audit_id": "CDEBA-005",
            ...
            "accepted_claim": "carrier-domain evidence constrains evaluation domain only",
            "blocked_claim": "carrier-domain evidence creates predicate completion, theorem proof, or readiness",
            "audit_status": "passed for targeted gap resolution decision",
        },
    ]
...
def render_report() -> str:
    ...
    lines = [
        "# Carrier-Domain Evidence Boundary Audit Plus Gap Resolution Decision v8.89",
        "",
        "## Purpose",
        "",
        "Audit the v8.88 carrier-domain exactness evidence and record a targeted gap resolution decision for SPCHG-001 linked to SPCC-001, while keeping completion decision, completion execution, completion authorization, definition execution, new draft clause creation, new completion criterion creation, predicate definition completion, theorem proof, proof execution, proof assistant verification, completed formalization, framework-level proof, external validation, citation addition, and manuscript readiness counts at zero.",
        "",
        "## Source artifact",
        "",
        ...
        "",
        "## Carrier-domain evidence boundary audit rows",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            f"- Carried carrier domain exactness evidence execution count: {carried['evidence_execution']}",
            f"- Carried carrier domain evidence row count: {carried['evidence_rows']}",
            f"- Carried carrier domain scope rule count: {carried['scope_rules']}",
            f"- Carried carrier domain acceptance test count: {carried['acceptance_tests']}",
            f"- Carried carrier domain blocked overreach count: {carried['blocked_overreach']}",
            ...
            f"- Carried gap resolution count: {carried['gap_resolution']}",
            ...
            "## Warnings",
            "",
            "- This milestone audits carrier-domain evidence and records a targeted gap resolution decision only.",
            "- Resolving SPCHG-001 does not authorize completion execution or predicate completion.",
...
def run():
    ...
    if not source_exists:
        errors.append("Missing required v8.88 carrier-domain evidence source artifact.")
    ...
    expected_carried = {
        ...
        "resolved_gap": 0,
        "unresolved_gap": 7,
        "remaining_blocking_gap": 7,
        "new_completion_decision": 0,
        "new_completion_execution": 0,
    ...
    return SimpleNamespace(
        title="Carrier-Domain Evidence Boundary Audit Plus Gap Resolution Decision v8.89",
        output_path=OUTPUT_PATH,
        source_path=SOURCE_PATH,
        source_artifact_count=1 if source_exists else 0,
        missing_source_artifact_count=0 if source_exists else 1,
        carrier_domain_evidence_boundary_audit_count=1,
        audited_carrier_domain_evidence_count=1,
        carrier_domain_boundary_audit_row_count=5,
        ...
        carried_carrier_domain_exactness_evidence_execution_count=carried["evidence_execution"],
        carried_carrier_domain_evidence_row_count=carried["evidence_rows"],
        carried_carrier_domain_scope_rule_count=carried["scope_rules"],
        carried_carrier_domain_acceptance_test_count=carried["acceptance_tests"],
        carried_carrier_domain_blocked_overreach_count=carried["blocked_overreach"],
        ...
        carried_gap_resolution_count=carried["gap_resolution"],
