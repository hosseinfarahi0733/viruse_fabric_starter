...
SOURCE_PATH = Path("outputs/reachability_preorder_theorem_attempt_v8_71.md")
OUTPUT_PATH = Path("outputs/mutual_reachability_equivalence_theorem_attempt_v8_72.md")


@dataclass(frozen=True)
class MutualReachabilityEquivalenceTheorem:
    theorem_id: str
    theorem_name: str
    depends_on: str
    formal_context: str
...
@dataclass(frozen=True)
class MutualReachabilityEquivalenceTheoremAttemptReport:
    title: str
    output_path: Path
    source_path: Path
    ...
    mutual_reachability_equivalence_theorem_attempt_count: int
    ...
    mutual_reachability_relation_definition_count: int
    equivalence_property_count: int
    theorem_dependency_count: int

    carried_reachability_preorder_theorem_attempt_count: int
    carried_reachability_relation_definition_count: int
    carried_preorder_property_count: int
...
def build_theorem() -> MutualReachabilityEquivalenceTheorem:
    return MutualReachabilityEquivalenceTheorem(
        theorem_id="PKT-004",
        theorem_name="Finite R-path mutual reachability equivalence theorem",
        depends_on=(
            "PKT-003 finite R-path reachability preorder theorem; "
            "Reach_R reflexivity; Reach_R transitivity"
        ),
        formal_context=(
            "Let S be a nonempty state space and let Reach_R be the finite R-path reachability relation "
            "from PKT-003. Define mutual reachability a ~_R b to hold exactly when Reach_R(a,b) "
            "and Reach_R(b,a)."
        ),
        statement=(
        ...
        proof_steps=[
            ...
            "By PKT-003, Reach_R is reflexive, so Reach_R(a,a) holds.",
            "Thus Reach_R(a,a) and Reach_R(a,a) both hold, so a ~_R a.",
            "Therefore ~_R is reflexive.",
            "Symmetry: assume a ~_R b.",
            "By definition of ~_R, Reach_R(a,b) and Reach_R(b,a) both hold.",
            "Reordering the two conjuncts gives Reach_R(b,a) and Reach_R(a,b), so b ~_R a.",
            "Therefore ~_R is symmetric.",
            "Transitivity: assume a ~_R b and b ~_R c.",
            "From a ~_R b, obtain Reach_R(a,b) and Reach_R(b,a).",
            "From b ~_R c, obtain Reach_R(b,c) and Reach_R(c,b).",
            "By transitivity of Reach_R from PKT-003, Reach_R(a,b) and Reach_R(b,c) imply Reach_R(a,c).",
            "Again by transitivity of Reach_R, Reach_R(c,b) and Reach_R(b,a) imply Reach_R(c,a).",
            "Thus Reach_R(a,c) and Reach_R(c,a), so a ~_R c.",
            "Therefore ~_R is transitive.",
            "Since ~_R is reflexive, symmetric, and transitive, ~_R is an equivalence relation on S.",
        ],
        proof_status="manual limited theorem proof executed for finite R-path mutual reachability equivalence",
...
def render_report() -> str:
    ...
    carried_reachability_attempt_count = 1 if _has_count(source_text, "Reachability preorder theorem attempt", "1") else 0
    carried_cumulative_limited_theorem_proven_count = 3 if _has_count(source_text, "Cumulative limited theorem proven", "3") else 0
    carried_reachability_relation_definition_count = 1 if _has_count(source_text, "Reachability relation definition", "1") else 0
    carried_preorder_property_count = 2 if _has_count(source_text, "Preorder property", "2") else 0
    ...
    mutual_reachability_equivalence_theorem_attempt_count = 1
    ...
    mutual_reachability_relation_definition_count = 1
    equivalence_property_count = 3
    ...
    if missing_source_artifact_count != 0:
        errors.append("Missing required v8.71 reachability preorder theorem source artifact.")
    if carried_reachability_attempt_count != 1:
        errors.append("Expected carried v8.71 reachability theorem attempt signal is absent.")
    if carried_cumulative_limited_theorem_proven_count != 3:
        errors.append("Expected carried cumulative limited theorem count is absent.")
    if carried_reachability_relation_definition_count != 1:
        errors.append("Expected carried reachability relation definition signal is absent.")
    if carried_preorder_property_count != 2:
        errors.append("Expected carried preorder property count is absent.")
    ...
    if cumulative_limited_theorem_proven_count != 4:
        errors.append("Expected four cumulative limited theorem proofs.")
    if mutual_reachability_relation_definition_count != 1:
        errors.append("Expected one mutual reachability relation definition.")
    if equivalence_property_count != 3:
        errors.append("Expected three equivalence properties: reflexivity, symmetry, transitivity.")
    ...
    lines = [
        ...
        "## Counts",
        "",
        f"- Mutual reachability equivalence theorem attempt count: {mutual_reachability_equivalence_theorem_attempt_count}",
        ...
        f"- Mutual reachability relation definition count: {mutual_reachability_relation_definition_count}",
        f"- Equivalence property count: {equivalence_property_count}",
        ...
        "",
        f"- Carried reachability preorder theorem attempt count: {carried_reachability_attempt_count}",
        f"- Carried reachability relation definition count: {carried_reachability_relation_definition_count}",
        f"- Carried preorder property count: {carried_preorder_property_count}",
    ...
    if overclaim_count != 0:
        errors.append("Overclaim pattern detected in v8.72 mutual reachability equivalence theorem report.")
    if invented_citation_like_pattern_count != 0:
        errors.append("Invented citation-like pattern detected in v8.72 mutual reachability equivalence theorem report.")
...
def run() -> MutualReachabilityEquivalenceTheoremAttemptReport:
    text = render_report()
    source_text = _read_source()
    ...
    return MutualReachabilityEquivalenceTheoremAttemptReport(
        title="Mutual Reachability Equivalence Theorem Attempt v8.72",
        ...
        equivalence_property_count=3,
        theorem_dependency_count=1,
        carried_reachability_preorder_theorem_attempt_count=carried_reachability_attempt_count,
        carried_reachability_relation_definition_count=carried_reachability_relation_definition_count,
        carried_preorder_property_count=carried_preorder_property_count,
