...
COUNTERS = {
    "controlled_r_a_definition_completion_execution_count": 1,
    "new_controlled_r_a_definition_completion_execution_count": 1,
    "r_a_definition_completion_execution_count": 1,
    ...
    "dependent_object_definition_completion_count": 5,
    "completed_dependent_object_definition_count": 5,

    "imported_controlled_m_c_definition_completion_execution_count": 1,
    ...
    "imported_dependent_object_definition_completion_count": 4,
    "imported_completed_dependent_object_definition_count": 4,
...
LABEL_OVERRIDES = {
    "controlled_r_a_definition_completion_execution_count": "Controlled R_A definition completion execution count",
    "new_controlled_r_a_definition_completion_execution_count": "New controlled R_A definition completion execution count",
    "r_a_definition_completion_execution_count": "R_A definition completion execution count",
    ...
    "imported_controlled_m_c_definition_completion_execution_count": "Imported controlled M_c definition completion execution count",
...
def _definition_block() -> str:
    return dedent(
        """
        ## Completed formal definition: R_A
        ...
        `R_A` is the A-indexed controlled relation object that links admissible states, regular transitions, observable representations, and controlled causal mass records without claiming theorem-level closure or full framework formalization.
        ...
        `R_A ⊆ X_A^adm × T_A^reg × O_A × Q_A^mass`

        ### Relation membership form

        For a tuple `(x, tau, o, q)` with:

        `x in X_A^adm`

        `tau = (x_0, x_1, i) in T_A^reg`
        ...
        1. `x` satisfies completed `Adm_A`;
        2. `tau` satisfies completed `C_reg`;
        3. `o` is produced through completed `Pi_obs`;
        4. `q` is produced through completed `M_c`;
        5. the tuple preserves A-indexed structural scope inherited from completed `Sigma_A`;
        6. relation membership records compatibility, not theorem-level equivalence;
        7. relation membership remains eligible for later `Traj_A` completion without implying that `Traj_A` is already complete.

        ### Compatibility boundary

        `R_A` is a controlled compatibility relation.

        `R_A` does not prove equivalence, uniqueness, conservation, completeness, or reachability.

        `R_A` does not validate empirical relation structure.

        `R_A` does not complete trajectory semantics.

        `R_A` only records controlled relation membership over the completed admissibility, regularity, observation, and mass carriers.

        ### Completion boundary

        This definition completes `R_A` only.
...
def _next_steps() -> str:
    return "\n".join(
        [
            "1. Execute controlled Traj_A definition completion.",
            "2. Integrate completed dependent-object definitions into a bundle only after all six completions exist.",
...
def build_report(source_text: str) -> CompletionResult:
    ...
    report = dedent(
        ...
        Can Viruse Fabric execute the fifth controlled dependent-object definition completion by completing `R_A` after official v8.152 M_c definition completion, while keeping Traj_A, all-dependent-object completion, bundle integration, full formalization, theorem candidate planning, theorem proof, proof assistant verification, validation, manuscript readiness, readiness approval, and new citations at zero?

        ## Source artifact

        - `outputs/controlled_m_c_definition_completion_execution_v8_152.md`

        ## Execution interpretation

        v8.153 executes one controlled dependent-object definition completion.

        The newly completed object is `R_A`.

        The previously completed dependent objects `Adm_A`, `C_reg`, `Pi_obs`, and `M_c` remain completed.
        ...
        This milestone completes exactly one additional dependent-object definition: `R_A`.
        ...
        The project has executed one controlled R_A definition completion and records Adm_A, C_reg, Pi_obs, M_c, and R_A as five completed dependent-object definitions while keeping Traj_A, all-dependent-object completion, dependent-object bundle integration, full formalization completion, theorem candidate planning, theorem proof, proof assistant verification, external validation, independent experiment, manuscript readiness, readiness approval, and new citations at zero.
        """
    ).strip() + "\n"
    ...
    warnings = [
        "This milestone completes R_A only as the fifth completed dependent object.",
        "Traj_A remains uncompleted.",
