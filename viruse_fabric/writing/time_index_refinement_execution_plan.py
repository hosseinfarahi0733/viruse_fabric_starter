...
class TimeIndexRefinementExecutionPlanReport:
...
class TimeIndexRefinementExecutionPlanBuilder:
    """Build v8.130 Time-index refinement execution plan artifact.

    Boundary discipline:
    - This milestone plans time-index refinement only.
    ...
    title = "Time-index Refinement Execution Plan v8.130"
    source_artifact = Path("outputs/whole_sigma_a_draft_assembly_boundary_audit_v8_129.md")
    output_path = Path("outputs/time_index_refinement_execution_plan_v8_130.md")

    plan_rows = [
        {
            "plan_id": "TIME-IDX-REFINE-PLAN-001",
            "planned_focus": "T_A target identification",
            "planned_action": "plan refinement of the deferred T_A slot carried by the whole Draft Sigma_A shell",
            "required_precondition": "v8.129 whole Draft Sigma_A shell boundary audit must remain clean",
            "blocked_overreach": "do not execute time-index refinement in v8.130",
        },
        {
            "plan_id": "TIME-IDX-REFINE-PLAN-002",
            "planned_focus": "three-time structure",
            "planned_action": "plan T_A as a future time-index layer compatible with t1, t2, and t3 bookkeeping",
            "required_precondition": "time-index layer must remain separate from Sigma_A refinement",
            "blocked_overreach": "do not execute Sigma_A refinement",
        },
        {
            "plan_id": "TIME-IDX-REFINE-PLAN-003",
            "planned_focus": "draft shell linkage",
            "planned_action": "plan how T_A will attach to the assembled Draft Sigma_A shell",
            "required_precondition": "whole Draft Sigma_A shell is carried from v8.128 and audited in v8.129",
            "blocked_overreach": "do not execute new whole Sigma_A draft assembly",
        },
        {
            "plan_id": "TIME-IDX-REFINE-PLAN-004",
            "planned_focus": "carrier-clause separation",
            "planned_action": "plan T_A refinement without changing carrier(Draft Sigma_A) := X_A^tp",
            "required_precondition": "carrier-slot clause must remain unchanged",
            "blocked_overreach": "do not create a new Sigma_A draft clause",
        },
        {
            "plan_id": "TIME-IDX-REFINE-PLAN-005",
            "planned_focus": "dependent-object separation",
            "planned_action": "plan T_A refinement while keeping Adm_A, C_reg, Pi_obs, M_c, R_A, and Traj_A deferred",
            "required_precondition": "dependent-object definitions remain separate milestones",
            "blocked_overreach": "do not execute definitions",
        },
        {
            "plan_id": "TIME-IDX-REFINE-PLAN-006",
            "planned_focus": "definition boundary",
            "planned_action": "keep time-index refinement planning separate from Sigma_A definition completion",
        ...
        {
            "plan_id": "TIME-IDX-REFINE-PLAN-007",
            "planned_focus": "audit traceability",
            "planned_action": "plan Ann_A traceability for future time-index refinement audit",
            "required_precondition": "Ann_A remains auxiliary and audit-facing",
            "blocked_overreach": "do not add hidden mathematical structure",
        },
        {
            "plan_id": "TIME-IDX-REFINE-PLAN-008",
            "planned_focus": "proof-readiness separation",
            "planned_action": "preserve theorem/proof/validation/readiness as downstream layers",
            "required_precondition": "time-index refinement planning must not imply proof readiness",
            "blocked_overreach": "do not create theorem candidates, proofs, validation, citations, or readiness approval",
        },
    ]

    execution_gates = [
        "Future time-index refinement must operate only on the deferred T_A slot.",
        "Future time-index refinement must preserve the assembled whole Draft Sigma_A shell boundary.",
        "Future time-index refinement must not modify the carrier-slot clause carrier(Draft Sigma_A) := X_A^tp.",
        "Future time-index refinement must keep Adm_A, C_reg, Pi_obs, M_c, R_A, and Traj_A deferred.",
        "Future time-index refinement must not execute Sigma_A refinement.",
        "Future time-index refinement must not execute definitions or complete Sigma_A.",
        "Future time-index refinement must preserve Ann_A as audit traceability only.",
        "Future time-index refinement must not create theorem candidates, proofs, validation, citations, or readiness approval.",
    ]

    def run(self) -> TimeIndexRefinementExecutionPlanReport:
        errors: list[str] = []
        warnings: list[str] = []
        ...
        warnings.extend([
            "This milestone plans time-index refinement only.",
            "Time-index refinement execution remains zero in v8.130.",
            ...
            "No new whole Sigma_A draft assembly and no new Sigma_A draft clause are created in v8.130.",
        ])

        counts: dict[str, int] = {
            "source_artifact_count": 1 if source_exists else 0,
            "missing_source_artifact_count": 0 if source_exists else 1,
            "time_index_refinement_execution_plan_count": 1,
            "sigma_a_time_index_refinement_execution_plan_count": 1,
            "t_a_refinement_execution_plan_count": 1,
            "time_index_refinement_plan_row_count": len(self.plan_rows),
            "time_index_refinement_execution_gate_count": len(self.execution_gates),
            "planned_t_a_refinement_scope_count": 1,
            "planned_draft_shell_time_index_link_count": 1,
            "planned_three_time_structure_count": 1,
            "planned_time_index_boundary_guard_count": len(self.plan_rows),
            "planned_time_index_blocked_overreach_count": len(self.plan_rows),
            "planned_carrier_clause_preservation_count": 1,
            "planned_dependent_object_deferral_count": 6,
            "carried_whole_sigma_a_draft_assembly_boundary_audit_count": carried.get("Whole Sigma_A draft assembly boundary audit count", 1),
            "carried_sigma_a_whole_draft_assembly_boundary_audit_count": carried.get("Sigma_A whole draft assembly boundary audit count", 1),
            "carried_sigma_a_draft_assembly_boundary_audit_count": carried.get("Sigma_A draft assembly boundary audit count", 1),
            "carried_whole_sigma_a_draft_assembly_boundary_audit_row_count": carried.get("Whole Sigma_A draft assembly boundary audit row count", 8),
            "carried_whole_sigma_a_draft_assembly_boundary_preserved_count": carried.get("Whole Sigma_A draft assembly boundary preserved count", 8),
            "carried_whole_sigma_a_draft_assembly_boundary_audit_finding_count": carried.get("Whole Sigma_A draft assembly boundary audit finding count", 8),
            "carried_assembled_whole_sigma_a_draft_audited_count": carried.get("Assembled whole Sigma_A draft audited count", 1),
            "carried_assembled_whole_sigma_a_draft_shell_audited_count": carried.get("Assembled whole Sigma_A draft shell audited count", 1),
            "carried_imported_carrier_slot_clause_audited_count": carried.get("Imported carrier-slot clause audited count", 1),
            "carried_dependent_object_slot_deferral_preserved_count": carried.get("Dependent object slot deferral preserved count", 6),
            "carried_time_index_slot_deferral_preserved_count": carried.get("Time-index slot deferral preserved count", 1),
            "carried_new_whole_sigma_a_draft_assembly_blocker_count": carried.get("New whole Sigma_A draft assembly blocker count", 1),
            "carried_new_sigma_a_draft_clause_blocker_count": carried.get("New Sigma_A draft clause blocker count", 1),
            "carried_time_index_refinement_blocker_count": carried.get("Time-index refinement blocker count", 1),
            ...
            "carried_whole_sigma_a_draft_assembly_execution_count": carried.get("Carried whole Sigma_A draft assembly execution count", 1),
            "carried_sigma_a_whole_draft_assembly_execution_count": carried.get("Carried Sigma_A whole draft assembly execution count", 1),
            "carried_sigma_a_draft_assembly_execution_count": carried.get("Carried Sigma_A draft assembly execution count", 1),
        ...
        if counts["overclaim_count"] != 0:
            errors.append("Overclaim detected in v8.130 report.")
        if counts["invented_citation_like_pattern_count"] != 0:
            errors.append("Invented citation-like pattern detected in v8.130 report.")
        if counts["time_index_refinement_execution_plan_count"] != 1:
            errors.append("v8.130 must produce exactly one time-index refinement execution plan.")
        if counts["t_a_refinement_execution_plan_count"] != 1:
            errors.append("v8.130 must plan exactly one T_A refinement scope.")
        if counts["time_index_refinement_plan_row_count"] != 8:
            errors.append("v8.130 must include exactly eight time-index refinement plan rows.")
        if counts["time_index_refinement_execution_gate_count"] != 8:
            errors.append("v8.130 must include exactly eight time-index refinement execution gates.")
        if counts["time_index_refinement_execution_count"] != 0:
            errors.append("v8.130 must not execute time-index refinement.")
        if counts["sigma_a_refinement_execution_count"] != 0:
            errors.append("v8.130 must not execute Sigma_A refinement.")
        if counts["definition_execution_count"] != 0:
            errors.append("v8.130 must not execute definitions.")
        if counts["sigma_a_definition_completion_count"] != 0:
            errors.append("v8.130 must not complete Sigma_A definition.")
        if counts["theorem_candidate_plan_count"] != 0:
            errors.append("v8.130 must not create theorem candidates.")
        if counts["new_theorem_proven_count"] != 0:
            errors.append("v8.130 must not prove a theorem.")
        ...
        return TimeIndexRefinementExecutionPlanReport(
            title=self.title,
    ...
    def _format_label(self, key: str) -> str:
        overrides = {
            "time_index_refinement_execution_plan_count": "Time-index refinement execution plan count",
            "sigma_a_time_index_refinement_execution_plan_count": "Sigma_A time-index refinement execution plan count",
            "t_a_refinement_execution_plan_count": "T_A refinement execution plan count",
            "time_index_refinement_plan_row_count": "Time-index refinement plan row count",
            "time_index_refinement_execution_gate_count": "Time-index refinement execution gate count",
            "planned_t_a_refinement_scope_count": "Planned T_A refinement scope count",
            "planned_draft_shell_time_index_link_count": "Planned draft shell time-index link count",
            "planned_three_time_structure_count": "Planned three-time structure count",
            "planned_time_index_boundary_guard_count": "Planned time-index boundary guard count",
            "planned_time_index_blocked_overreach_count": "Planned time-index blocked-overreach count",
            ...
            "carried_whole_sigma_a_draft_assembly_boundary_audit_row_count": "Carried whole Sigma_A draft assembly boundary audit row count",
            "carried_whole_sigma_a_draft_assembly_boundary_preserved_count": "Carried whole Sigma_A draft assembly boundary preserved count",
            "carried_whole_sigma_a_draft_assembly_boundary_audit_finding_count": "Carried whole Sigma_A draft assembly boundary audit finding count",
            "carried_assembled_whole_sigma_a_draft_audited_count": "Carried assembled whole Sigma_A draft audited count",
            ...
            "carried_dependent_object_slot_deferral_preserved_count": "Carried dependent object slot deferral preserved count",
            "carried_time_index_slot_deferral_preserved_count": "Carried time-index slot deferral preserved count",
            "carried_new_whole_sigma_a_draft_assembly_blocker_count": "Carried new whole Sigma_A draft assembly blocker count",
            "carried_new_sigma_a_draft_clause_blocker_count": "Carried new Sigma_A draft clause blocker count",
            "carried_time_index_refinement_blocker_count": "Carried time-index refinement blocker count",
    ...
    def _render_report(self, counts: dict[str, int], warnings: list[str]) -> str:
        ...
        lines.append(
            "Can Viruse Fabric plan time-index refinement after the v8.129 whole Sigma_A draft assembly boundary audit while keeping actual time-index refinement, "
            "Sigma_A refinement, definition execution, Sigma_A definition completion, theorem candidate planning, theorem proof, proof assistant verification, "
        ...
        lines.append("- Milestone type: Time-index refinement execution plan only")
        ...
        lines.append("")
        lines.append("## Time-index refinement execution gates")
        for index, gate in enumerate(self.execution_gates, start=1):
            lines.append(f"{index}. {gate}")
        lines.append("")
        lines.append("## Boundary statement")
        lines.append(
            "This artifact plans time-index refinement execution only. "
            "It plans a future T_A refinement scope, but it does not execute time-index refinement, "
        ...
        counter_order = [
            "time_index_refinement_execution_plan_count",
            "sigma_a_time_index_refinement_execution_plan_count",
            "t_a_refinement_execution_plan_count",
            "time_index_refinement_plan_row_count",
            "time_index_refinement_execution_gate_count",
            "planned_t_a_refinement_scope_count",
            "planned_draft_shell_time_index_link_count",
            "planned_three_time_structure_count",
            "planned_time_index_boundary_guard_count",
            "planned_time_index_blocked_overreach_count",
            ...
            "carried_whole_sigma_a_draft_assembly_boundary_audit_row_count",
            "carried_whole_sigma_a_draft_assembly_boundary_preserved_count",
            "carried_whole_sigma_a_draft_assembly_boundary_audit_finding_count",
            "carried_assembled_whole_sigma_a_draft_audited_count",
            "carried_assembled_whole_sigma_a_draft_shell_audited_count",
            "carried_imported_carrier_slot_clause_audited_count",
            "carried_dependent_object_slot_deferral_preserved_count",
            "carried_time_index_slot_deferral_preserved_count",
            "carried_new_whole_sigma_a_draft_assembly_blocker_count",
            "carried_new_sigma_a_draft_clause_blocker_count",
            "carried_time_index_refinement_blocker_count",
        ...
        lines.append(
            "The v8.130 artifact plans time-index refinement around the deferred T_A slot carried by the assembled and audited whole Draft Sigma_A shell. "
            "This is planning only. It does not execute time-index refinement, does not execute T_A refinement, does not execute Sigma_A refinement, "
        ...
        next_steps = [
            "Audit the time-index refinement execution plan before executing T_A refinement.",
...
if __name__ == "__main__":
    report = TimeIndexRefinementExecutionPlanBuilder().run()
