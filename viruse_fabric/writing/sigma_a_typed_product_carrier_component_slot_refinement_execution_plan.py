...
class SigmaATypedProductCarrierComponentSlotRefinementExecutionPlanReport:
...
class SigmaATypedProductCarrierComponentSlotRefinementExecutionPlanBuilder:
    """Build v8.112 Sigma_A typed-product carrier component-slot refinement execution plan.

    Boundary discipline:
    - This milestone plans component-slot refinement execution.
    - It does not execute component-slot refinement.
    ...
    title = "Sigma_A Typed-Product Carrier Component-Slot Refinement Execution Plan v8.112"
    source_artifact = Path("outputs/sigma_a_typed_product_carrier_component_slot_audit_execution_v8_111.md")
    output_path = Path("outputs/sigma_a_typed_product_carrier_component_slot_refinement_execution_plan_v8_112.md")

    planned_slot_rows = [
        {
            "plan_id": "X-A-SLOT-REF-PLAN-001",
            "slot": "B_A biological-state component",
            "planned_refinement": "Specify candidate biological state typing and admissibility-facing fields.",
            "dependency": "Adm_A domain planning",
            "blocked_overreach": "Do not execute B_A refinement in this milestone.",
        },
        {
            "plan_id": "X-A-SLOT-REF-PLAN-002",
            "slot": "S_A spatial-context component",
            "planned_refinement": "Specify spatial context typing options and compatibility checks against C_reg and Pi_obs.",
            "dependency": "C_reg and Pi_obs compatibility planning",
            "blocked_overreach": "Do not execute S_A refinement in this milestone.",
        },
        {
            "plan_id": "X-A-SLOT-REF-PLAN-003",
            "slot": "Q_A constraint-status component",
            "planned_refinement": "Plan status flags or judgments that support but do not replace C_reg membership.",
            "dependency": "C_reg membership semantics",
            "blocked_overreach": "Do not complete C_reg or Sigma_A.",
        },
        {
            "plan_id": "X-A-SLOT-REF-PLAN-004",
            "slot": "O_A observer-visible component",
            "planned_refinement": "Plan observed-state field typing and full-state separation rules.",
            "dependency": "Pi_obs domain and codomain planning",
            "blocked_overreach": "Do not define Pi_obs in this milestone.",
        },
        {
            "plan_id": "X-A-SLOT-REF-PLAN-005",
            "slot": "K_A causal-mass support component",
            "planned_refinement": "Plan support fields for future M_c typing without selecting measure-like semantics.",
            "dependency": "M_c typing flexibility",
            "blocked_overreach": "Do not define M_c or execute causal-mass refinement.",
        },
        {
            "plan_id": "X-A-SLOT-REF-PLAN-006",
            "slot": "Rdy_A transition-readiness component",
            "planned_refinement": "Plan transition-readiness predicates required by future R_A checks.",
            "dependency": "R_A transition refinement boundary",
            "blocked_overreach": "Do not execute R_A refinement.",
        },
        {
            "plan_id": "X-A-SLOT-REF-PLAN-007",
            "slot": "J_A trajectory-support component",
            "planned_refinement": "Plan trajectory-support fields needed before Traj_A construction.",
            "dependency": "Traj_A construction boundary",
            "blocked_overreach": "Do not complete Traj_A or theorem candidates.",
        },
        {
            "plan_id": "X-A-SLOT-REF-PLAN-008",
            "slot": "Ann_A audit-annotation component",
            "planned_refinement": "Plan audit-facing annotations while keeping them auxiliary and non-mathematical unless later formalized.",
            "dependency": "audit traceability",
            "blocked_overreach": "Do not turn annotation into hidden mathematical structure.",
        },
    ]

    execution_gate_rows = [
        "All eight component slots must have explicit candidate type forms before refinement execution.",
        "No slot may silently replace C_reg, Pi_obs, M_c, R_A, or Traj_A.",
        "Auxiliary annotation must remain separate from mathematical carrier structure.",
        "Carrier refinement execution must remain separate from Sigma_A definition completion.",
        "Time-index refinement must remain separate from typed-product slot refinement.",
        "Theorem candidate planning must wait for completed definitions.",
        "Proof assistant verification must wait for proof execution artifacts.",
        "Validation, citations, and manuscript readiness must remain separate milestones.",
    ]

    def run(self) -> SigmaATypedProductCarrierComponentSlotRefinementExecutionPlanReport:
        errors: list[str] = []
        warnings: list[str] = []
        ...
        warnings.extend([
            "This milestone plans typed-product component-slot refinement execution only.",
            "Component-slot refinement execution remains zero in v8.112.",
            "Carrier refinement and Sigma_A refinement remain zero in v8.112.",
        ...
        counts: dict[str, int] = {
            ...
            "sigma_a_typed_product_carrier_component_slot_refinement_execution_plan_count": 1,
            "component_slot_refinement_plan_row_count": len(self.planned_slot_rows),
            "component_slot_refinement_execution_gate_count": len(self.execution_gate_rows),
            "planned_component_slot_count": len(self.planned_slot_rows),
            "planned_component_slot_dependency_count": len(self.planned_slot_rows),
            "planned_component_slot_blocked_overreach_count": len(self.planned_slot_rows),
            "carried_sigma_a_typed_product_carrier_component_slot_audit_execution_count": carried.get("Sigma_A typed-product carrier component-slot audit execution count", 1),
            "carried_typed_product_carrier_component_slot_audited_count": carried.get("Typed-product carrier component slot audited count", 8),
            "carried_component_slot_compatibility_check_count": carried.get("Component-slot compatibility check count", 8),
            "carried_component_slot_unresolved_status_count": carried.get("Component-slot unresolved status count", 8),
            "carried_selected_typed_product_carrier_count": carried.get("Selected typed-product carrier carried count", 1),
            ...
            "carried_carrier_type_selection_count": carried.get("Carried carrier type selection count", 1),
            "carried_carrier_type_selection_decision_count": carried.get("Carried carrier type selection decision count", 1),
            ...
            "component_slot_refinement_execution_count": 0,
        ...
        if counts["overclaim_count"] != 0:
            errors.append("Overclaim detected in v8.112 report.")
        if counts["invented_citation_like_pattern_count"] != 0:
            errors.append("Invented citation-like pattern detected in v8.112 report.")
        if counts["sigma_a_typed_product_carrier_component_slot_refinement_execution_plan_count"] != 1:
            errors.append("v8.112 must produce exactly one component-slot refinement execution plan.")
        if counts["component_slot_refinement_plan_row_count"] != 8:
            errors.append("v8.112 must plan exactly eight component slots.")
        if counts["component_slot_refinement_execution_gate_count"] != 8:
            errors.append("v8.112 must include exactly eight execution gates.")
        if counts["component_slot_refinement_execution_count"] != 0:
            errors.append("v8.112 must not execute component-slot refinement.")
        if counts["typed_product_carrier_refinement_execution_count"] != 0:
            errors.append("v8.112 must not execute typed-product carrier refinement.")
        if counts["carrier_refinement_execution_count"] != 0:
            errors.append("v8.112 must not execute carrier refinement.")
        if counts["sigma_a_refinement_execution_count"] != 0:
            errors.append("v8.112 must not execute Sigma_A refinement.")
        if counts["sigma_a_definition_completion_count"] != 0:
            errors.append("v8.112 must not complete Sigma_A definition.")
        if counts["theorem_candidate_plan_count"] != 0:
            errors.append("v8.112 must not create theorem candidates.")
        if counts["new_theorem_proven_count"] != 0:
            errors.append("v8.112 must not prove a theorem.")
        ...
        return SigmaATypedProductCarrierComponentSlotRefinementExecutionPlanReport(
            title=self.title,
    ...
    def _format_label(self, key: str) -> str:
        overrides = {
            "sigma_a_typed_product_carrier_component_slot_refinement_execution_plan_count": "Sigma_A typed-product carrier component-slot refinement execution plan count",
            "component_slot_refinement_plan_row_count": "Component-slot refinement plan row count",
            "component_slot_refinement_execution_gate_count": "Component-slot refinement execution gate count",
            ...
            "carried_component_slot_compatibility_check_count": "Carried component-slot compatibility check count",
            "carried_component_slot_unresolved_status_count": "Carried component-slot unresolved status count",
    ...
    def _render_report(self, counts: dict[str, int], warnings: list[str]) -> str:
        ...
        lines.append(
            "Can Viruse Fabric plan typed-product carrier component-slot refinement execution after component-slot audit "
            "while keeping component-slot refinement execution, carrier refinement execution, Sigma_A refinement execution, "
        ...
        lines.append("- Milestone type: Sigma_A typed-product carrier component-slot refinement execution plan only")
        lines.append("- Component-slot refinement execution after this milestone: not executed")
        ...
        lines.append("")
        lines.append("## Component-slot refinement plan rows")
        lines.append("")
        lines.append("| Plan ID | Slot | Planned refinement | Dependency | Blocked overreach |")
        lines.append("|---|---|---|---|---|")
        for row in self.planned_slot_rows:
            lines.append(
                f"| {row['plan_id']} | {row['slot']} | {row['planned_refinement']} | "
                f"{row['dependency']} | {row['blocked_overreach']} |"
            )
        lines.append("")
        lines.append("## Refinement execution gates")
        for index, gate in enumerate(self.execution_gate_rows, start=1):
            lines.append(f"{index}. {gate}")
        lines.append("")
        lines.append("## Boundary statement")
        lines.append(
            "This artifact plans Sigma_A typed-product carrier component-slot refinement execution only. "
            "It does not execute component-slot refinement, does not execute typed-product carrier refinement, "
            "does not execute carrier-type refinement, does not execute carrier refinement, does not execute time-index refinement, "
            "does not execute Sigma_A refinement, does not perform a new carrier type selection, "
        ...
        counter_order = [
            "sigma_a_typed_product_carrier_component_slot_refinement_execution_plan_count",
            "component_slot_refinement_plan_row_count",
            "component_slot_refinement_execution_gate_count",
            "planned_component_slot_count",
            "planned_component_slot_dependency_count",
            "planned_component_slot_blocked_overreach_count",
            ...
            "carried_component_slot_compatibility_check_count",
            "carried_component_slot_unresolved_status_count",
        ...
        lines.append(
            "The v8.112 artifact plans typed-product carrier component-slot refinement execution. "
            "This is a planning artifact only. It does not execute component-slot refinement, does not execute carrier refinement, "
        ...
        next_steps = [
            "Execute typed-product component-slot refinement only after this plan is closed.",
            "Keep component-slot refinement execution separate from Sigma_A completion.",
            "Keep carrier refinement execution separate from time-index refinement.",
    ...
    def _count_prohibited_behaviors(self, text: str) -> int:
        prohibited = [
            "does not execute component-slot refinement",
    ...
    def _count_overclaims(self, text: str) -> int:
        forbidden_positive_counter_names = {
            "component slot refinement execution count",
            "component-slot refinement execution count",
        ...
        unsafe_phrases = [
            "component-slot refinement executed",
...
if __name__ == "__main__":
    report = SigmaATypedProductCarrierComponentSlotRefinementExecutionPlanBuilder().run()
