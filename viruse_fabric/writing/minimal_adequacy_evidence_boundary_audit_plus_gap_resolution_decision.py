...
SOURCE_PATH = Path("outputs/minimal_adequacy_condition_evidence_execution_v8_96.md")
OUTPUT_PATH = Path("outputs/minimal_adequacy_evidence_boundary_audit_plus_gap_resolution_decision_v8_97.md")
...
def _overclaim_count(text: str) -> int:
    prohibited_assertions = [
        ...
        "minimal adequacy completes sigma_a",
        "minimal adequacy proves sigma_a",
        "minimal adequacy evidence proves completion",
        "minimal adequacy audit proves sigma_a",
    ]
    lowered = text.lower()
...
def _carried_counts(source_text: str) -> dict[str, int]:
    return {
        "minimal_adequacy_evidence_execution": 1 if _has_count(source_text, "Minimal adequacy condition evidence execution", "1") else 0,
        "minimal_adequacy_evidence_rows": 5 if _has_count(source_text, "Minimal adequacy evidence row", "5") else 0,
        "minimal_adequacy_scope_rules": 3 if _has_count(source_text, "Minimal adequacy scope rule", "3") else 0,
        "minimal_adequacy_acceptance_tests": 5 if _has_count(source_text, "Minimal adequacy acceptance test", "5") else 0,
        "minimal_adequacy_blocked_overreach": 5 if _has_count(source_text, "Minimal adequacy blocked overreach", "5") else 0,
        ...
        "prior_resolved_gaps_retained": 4 if _has_count(source_text, "Prior resolved gaps retained", "4") else 0,
        ...
        "resolved_gap": 4 if _has_count(source_text, "Resolved gap", "4") else 0,
        "unresolved_gap": 3 if _has_count(source_text, "Unresolved gap", "3") else 0,
        "remaining_blocking_gap": 3 if _has_count(source_text, "Remaining blocking gap", "3") else 0,
...
def _audit_rows() -> list[dict[str, str]]:
    return [
        {
            "audit_id": "MACBA-001",
            "evidence_id": "MAC-EE-001",
            "audit_focus": "minimal input set",
            "accepted_claim": "the minimal input set must name least carrier-local inputs for a candidate check",
            "blocked_claim": "broad narrative completeness counts as minimal adequacy",
            "audit_status": "passed for targeted gap resolution decision",
        },
        {
            "audit_id": "MACBA-002",
            "evidence_id": "MAC-EE-002",
            "audit_focus": "adequacy threshold",
            "accepted_claim": "adequacy threshold must separate sufficient local evidence from completion decision or theorem proof",
            "blocked_claim": "threshold language substitutes for predicate completion",
            "audit_status": "passed for targeted gap resolution decision",
        },
        {
            "audit_id": "MACBA-003",
            "evidence_id": "MAC-EE-003",
            "audit_focus": "insufficiency witness",
            "accepted_claim": "insufficiency witnesses must be preserved when local evidence falls below threshold",
            "blocked_claim": "insufficient cases are silently converted into positive evidence",
            "audit_status": "passed for targeted gap resolution decision",
        },
        {
            "audit_id": "MACBA-004",
            "evidence_id": "MAC-EE-004",
            "audit_focus": "non-expansive guard",
            "accepted_claim": "minimal adequacy must remain finite, carrier-local, and non-expansive",
            "blocked_claim": "minimal adequacy expands into global validity, proof completion, external validation, or readiness",
            "audit_status": "passed for targeted gap resolution decision",
        },
        {
            "audit_id": "MACBA-005",
            "evidence_id": "MAC-EE-005",
            "audit_focus": "draft-predicate boundary",
            "accepted_claim": "minimal adequacy evidence supports the fifth gap but does not finish Sigma_A",
            "blocked_claim": "minimal adequacy evidence creates predicate completion, theorem proof, or readiness",
            "audit_status": "passed for targeted gap resolution decision",
        },
    ]
...
def render_report() -> str:
    ...
    lines = [
        "# Minimal Adequacy Evidence Boundary Audit Plus Gap Resolution Decision v8.97",
        "",
        "## Purpose",
        "",
        "Audit the v8.96 minimal adequacy condition evidence and record a targeted gap resolution decision for SPCHG-005 linked to SPCC-005, while keeping completion decision, completion execution, completion authorization, definition execution, new draft clause creation, new completion criterion creation, predicate definition completion, theorem proof, proof execution, proof assistant verification, completed formalization, framework-level proof, external validation, citation addition, and manuscript readiness counts at zero.",
        "",
        "## Source artifact",
        "",
        ...
        "- Previously resolved gaps retained: SPCHG-001, SPCHG-002, SPCHG-003, and SPCHG-004",
        "- Resolved gap count: 5",
        "- Newly resolved gap count: 1",
        "- Unresolved gap count after decision: 2",
        "- Remaining blocking gap count: 2",
        "- Completion execution authorized: no",
        "",
        "## Minimal adequacy evidence boundary audit rows",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            "## Carried v8.96 signals",
            "",
            f"- Carried minimal adequacy condition evidence execution count: {carried['minimal_adequacy_evidence_execution']}",
            f"- Carried minimal adequacy evidence row count: {carried['minimal_adequacy_evidence_rows']}",
            f"- Carried minimal adequacy scope rule count: {carried['minimal_adequacy_scope_rules']}",
            f"- Carried minimal adequacy acceptance test count: {carried['minimal_adequacy_acceptance_tests']}",
            f"- Carried minimal adequacy blocked overreach count: {carried['minimal_adequacy_blocked_overreach']}",
            ...
            "",
            "The v8.97 artifact audits the v8.96 minimal adequacy condition evidence and records a targeted gap resolution decision for SPCHG-005 only. It does not create a completion decision, does not authorize completion execution, performs no definition execution, creates no new draft clause, creates no new completion criterion, completes no stabilization predicate definition, completes no attractor-class definition, completes no constraint-region definition, completes no causal-mass definition, completes no observer-projection definition, proves no theorem, performs no proof execution, performs no proof assistant verification, adds no citation, completes no external validation, and creates no manuscript readiness.",
            "",
            "## Critical reviewer note",
            "",
            "Resolving SPCHG-005 reduces the conditional-hold gap register from three unresolved blocking gaps to two unresolved blocking gaps. It does not lift the conditional hold on Sigma_A completion execution.",
            "",
            "## Warnings",
            "",
            "- This milestone audits minimal adequacy evidence and records a targeted gap resolution decision only.",
            "- Resolving SPCHG-005 does not authorize completion execution or predicate completion.",
...
def run():
    ...
    if not source_exists:
        errors.append("Missing required v8.96 minimal adequacy evidence source artifact.")

    expected_carried = {
        "minimal_adequacy_evidence_execution": 1,
        "minimal_adequacy_evidence_rows": 5,
        "minimal_adequacy_scope_rules": 3,
        "minimal_adequacy_acceptance_tests": 5,
        "minimal_adequacy_blocked_overreach": 5,
        "targeted_gap": 1,
        "targeted_gap_id": 1,
        "targeted_criterion": 1,
        "evidence_support": 1,
        "prior_resolved_gaps_retained": 4,
        "gap_resolution_decision": 0,
        "targeted_gap_resolution_decision": 0,
        "newly_resolved_gap": 0,
        "resolved_gap": 4,
        "unresolved_gap": 3,
        "remaining_blocking_gap": 3,
    ...
    return SimpleNamespace(
        title="Minimal Adequacy Evidence Boundary Audit Plus Gap Resolution Decision v8.97",
        output_path=OUTPUT_PATH,
        source_path=SOURCE_PATH,
        source_artifact_count=1 if source_exists else 0,
        missing_source_artifact_count=0 if source_exists else 1,
        minimal_adequacy_evidence_boundary_audit_count=1,
        audited_minimal_adequacy_evidence_count=1,
        minimal_adequacy_boundary_audit_row_count=5,
        ...
        previously_resolved_gaps_retained_count=4,
        ...
        carried_minimal_adequacy_condition_evidence_execution_count=carried["minimal_adequacy_evidence_execution"],
        carried_minimal_adequacy_evidence_row_count=carried["minimal_adequacy_evidence_rows"],
        carried_minimal_adequacy_scope_rule_count=carried["minimal_adequacy_scope_rules"],
        carried_minimal_adequacy_acceptance_test_count=carried["minimal_adequacy_acceptance_tests"],
        carried_minimal_adequacy_blocked_overreach_count=carried["minimal_adequacy_blocked_overreach"],
