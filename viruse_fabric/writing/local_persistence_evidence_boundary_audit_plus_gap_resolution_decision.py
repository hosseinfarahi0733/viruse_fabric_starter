...
SOURCE_PATH = Path("outputs/local_persistence_testability_evidence_execution_v8_90.md")
OUTPUT_PATH = Path("outputs/local_persistence_evidence_boundary_audit_plus_gap_resolution_decision_v8_91.md")
...
def _overclaim_count(text: str) -> int:
    prohibited_assertions = [
        ...
        "local persistence completes sigma_a",
...
def _carried_counts(source_text: str) -> dict[str, int]:
    return {
        "local_persistence_evidence_execution": 1 if _has_count(source_text, "Local persistence testability evidence execution", "1") else 0,
        "local_persistence_evidence_rows": 5 if _has_count(source_text, "Local persistence evidence row", "5") else 0,
        "local_persistence_scope_rules": 3 if _has_count(source_text, "Local persistence scope rule", "3") else 0,
        "local_persistence_acceptance_tests": 5 if _has_count(source_text, "Local persistence acceptance test", "5") else 0,
        "local_persistence_blocked_overreach": 5 if _has_count(source_text, "Local persistence blocked overreach", "5") else 0,
        ...
        "prior_resolved_gap_retained": 1 if _has_count(source_text, "Prior resolved gap retained", "1") else 0,
    ...
    return [
        {
            ...
            "audit_focus": "local evaluation window",
            "accepted_claim": "local persistence is testable only inside a bounded neighborhood of a finite quotient carrier element",
            "blocked_claim": "global behavior or undefined neighborhoods count as local persistence evidence",
            "audit_status": "passed for targeted gap resolution decision",
        },
        {
            "audit_id": "LPBA-002",
            "evidence_id": "LPT-EE-002",
            "audit_focus": "predicate input structure",
            "accepted_claim": "test input includes carrier term, local condition, and bounded perturbation relation",
            "blocked_claim": "narrative stability or observer preference count as test input",
            "audit_status": "passed for targeted gap resolution decision",
        },
        {
            "audit_id": "LPBA-003",
            "evidence_id": "LPT-EE-003",
            "audit_focus": "persistence witness",
            "accepted_claim": "a persistence witness requires invariant local condition across bounded comparison target",
            "blocked_claim": "a single untested state counts as persistence witness",
            "audit_status": "passed for targeted gap resolution decision",
        },
        {
            "audit_id": "LPBA-004",
            "evidence_id": "LPT-EE-004",
            "audit_focus": "failure witness",
            "accepted_claim": "failure witness discipline is required when the local condition changes under permitted perturbation",
            "blocked_claim": "positive-only evidence is sufficient despite hidden local counterexamples",
            "audit_status": "passed for targeted gap resolution decision",
        },
        {
            "audit_id": "LPBA-005",
            ...
            "accepted_claim": "testability evidence supports the second gap but does not finish Sigma_A",
            "blocked_claim": "testability evidence creates predicate completion, theorem proof, or readiness",
            "audit_status": "passed for targeted gap resolution decision",
        },
    ]
...
def render_report() -> str:
    ...
    lines = [
        "# Local Persistence Evidence Boundary Audit Plus Gap Resolution Decision v8.91",
        "",
        "## Purpose",
        "",
        "Audit the v8.90 local persistence testability evidence and record a targeted gap resolution decision for SPCHG-002 linked to SPCC-002, while keeping completion decision, completion execution, completion authorization, definition execution, new draft clause creation, new completion criterion creation, predicate definition completion, theorem proof, proof execution, proof assistant verification, completed formalization, framework-level proof, external validation, citation addition, and manuscript readiness counts at zero.",
        "",
        "## Source artifact",
        "",
        ...
        "",
        "## Local persistence evidence boundary audit rows",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            f"- Carried local persistence testability evidence execution count: {carried['local_persistence_evidence_execution']}",
            f"- Carried local persistence evidence row count: {carried['local_persistence_evidence_rows']}",
            f"- Carried local persistence scope rule count: {carried['local_persistence_scope_rules']}",
            f"- Carried local persistence acceptance test count: {carried['local_persistence_acceptance_tests']}",
            f"- Carried local persistence blocked overreach count: {carried['local_persistence_blocked_overreach']}",
            ...
            f"- Carried prior resolved gap retained count: {carried['prior_resolved_gap_retained']}",
            ...
            "## Warnings",
            "",
            "- This milestone audits local persistence evidence and records a targeted gap resolution decision only.",
            "- Resolving SPCHG-002 does not authorize completion execution or predicate completion.",
...
def run():
    ...
    if not source_exists:
        errors.append("Missing required v8.90 local persistence evidence source artifact.")

    expected_carried = {
        "local_persistence_evidence_execution": 1,
        "local_persistence_evidence_rows": 5,
        "local_persistence_scope_rules": 3,
        "local_persistence_acceptance_tests": 5,
        "local_persistence_blocked_overreach": 5,
        "targeted_gap": 1,
        "targeted_gap_id": 1,
        "targeted_criterion": 1,
        "evidence_support": 1,
        "prior_resolved_gap_retained": 1,
    ...
    return SimpleNamespace(
        title="Local Persistence Evidence Boundary Audit Plus Gap Resolution Decision v8.91",
        output_path=OUTPUT_PATH,
        source_path=SOURCE_PATH,
        source_artifact_count=1 if source_exists else 0,
        missing_source_artifact_count=0 if source_exists else 1,
        local_persistence_evidence_boundary_audit_count=1,
        audited_local_persistence_evidence_count=1,
        local_persistence_boundary_audit_row_count=5,
        ...
        previously_resolved_gap_retained_count=1,
        ...
        carried_local_persistence_testability_evidence_execution_count=carried["local_persistence_evidence_execution"],
        carried_local_persistence_evidence_row_count=carried["local_persistence_evidence_rows"],
        carried_local_persistence_scope_rule_count=carried["local_persistence_scope_rules"],
        carried_local_persistence_acceptance_test_count=carried["local_persistence_acceptance_tests"],
        carried_local_persistence_blocked_overreach_count=carried["local_persistence_blocked_overreach"],
        ...
        carried_prior_resolved_gap_retained_count=carried["prior_resolved_gap_retained"],
