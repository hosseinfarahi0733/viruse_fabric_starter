...
SOURCE_PATH = Path("outputs/exclusion_and_failure_evidence_boundary_audit_plus_gap_resolution_decision_v8_95.md")
OUTPUT_PATH = Path("outputs/minimal_adequacy_condition_evidence_execution_v8_96.md")
...
def _overclaim_count(text: str) -> int:
    prohibited_assertions = [
        ...
        "minimal adequacy evidence resolves the gap",
        "minimal adequacy condition proves completion",
    ]
    lowered = text.lower()
...
def _carried_counts(source_text: str) -> dict[str, int]:
    return {
        "exclusion_failure_boundary_audit": 1 if _has_count(source_text, "Exclusion and failure evidence boundary audit", "1") else 0,
        "audited_exclusion_failure_evidence": 1 if _has_count(source_text, "Audited exclusion and failure evidence", "1") else 0,
        "exclusion_failure_boundary_rows": 5 if _has_count(source_text, "Exclusion and failure boundary audit row", "5") else 0,
        ...
        "previously_resolved_gaps_retained": 3 if _has_count(source_text, "Previously resolved gaps retained", "3") else 0,
    ...
    return [
        {
            ...
            "component": "minimal input set",
            "statement": "A minimal adequacy condition must name the least carrier-local inputs required for a Sigma_A candidate check.",
            "acceptance_test": "Accept only rows that identify the minimal input set without importing external biological, clinical, or manuscript-readiness assumptions.",
            "blocked_overreach": "treating broad narrative completeness as minimal adequacy",
            "status": "executed evidence",
        },
        {
            "evidence_id": "MAC-EE-002",
            "component": "adequacy threshold",
            "statement": "A minimal adequacy condition must specify the threshold at which evidence becomes sufficient for a local candidate check.",
            "acceptance_test": "Accept only rows that separate threshold satisfaction from completion decision or theorem proof.",
            "blocked_overreach": "using adequacy threshold language as a substitute for predicate completion",
            "status": "executed evidence",
        },
        {
            "evidence_id": "MAC-EE-003",
            "component": "insufficiency witness",
            "statement": "A minimal adequacy condition must preserve an insufficiency witness when the local evidence falls below the threshold.",
            "acceptance_test": "Accept only structures that can record insufficient cases instead of silently converting them to positive evidence.",
            "blocked_overreach": "hiding insufficient cases while counting only adequate examples",
            "status": "executed evidence",
        },
        {
            "evidence_id": "MAC-EE-004",
            "component": "non-expansive guard",
            "statement": "The minimal adequacy condition must prevent the draft predicate from expanding beyond its finite carrier-local evidence base.",
            "acceptance_test": "Reject evidence that expands adequacy into global validity, proof completion, external validation, or readiness.",
            "blocked_overreach": "turning minimal adequacy into a global completion claim",
            "status": "executed evidence",
        },
        {
            "evidence_id": "MAC-EE-005",
            "component": "draft-predicate boundary",
            "statement": "This evidence describes minimal adequacy handling, but it does not finish Sigma_A or authorize completion execution.",
            ...
            "blocked_overreach": "treating minimal adequacy evidence as predicate completion, theorem proof, or readiness",
            "status": "executed evidence",
        },
    ]
...
def render_report() -> str:
    ...
    lines = [
        ...
        "",
        "## Minimal adequacy condition evidence rows",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            f"- Carried exclusion and failure evidence boundary audit count: {carried['exclusion_failure_boundary_audit']}",
            f"- Carried audited exclusion and failure evidence count: {carried['audited_exclusion_failure_evidence']}",
            f"- Carried exclusion and failure boundary audit row count: {carried['exclusion_failure_boundary_rows']}",
            ...
            "",
            "This artifact advances the fifth conditional-hold gap by creating minimal adequacy condition evidence only. SPCHG-005 should require a later boundary audit plus resolution decision before the unresolved-gap count can fall again.",
            "",
            "## Warnings",
            "",
            "- This milestone executes minimal adequacy condition evidence only.",
...
def run():
    ...
    if not source_exists:
        errors.append("Missing required v8.95 exclusion and failure resolution decision source artifact.")

    expected_carried = {
        "exclusion_failure_boundary_audit": 1,
        "audited_exclusion_failure_evidence": 1,
        "exclusion_failure_boundary_rows": 5,
        "accepted_claim": 5,
        "blocked_claim": 5,
        "gap_resolution_decision": 1,
        "targeted_gap_resolution_decision": 1,
        "evidence_supported_gap": 1,
        "previously_resolved_gaps_retained": 3,
        "resolved_gap": 4,
    ...
    return SimpleNamespace(
        title="Minimal Adequacy Condition Evidence Execution v8.96",
        output_path=OUTPUT_PATH,
        source_path=SOURCE_PATH,
        source_artifact_count=1 if source_exists else 0,
        missing_source_artifact_count=0 if source_exists else 1,
        minimal_adequacy_condition_evidence_execution_count=1,
        minimal_adequacy_evidence_row_count=5,
        minimal_adequacy_scope_rule_count=3,
        minimal_adequacy_acceptance_test_count=5,
        minimal_adequacy_blocked_overreach_count=5,
        targeted_gap_count=1,
        targeted_gap_id_count=1,
        targeted_criterion_count=1,
        evidence_support_count=1,
        prior_resolved_gaps_retained_count=4,
        ...
        carried_exclusion_and_failure_evidence_boundary_audit_count=carried["exclusion_failure_boundary_audit"],
        carried_audited_exclusion_and_failure_evidence_count=carried["audited_exclusion_failure_evidence"],
        carried_exclusion_and_failure_boundary_audit_row_count=carried["exclusion_failure_boundary_rows"],
