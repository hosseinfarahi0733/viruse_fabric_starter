...
SOURCE_PATH = Path("outputs/controlled_formal_definition_completion_approval_execution_preflight_execution_v8_41.md")
OUTPUT_PATH = Path("outputs/controlled_formal_definition_completion_approval_execution_preflight_decision_plan_v8_42.md")


@dataclass(frozen=True)
class PreflightDecisionPlanRow:
    row_id: str
    decision_focus: str
    source_signal: str
    planned_decision_check: str
    hard_boundary: str


@dataclass(frozen=True)
class PreflightDecisionPlanReport:
    title: str
    output_path: Path
    source_path: Path
    ...
    preflight_decision_plan_row_count: int
    preflight_execution_source_row_count: int
    preflight_execution_count: int
    preflight_plan_count: int
...
def _source_has_preflight_execution_count(text: str) -> bool:
    return (
        ...
        or _has_all_terms(
            text,
            [
                "controlled formal definition completion approval execution preflight execution",
                "count",
                "1",
            ],
        )
...
def _source_has_preflight_execution_rows(text: str) -> bool:
    return (
        "Controlled formal definition completion approval execution preflight execution row count: 4" in text
        or _has_all_terms(
            text,
            [
                "controlled formal definition completion approval execution preflight execution row count",
                "4",
            ],
        )
    )
...
def _overclaim_count(text: str) -> int:
    prohibited_assertions = [
        ...
        "preflight decision is executed",
...
def build_rows() -> List[PreflightDecisionPlanRow]:
    return [
        PreflightDecisionPlanRow(
            row_id="PDP-001",
            decision_focus="source preflight execution integrity",
            source_signal="v8.41 preflight execution exists and passed with hard-zero boundaries preserved",
            planned_decision_check="verify whether the preflight execution can support a later preflight decision execution milestone",
            hard_boundary="does not execute the preflight decision",
        ),
        PreflightDecisionPlanRow(
            row_id="PDP-002",
            decision_focus="preflight approval boundary",
            source_signal="v8.41 executes preflight checks but keeps preflight approval absent",
            planned_decision_check="define a future decision gate for whether preflight approval can be considered",
            hard_boundary="does not approve preflight",
        ),
        PreflightDecisionPlanRow(
            row_id="PDP-003",
            decision_focus="approval execution boundary",
            source_signal="v8.41 keeps approval execution absent",
            planned_decision_check="block any direct approval execution transition before a separate decision execution milestone",
            hard_boundary="does not execute approval",
        ),
        PreflightDecisionPlanRow(
            row_id="PDP-004",
            decision_focus="completion proof and readiness boundary",
            source_signal="v8.41 keeps approved completion, completed definitions, proof execution, validation, citations, and readiness absent",
            planned_decision_check="preserve all hard-zero claims while planning the decision structure",
...
def render_report() -> str:
    ...
    preflight_execution_source_row_count = 4 if _source_has_preflight_execution_rows(source_text) else 0
    ...
    if missing_source_artifact_count != 0:
        errors.append("Missing required v8.41 preflight execution source artifact.")
    if preflight_execution_source_row_count != 4:
        errors.append("Expected v8.41 preflight execution row count signal is absent.")
    if preflight_execution_count != 1:
        errors.append("Expected v8.41 preflight execution count signal is absent.")
    if preflight_plan_count != 1:
        errors.append("Expected carried v8.40 preflight plan count signal is absent.")
    ...
    warnings = [
        "Preflight decision planning is created, but preflight decision execution remains absent.",
    ...
    lines = [
        "# Controlled Formal Definition Completion Approval Execution Preflight Decision Plan v8.42",
        "",
        "## Purpose",
        "",
        "Plan a controlled decision about whether the v8.41 preflight execution is sufficient for a later approval execution gate while preserving all hard-zero boundaries.",
        "",
        "## Source artifact",
        "",
        ...
        "",
        "## Preflight decision plan rows",
        "",
        "| Row | Decision focus | Source signal | Planned decision check | Hard boundary |",
        "|---|---|---|---|---|",
    ]

    for row in rows:
        lines.append(
            f"| {row.row_id} | {row.decision_focus} | {row.source_signal} | {row.planned_decision_check} | {row.hard_boundary} |"
        )
    ...
    lines.extend(
        [
            ...
            f"- Controlled formal definition completion approval execution preflight decision plan row count: {len(rows)}",
            f"- Controlled formal definition completion approval execution preflight execution source row count: {preflight_execution_source_row_count}",
            ...
            "",
            "The v8.42 artifact creates a preflight decision plan only. It does not execute the preflight decision, does not approve preflight, does not execute approval, does not approve completion, does not complete formal definitions, does not execute proof work, does not add citations, does not provide external validation, and does not make the manuscript submission-ready.",
            "",
            "## Critical reviewer note",
            "",
            "This milestone is justified because v8.41 executed preflight checks but did not approve preflight or approval execution. The next defensible step is preflight decision execution, not direct approval execution.",
            "",
            "## Warnings",
            "",
        ]
    ...
    lines.extend(
        [
            ...
            "",
            "- Do not call this preflight decision execution.",
    ...
    if overclaim_count != 0:
        errors.append("Overclaim pattern detected in v8.42 preflight decision plan report.")
    if invented_citation_like_pattern_count != 0:
        errors.append("Invented citation-like pattern detected in v8.42 preflight decision plan report.")
...
def run() -> PreflightDecisionPlanReport:
    text = render_report()
    source_text = _read_source()
    rows = build_rows()
    ...
    return PreflightDecisionPlanReport(
        title="Controlled Formal Definition Completion Approval Execution Preflight Decision Plan v8.42",
        output_path=OUTPUT_PATH,
        source_path=SOURCE_PATH,
        source_artifact_count=1 if source_exists else 0,
        missing_source_artifact_count=missing_source_artifact_count,
        preflight_decision_plan_count=1,
        preflight_decision_plan_row_count=len(rows),
        preflight_execution_source_row_count=preflight_execution_source_row_count,
        preflight_execution_count=preflight_execution_count,
        preflight_plan_count=preflight_plan_count,
        approval_execution_preflight_required_count=preflight_required_count,
        approval_execution_preflight_decision_execution_count=0,
