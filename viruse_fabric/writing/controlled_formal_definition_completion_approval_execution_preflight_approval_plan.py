...
SOURCE_PATH = Path("outputs/controlled_formal_definition_completion_approval_execution_preflight_decision_execution_v8_43.md")
OUTPUT_PATH = Path("outputs/controlled_formal_definition_completion_approval_execution_preflight_approval_plan_v8_44.md")


@dataclass(frozen=True)
class PreflightApprovalPlanRow:
    row_id: str
    approval_focus: str
    source_signal: str
    planned_approval_check: str
    hard_boundary: str


@dataclass(frozen=True)
class PreflightApprovalPlanReport:
    title: str
    output_path: Path
    source_path: Path
    ...
    preflight_approval_plan_row_count: int
    preflight_decision_execution_source_row_count: int
...
def _source_has_preflight_decision_execution_count(text: str) -> bool:
    return (
        ...
        or _has_all_terms(
            text,
            [
                "controlled formal definition completion approval execution preflight decision execution",
                "count",
                "1",
            ],
        )
...
def _source_has_preflight_decision_execution_rows(text: str) -> bool:
    return (
        "Controlled formal definition completion approval execution preflight decision execution row count: 4" in text
        or _has_all_terms(
            text,
            [
...
def _source_has_preflight_decision_plan_count(text: str) -> bool:
    return (
        ...
        or _has_all_terms(text, ["controlled formal definition completion approval execution preflight decision plan", "count", "1"])
    )
...
def _source_has_preflight_decision_execution_count_short(text: str) -> bool:
    return (
...
def _overclaim_count(text: str) -> int:
    prohibited_assertions = [
        ...
        "preflight approval is completed",
...
def build_rows() -> List[PreflightApprovalPlanRow]:
    return [
        PreflightApprovalPlanRow(
            row_id="PAP-001",
            approval_focus="source preflight decision execution integrity",
            source_signal="v8.43 preflight decision execution exists and preserves hard-zero boundaries",
            planned_approval_check="verify that the preflight decision execution can serve as the source for a preflight approval planning milestone",
            hard_boundary="does not execute preflight approval",
        ),
        PreflightApprovalPlanRow(
            row_id="PAP-002",
            approval_focus="preflight approval eligibility",
            source_signal="v8.43 requires a separate preflight approval plan and keeps preflight approval absent",
            planned_approval_check="define the evidence and boundary checks required before any future preflight approval execution",
            hard_boundary="does not approve preflight",
        ),
        PreflightApprovalPlanRow(
            row_id="PAP-003",
            approval_focus="approval execution containment",
            source_signal="v8.43 keeps approval execution absent",
            planned_approval_check="block any movement from preflight approval planning directly into approval execution",
            hard_boundary="does not execute approval",
        ),
        PreflightApprovalPlanRow(
            row_id="PAP-004",
            approval_focus="completion proof validation and readiness containment",
            source_signal="v8.43 keeps completion, proof, validation, citations, and readiness absent",
            planned_approval_check="preserve all hard-zero claim boundaries during preflight approval planning",
...
def render_report() -> str:
    ...
    preflight_decision_execution_source_row_count = 4 if _source_has_preflight_decision_execution_rows(source_text) else 0
    ...
    preflight_decision_execution_count_short = 1 if _source_has_preflight_decision_execution_count_short(source_text) else 0
    preflight_approval_plan_required_count = 1 if _source_has_preflight_approval_plan_required(source_text) else 0
    source_hard_zero_signal_count = _source_hard_zero_signal_count(source_text)

    errors: List[str] = []
    if missing_source_artifact_count != 0:
        errors.append("Missing required v8.43 preflight decision execution source artifact.")
    if preflight_decision_execution_source_row_count != 4:
        errors.append("Expected v8.43 preflight decision execution row count signal is absent.")
    if preflight_decision_execution_count != 1:
        errors.append("Expected v8.43 preflight decision execution count signal is absent.")
    ...
    if preflight_decision_execution_count_short != 1:
        errors.append("Expected v8.43 approval execution preflight decision execution signal is absent.")
    if preflight_approval_plan_required_count != 1:
        errors.append("Expected v8.43 preflight approval plan required signal is absent.")
    if source_hard_zero_signal_count < 13:
        errors.append("Expected hard-zero source signals are incomplete.")

    warnings = [
        "Preflight approval planning is created, but preflight approval execution remains absent.",
    ...
    lines = [
        "# Controlled Formal Definition Completion Approval Execution Preflight Approval Plan v8.44",
        "",
        "## Purpose",
        "",
        "Plan a controlled preflight approval stage after v8.43 preflight decision execution while preserving all hard-zero boundaries and refusing approval execution.",
        "",
        "## Source artifact",
        "",
        ...
        "",
        "## Preflight approval plan rows",
        "",
        "| Row | Approval focus | Source signal | Planned approval check | Hard boundary |",
        "|---|---|---|---|---|",
    ]

    for row in rows:
        lines.append(
            f"| {row.row_id} | {row.approval_focus} | {row.source_signal} | {row.planned_approval_check} | {row.hard_boundary} |"
        )
    ...
    lines.extend(
        [
            ...
            f"- Controlled formal definition completion approval execution preflight approval plan row count: {len(rows)}",
            f"- Controlled formal definition completion approval execution preflight decision execution source row count: {preflight_decision_execution_source_row_count}",
            ...
            f"- Approval execution preflight decision execution count: {preflight_decision_execution_count_short}",
            f"- Approval execution preflight approval plan required count: {preflight_approval_plan_required_count}",
            ...
            "",
            "The v8.44 artifact creates a preflight approval plan only. It does not execute preflight approval, does not approve preflight, does not execute approval, does not approve completion, does not complete formal definitions, does not execute proof work, does not add citations, does not provide external validation, and does not make the manuscript submission-ready.",
            "",
            "## Critical reviewer note",
            "",
            "This milestone is justified because v8.43 required a separate preflight approval plan before any approval execution path can advance. It still does not grant preflight approval and does not authorize approval execution.",
            "",
            "## Warnings",
            "",
        ]
    ...
    lines.extend(
        [
            ...
            "",
            "- Do not call this preflight approval execution.",
    ...
    if overclaim_count != 0:
        errors.append("Overclaim pattern detected in v8.44 preflight approval plan report.")
    if invented_citation_like_pattern_count != 0:
        errors.append("Invented citation-like pattern detected in v8.44 preflight approval plan report.")
...
def run() -> PreflightApprovalPlanReport:
    text = render_report()
    source_text = _read_source()
    rows = build_rows()
    ...
    return PreflightApprovalPlanReport(
        title="Controlled Formal Definition Completion Approval Execution Preflight Approval Plan v8.44",
        output_path=OUTPUT_PATH,
        source_path=SOURCE_PATH,
        source_artifact_count=1 if source_exists else 0,
        missing_source_artifact_count=missing_source_artifact_count,
        preflight_approval_plan_count=1,
        preflight_approval_plan_row_count=len(rows),
        preflight_decision_execution_source_row_count=preflight_decision_execution_source_row_count,
        ...
        approval_execution_preflight_decision_execution_count=preflight_decision_execution_count_short,
        approval_execution_preflight_approval_plan_required_count=preflight_approval_plan_required_count,
        approval_execution_preflight_approval_execution_count=0,
