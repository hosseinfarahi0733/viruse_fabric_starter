...
SOURCE_PATH = Path("outputs/recurrence_witness_evidence_boundary_audit_plus_gap_resolution_decision_v8_93.md")
OUTPUT_PATH = Path("outputs/exclusion_and_failure_condition_evidence_execution_v8_94.md")
...
def _overclaim_count(text: str) -> int:
    prohibited_assertions = [
        ...
        "exclusion and failure condition completes sigma_a",
        "exclusion and failure evidence proves sigma_a",
        "exclusion evidence resolves the gap",
        "failure condition evidence proves completion",
    ]
    lowered = text.lower()
...
def _carried_counts(source_text: str) -> dict[str, int]:
    return {
        "recurrence_witness_boundary_audit": 1 if _has_count(source_text, "Recurrence witness evidence boundary audit", "1") else 0,
        "audited_recurrence_witness_evidence": 1 if _has_count(source_text, "Audited recurrence witness evidence", "1") else 0,
        "recurrence_witness_boundary_rows": 5 if _has_count(source_text, "Recurrence witness boundary audit row", "5") else 0,
        "accepted_claim": 5 if _has_count(source_text, "Accepted claim", "5") else 0,
        "blocked_claim": 5 if _has_count(source_text, "Blocked claim", "5") else 0,
        ...
        "previously_resolved_gaps_retained": 2 if _has_count(source_text, "Previously resolved gaps retained", "2") else 0,
        "resolved_gap": 3 if _has_count(source_text, "Resolved gap", "3") else 0,
        "newly_resolved_gap": 1 if _has_count(source_text, "Newly resolved gap", "1") else 0,
        "unresolved_gap": 4 if _has_count(source_text, "Unresolved gap", "4") else 0,
        "remaining_blocking_gap": 4 if _has_count(source_text, "Remaining blocking gap", "4") else 0,
...
def _evidence_rows() -> list[dict[str, str]]:
    return [
        {
            "evidence_id": "EFC-EE-001",
            "component": "exclusion boundary",
            "statement": "An exclusion condition must identify when a candidate local state is outside the acceptable Sigma_A evaluation boundary.",
            "acceptance_test": "Accept only rows that name the excluded condition and the boundary rule that excludes it.",
            "blocked_overreach": "treating every nearby state as acceptable merely because it shares vocabulary with the carrier condition",
            "status": "executed evidence",
        },
        {
            "evidence_id": "EFC-EE-002",
            "component": "failure trigger",
            "statement": "A failure condition must specify the local event that invalidates persistence or recurrence support.",
            "acceptance_test": "Accept only rows that specify a failure trigger and its effect on the candidate condition.",
            "blocked_overreach": "allowing evidence to remain positive after an explicit local failure trigger appears",
            "status": "executed evidence",
        },
        {
            "evidence_id": "EFC-EE-003",
            "component": "negative witness",
            "statement": "A negative witness must be recordable whenever the exclusion boundary or failure trigger is met.",
            "acceptance_test": "Accept only evidence structures that preserve a negative witness row rather than silently discarding it.",
            "blocked_overreach": "hiding negative cases while counting only successful local behavior",
            "status": "executed evidence",
        },
        {
            "evidence_id": "EFC-EE-004",
            "component": "non-vacuity guard",
            "statement": "The exclusion and failure condition must prevent the draft predicate from becoming automatically true for all carrier elements.",
            "acceptance_test": "Reject evidence that has no way to mark a candidate as excluded or failed.",
            "blocked_overreach": "making Sigma_A vacuously positive by omitting exclusion and failure handling",
            "status": "executed evidence",
        },
        {
            "evidence_id": "EFC-EE-005",
            "component": "draft-predicate boundary",
            "statement": "This evidence describes exclusion and failure handling, but it does not finish Sigma_A or authorize completion execution.",
            ...
            "blocked_overreach": "treating exclusion evidence as predicate completion, theorem proof, or readiness",
            "status": "executed evidence",
        },
    ]
...
def render_report() -> str:
    ...
    lines = [
        ...
        "",
        "## Exclusion and failure condition evidence rows",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            f"- Carried recurrence witness evidence boundary audit count: {carried['recurrence_witness_boundary_audit']}",
            f"- Carried audited recurrence witness evidence count: {carried['audited_recurrence_witness_evidence']}",
            f"- Carried recurrence witness boundary audit row count: {carried['recurrence_witness_boundary_rows']}",
            f"- Carried accepted claim count: {carried['accepted_claim']}",
            ...
            f"- Carried previously resolved gaps retained count: {carried['previously_resolved_gaps_retained']}",
            ...
            "",
            "This artifact advances the fourth conditional-hold gap by creating exclusion and failure condition evidence only. SPCHG-004 should require a later boundary audit plus resolution decision before the unresolved-gap count can fall again.",
            "",
            "## Warnings",
            "",
            "- This milestone executes exclusion and failure condition evidence only.",
...
def run():
    ...
    if not source_exists:
        errors.append("Missing required v8.93 recurrence witness resolution decision source artifact.")

    expected_carried = {
        "recurrence_witness_boundary_audit": 1,
        "audited_recurrence_witness_evidence": 1,
        "recurrence_witness_boundary_rows": 5,
        "accepted_claim": 5,
        "blocked_claim": 5,
        "gap_resolution_decision": 1,
        "targeted_gap_resolution_decision": 1,
        "evidence_supported_gap": 1,
        "previously_resolved_gaps_retained": 2,
        "resolved_gap": 3,
        "newly_resolved_gap": 1,
        "unresolved_gap": 4,
        "remaining_blocking_gap": 4,
    ...
    return SimpleNamespace(
        title="Exclusion and Failure Condition Evidence Execution v8.94",
        output_path=OUTPUT_PATH,
        source_path=SOURCE_PATH,
        source_artifact_count=1 if source_exists else 0,
        missing_source_artifact_count=0 if source_exists else 1,
        exclusion_and_failure_condition_evidence_execution_count=1,
        exclusion_and_failure_evidence_row_count=5,
        exclusion_and_failure_scope_rule_count=3,
        exclusion_and_failure_acceptance_test_count=5,
        exclusion_and_failure_blocked_overreach_count=5,
        targeted_gap_count=1,
        targeted_gap_id_count=1,
        targeted_criterion_count=1,
        evidence_support_count=1,
        prior_resolved_gaps_retained_count=3,
        ...
        carried_recurrence_witness_evidence_boundary_audit_count=carried["recurrence_witness_boundary_audit"],
        carried_audited_recurrence_witness_evidence_count=carried["audited_recurrence_witness_evidence"],
        carried_recurrence_witness_boundary_audit_row_count=carried["recurrence_witness_boundary_rows"],
        carried_accepted_claim_count=carried["accepted_claim"],
        ...
        carried_previously_resolved_gaps_retained_count=carried["previously_resolved_gaps_retained"],
