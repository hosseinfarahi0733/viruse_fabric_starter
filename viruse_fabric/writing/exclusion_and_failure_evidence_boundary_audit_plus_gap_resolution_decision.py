...
SOURCE_PATH = Path("outputs/exclusion_and_failure_condition_evidence_execution_v8_94.md")
OUTPUT_PATH = Path("outputs/exclusion_and_failure_evidence_boundary_audit_plus_gap_resolution_decision_v8_95.md")
...
def _overclaim_count(text: str) -> int:
    prohibited_assertions = [
        ...
        "exclusion and failure completes sigma_a",
        "exclusion and failure proves sigma_a",
        "exclusion evidence proves completion",
        "failure evidence proves completion",
    ]
    lowered = text.lower()
...
def _carried_counts(source_text: str) -> dict[str, int]:
    return {
        "exclusion_failure_evidence_execution": 1 if _has_count(source_text, "Exclusion and failure condition evidence execution", "1") else 0,
        "exclusion_failure_evidence_rows": 5 if _has_count(source_text, "Exclusion and failure evidence row", "5") else 0,
        "exclusion_failure_scope_rules": 3 if _has_count(source_text, "Exclusion and failure scope rule", "3") else 0,
        "exclusion_failure_acceptance_tests": 5 if _has_count(source_text, "Exclusion and failure acceptance test", "5") else 0,
        "exclusion_failure_blocked_overreach": 5 if _has_count(source_text, "Exclusion and failure blocked overreach", "5") else 0,
        ...
        "prior_resolved_gaps_retained": 3 if _has_count(source_text, "Prior resolved gaps retained", "3") else 0,
    ...
    return [
        {
            ...
            "audit_focus": "exclusion boundary",
            "accepted_claim": "excluded local states must be identified by a named boundary rule",
            "blocked_claim": "nearby states automatically count as acceptable because they share vocabulary with the carrier condition",
            "audit_status": "passed for targeted gap resolution decision",
        },
        {
            "audit_id": "EFCBA-002",
            "evidence_id": "EFC-EE-002",
            "audit_focus": "failure trigger",
            "accepted_claim": "a failure trigger must invalidate persistence or recurrence support for the candidate condition",
            "blocked_claim": "positive evidence remains valid after an explicit local failure trigger appears",
            "audit_status": "passed for targeted gap resolution decision",
        },
        {
            "audit_id": "EFCBA-003",
            "evidence_id": "EFC-EE-003",
            "audit_focus": "negative witness",
            "accepted_claim": "negative witnesses must be recordable instead of silently discarded",
            "blocked_claim": "successful local behavior can hide negative cases",
            "audit_status": "passed for targeted gap resolution decision",
        },
        {
            "audit_id": "EFCBA-004",
            "evidence_id": "EFC-EE-004",
            "audit_focus": "non-vacuity guard",
            "accepted_claim": "exclusion and failure handling prevents the draft predicate from becoming automatically true",
            "blocked_claim": "Sigma_A can become vacuously positive by omitting exclusion and failure handling",
            "audit_status": "passed for targeted gap resolution decision",
        },
        {
            "audit_id": "EFCBA-005",
            ...
            "accepted_claim": "exclusion and failure evidence supports the fourth gap but does not finish Sigma_A",
            "blocked_claim": "exclusion and failure evidence creates predicate completion, theorem proof, or readiness",
            "audit_status": "passed for targeted gap resolution decision",
        },
    ]
...
def render_report() -> str:
    ...
    lines = [
        "# Exclusion and Failure Evidence Boundary Audit Plus Gap Resolution Decision v8.95",
        "",
        "## Purpose",
        "",
        "Audit the v8.94 exclusion and failure condition evidence and record a targeted gap resolution decision for SPCHG-004 linked to SPCC-004, while keeping completion decision, completion execution, completion authorization, definition execution, new draft clause creation, new completion criterion creation, predicate definition completion, theorem proof, proof execution, proof assistant verification, completed formalization, framework-level proof, external validation, citation addition, and manuscript readiness counts at zero.",
        "",
        "## Source artifact",
        "",
        ...
        "",
        "## Exclusion and failure evidence boundary audit rows",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            f"- Carried exclusion and failure condition evidence execution count: {carried['exclusion_failure_evidence_execution']}",
            f"- Carried exclusion and failure evidence row count: {carried['exclusion_failure_evidence_rows']}",
            f"- Carried exclusion and failure scope rule count: {carried['exclusion_failure_scope_rules']}",
            f"- Carried exclusion and failure acceptance test count: {carried['exclusion_failure_acceptance_tests']}",
            f"- Carried exclusion and failure blocked overreach count: {carried['exclusion_failure_blocked_overreach']}",
            ...
            "## Warnings",
            "",
            "- This milestone audits exclusion and failure evidence and records a targeted gap resolution decision only.",
            "- Resolving SPCHG-004 does not authorize completion execution or predicate completion.",
            "- Three conditional-hold gaps remain unresolved and blocking.",
...
def run():
    ...
    if not source_exists:
        errors.append("Missing required v8.94 exclusion and failure evidence source artifact.")

    expected_carried = {
        "exclusion_failure_evidence_execution": 1,
        "exclusion_failure_evidence_rows": 5,
        "exclusion_failure_scope_rules": 3,
        "exclusion_failure_acceptance_tests": 5,
        "exclusion_failure_blocked_overreach": 5,
        "targeted_gap": 1,
        "targeted_gap_id": 1,
        "targeted_criterion": 1,
        "evidence_support": 1,
        "prior_resolved_gaps_retained": 3,
    ...
    return SimpleNamespace(
        title="Exclusion and Failure Evidence Boundary Audit Plus Gap Resolution Decision v8.95",
        output_path=OUTPUT_PATH,
        source_path=SOURCE_PATH,
        source_artifact_count=1 if source_exists else 0,
        missing_source_artifact_count=0 if source_exists else 1,
        exclusion_and_failure_evidence_boundary_audit_count=1,
        audited_exclusion_and_failure_evidence_count=1,
        exclusion_and_failure_boundary_audit_row_count=5,
        ...
        previously_resolved_gaps_retained_count=3,
        resolved_gap_count=4,
        newly_resolved_gap_count=1,
        unresolved_gap_count=3,
        remaining_blocking_gap_count=3,
        ...
        carried_exclusion_and_failure_condition_evidence_execution_count=carried["exclusion_failure_evidence_execution"],
        carried_exclusion_and_failure_evidence_row_count=carried["exclusion_failure_evidence_rows"],
        carried_exclusion_and_failure_scope_rule_count=carried["exclusion_failure_scope_rules"],
        carried_exclusion_and_failure_acceptance_test_count=carried["exclusion_failure_acceptance_tests"],
        carried_exclusion_and_failure_blocked_overreach_count=carried["exclusion_failure_blocked_overreach"],
