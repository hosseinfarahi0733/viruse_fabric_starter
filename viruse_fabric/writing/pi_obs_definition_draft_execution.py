...
class PiObsDefinitionDraftExecutionReport:
...
class PiObsDefinitionDraftExecutionBuilder:
    """Build v8.139 Pi_obs definition draft execution artifact.

    Boundary discipline:
    - This milestone executes one Pi_obs definition draft only.
    - It does not execute final definition completion.
    - It does not complete Pi_obs.
    - It does not complete C_reg, Adm_A, or Sigma_A.
    - It does not prove theorems.
    """

    title = "Pi_obs Definition Draft Execution v8.139"
    source_artifact = Path("outputs/c_reg_definition_draft_execution_v8_138.md")
    output_path = Path("outputs/pi_obs_definition_draft_execution_v8_139.md")

    draft_statement = (
        "Draft Pi_obs := an observer-projection placeholder for the refined Draft Sigma_A shell, "
        "tracking how later observable projections may be attached to admissible configurations inside the carried C_reg draft. "
        "It is linked to the carried Adm_A and C_reg drafts as a dependent draft interface, but it remains a draft slot only. "
        "This draft does not complete Pi_obs, does not complete C_reg, does not complete Adm_A, does not complete Sigma_A, "
        "and does not execute theorem or proof work."
    )
    ...
    draft_rows = [
        {
            ...
            "result": "created exactly one Pi_obs definition draft slot",
            "boundary": "does not complete Pi_obs",
        },
        {
            "id": "PI-OBS-DRAFT-002",
            "focus": "Sigma_A linkage",
            "result": "linked the Pi_obs draft to the refined Draft Sigma_A shell",
            "boundary": "does not complete Sigma_A",
        },
        {
            "id": "PI-OBS-DRAFT-003",
            "focus": "Adm_A dependency",
            "result": "linked Pi_obs to the carried Adm_A admissibility-interface draft",
            "boundary": "does not complete Adm_A",
        },
        {
            "id": "PI-OBS-DRAFT-004",
            "focus": "C_reg dependency",
            "result": "linked Pi_obs to the carried C_reg constraint-region draft",
            "boundary": "does not complete C_reg",
        },
        {
            "id": "PI-OBS-DRAFT-005",
            "focus": "observer-projection role",
            "result": "recorded Pi_obs as an observer-projection placeholder",
            "boundary": "does not execute a final formal definition",
        },
        {
            "id": "PI-OBS-DRAFT-006",
            "focus": "remaining dependent objects",
            "result": "kept M_c, R_A, and Traj_A deferred",
    ...
    checks = [
        ...
        "Pi_obs is linked to the refined Draft Sigma_A shell.",
        "Pi_obs is linked to the carried Adm_A and C_reg drafts.",
    ...
    def run(self) -> PiObsDefinitionDraftExecutionReport:
        errors: list[str] = []
        warnings: list[str] = []
        ...
        warnings.extend([
            "This milestone executes one Pi_obs definition draft, not a completed formal definition.",
            "Pi_obs definition completion remains zero.",
            "C_reg, Adm_A, and Sigma_A definition completion remain zero.",
        ...
        counts: dict[str, int] = {
            ...
            "pi_obs_definition_draft_execution_count": 1,
            "new_pi_obs_definition_draft_execution_count": 1,
            "new_definition_draft_execution_count": 1,
            "dependent_object_definition_draft_execution_count": 1,
            "pi_obs_draft_slot_created_count": 1,
            "pi_obs_draft_linked_to_refined_sigma_a_count": 1,
            "pi_obs_linked_to_adm_a_draft_count": 1,
            "pi_obs_linked_to_c_reg_draft_count": 1,
            "pi_obs_observer_projection_placeholder_count": 1,
            "remaining_dependent_object_deferral_count": 3,
            ...
            "carried_c_reg_definition_draft_execution_count": carried.get("C_reg definition draft execution count", 1),
            "carried_new_c_reg_definition_draft_execution_count": carried.get("New C_reg definition draft execution count", 1),
            "carried_c_reg_draft_slot_created_count": carried.get("C_reg draft slot created count", 1),
            "carried_c_reg_draft_linked_to_refined_sigma_a_count": carried.get("C_reg draft linked to refined Sigma_A count", 1),
            "carried_c_reg_linked_to_adm_a_draft_count": carried.get("C_reg linked to Adm_A draft count", 1),
            "carried_c_reg_constraint_region_placeholder_count": carried.get("C_reg constraint-region placeholder count", 1),
            "carried_remaining_dependent_object_deferral_count": carried.get("Remaining dependent-object deferral count", 4),

            "carried_adm_a_definition_draft_execution_count": carried.get("Carried Adm_A definition draft execution count", 1),
            "carried_adm_a_draft_linked_to_refined_sigma_a_count": carried.get("Carried Adm_A draft linked to refined Sigma_A count", 1),
        ...
        if counts["overclaim_count"] != 0:
            errors.append("Overclaim detected in v8.139 report.")
        if counts["invented_citation_like_pattern_count"] != 0:
            errors.append("Invented citation-like pattern detected in v8.139 report.")
        if counts["pi_obs_definition_draft_execution_count"] != 1:
            errors.append("v8.139 must execute exactly one Pi_obs definition draft.")
        if counts["new_definition_draft_execution_count"] != 1:
            errors.append("v8.139 must execute exactly one new definition draft.")
        if counts["pi_obs_definition_completion_count"] != 0:
            errors.append("v8.139 must not complete Pi_obs.")
        if counts["definition_execution_count"] != 0:
            errors.append("v8.139 must not execute final definitions.")
        if counts["sigma_a_definition_completion_count"] != 0:
            errors.append("v8.139 must not complete Sigma_A definition.")
        if counts["completed_formal_definition_count"] != 0:
            errors.append("v8.139 must not complete a formal definition.")
        if counts["theorem_candidate_plan_count"] != 0:
            errors.append("v8.139 must not create theorem candidates.")
        if counts["new_theorem_proven_count"] != 0:
            errors.append("v8.139 must not prove theorem claims.")
        ...
        return PiObsDefinitionDraftExecutionReport(
            title=self.title,
    ...
    def _label(self, key: str) -> str:
        overrides = {
            ...
            "carried_new_c_reg_definition_draft_execution_count": "Carried new C_reg definition draft execution count",
            "carried_c_reg_draft_slot_created_count": "Carried C_reg draft slot created count",
            ...
            "carried_c_reg_constraint_region_placeholder_count": "Carried C_reg constraint-region placeholder count",
    ...
    def _render_report(self, counts: dict[str, int], warnings: list[str]) -> str:
        ...
        lines.append(
            "Can Viruse Fabric execute one Pi_obs definition draft after the C_reg definition draft while keeping final definition execution, "
        ...
        lines.append("- Milestone type: Pi_obs definition draft execution only")
        lines.append("- Pi_obs definition draft execution after this milestone: executed")
        ...
        lines.append("")
        lines.append("## Pi_obs draft statement")
        lines.append(self.draft_statement)
        lines.append("")
        lines.append("## Pi_obs definition draft execution rows")
        lines.append("")
        ...
        lines.append(
            "This artifact executes one Pi_obs definition draft only. "
            "It creates a Pi_obs draft slot linked to the refined Draft Sigma_A shell and the carried Adm_A and C_reg drafts, "
            "but it does not execute final definitions, does not complete Pi_obs, does not complete C_reg, does not complete Adm_A, does not complete Sigma_A, "
        ...
        order = [
            ...
            "carried_new_c_reg_definition_draft_execution_count",
            "carried_c_reg_draft_slot_created_count",
            "carried_c_reg_draft_linked_to_refined_sigma_a_count",
            "carried_c_reg_linked_to_adm_a_draft_count",
            "carried_c_reg_constraint_region_placeholder_count",
        ...
        lines.append(
            "The v8.139 artifact executes one Pi_obs definition draft. "
            "It moves Pi_obs from deferred slot to draft status while keeping final definition execution, Pi_obs completion, C_reg completion, Adm_A completion, "
        ...
        next_steps = [
            "Execute M_c definition draft next rather than auditing this draft by default.",
            "Keep Pi_obs completion separate from Pi_obs draft execution.",
            "Keep C_reg, Adm_A, and Sigma_A definition completion separate from dependent-object drafts.",
    ...
    def _count_boundary_phrases(self, text: str) -> int:
        phrases = ["does not", "not executed", "not completed", "not created", "Pi_obs", "definition draft", "boundary", "separate", "zero"]
    ...
    def _count_overclaims(self, text: str) -> int:
        ...
        allowed_positive_counter_names = {
            "pi_obs definition draft execution count",
            "pi obs definition draft execution count",
            "new pi_obs definition draft execution count",
            "new pi obs definition draft execution count",
            "new definition draft execution count",
            "dependent-object definition draft execution count",
            "dependent object definition draft execution count",
            "pi_obs draft slot created count",
            "pi obs draft slot created count",
            "pi_obs draft linked to refined sigma_a count",
            "pi obs draft linked to refined sigma a count",
            "pi_obs linked to adm_a draft count",
            "pi obs linked to adm a draft count",
            "pi_obs linked to c_reg draft count",
            "pi obs linked to c reg draft count",
            "pi_obs observer-projection placeholder count",
            "pi obs observer-projection placeholder count",
            ...
            "carried new c_reg definition draft execution count",
            "carried new c reg definition draft execution count",
            "carried c_reg draft slot created count",
            "carried c reg draft slot created count",
            ...
            "carried c_reg constraint-region placeholder count",
            "carried c reg constraint-region placeholder count",
