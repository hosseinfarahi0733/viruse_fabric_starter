...
SOURCE_PATH = Path("outputs/local_persistence_evidence_boundary_audit_plus_gap_resolution_decision_v8_91.md")
OUTPUT_PATH = Path("outputs/recurrence_witness_evidence_execution_v8_92.md")
...
def _overclaim_count(text: str) -> int:
    prohibited_assertions = [
        ...
        "recurrence witness evidence resolves the gap",
        "recurrence witness evidence proves completion",
    ]
    lowered = text.lower()
...
def _carried_counts(source_text: str) -> dict[str, int]:
    return {
        "local_persistence_boundary_audit": 1 if _has_count(source_text, "Local persistence evidence boundary audit", "1") else 0,
        "audited_local_persistence_evidence": 1 if _has_count(source_text, "Audited local persistence evidence", "1") else 0,
        "local_persistence_boundary_rows": 5 if _has_count(source_text, "Local persistence boundary audit row", "5") else 0,
        ...
        "previously_resolved_gap_retained": 1 if _has_count(source_text, "Previously resolved gap retained", "1") else 0,
    ...
    return [
        {
            ...
            "component": "recurrence domain anchor",
            "statement": "A recurrence witness must be anchored to a finite R-path quotient carrier element and a declared local condition.",
            "acceptance_test": "Accept only witness rows that name the carrier element and the local condition being revisited.",
            "blocked_overreach": "using recurrence language without a finite carrier anchor or declared local condition",
            "status": "executed evidence",
        },
        {
            "evidence_id": "RW-EE-002",
            "component": "return relation",
            "statement": "A recurrence witness must specify a bounded return relation from a later local state back to a comparable local condition.",
            "acceptance_test": "Accept only witness rows with explicit return relation and bounded comparison target.",
            "blocked_overreach": "treating vague similarity, narrative return, or observer impression as recurrence",
            "status": "executed evidence",
        },
        {
            "evidence_id": "RW-EE-003",
            "component": "witness repeatability",
            "statement": "A recurrence witness must separate one observed return from repeatable witness structure.",
            "acceptance_test": "Accept evidence only when the witness structure can be checked again under the same finite carrier and bounded relation.",
            "blocked_overreach": "counting a single accidental return as stable recurrence structure",
            "status": "executed evidence",
        },
        {
            "evidence_id": "RW-EE-004",
            "component": "non-recurrence failure case",
            "statement": "A non-recurrence case must be recorded when the bounded return relation fails to recover the declared local condition.",
            "acceptance_test": "Reject evidence that lacks failure-case discipline for recurrence witness testing.",
            "blocked_overreach": "hiding failed returns while counting only successful recurrence-like examples",
            "status": "executed evidence",
        },
        {
            "evidence_id": "RW-EE-005",
            "component": "draft-predicate boundary",
            "statement": "This evidence describes recurrence witness testability, but it does not finish Sigma_A or authorize completion execution.",
            ...
            "blocked_overreach": "treating recurrence witness evidence as predicate completion, theorem proof, or readiness",
            "status": "executed evidence",
        },
    ]
...
def render_report() -> str:
    ...
    lines = [
        ...
        "",
        "## Recurrence witness evidence rows",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            f"- Carried local persistence evidence boundary audit count: {carried['local_persistence_boundary_audit']}",
            f"- Carried audited local persistence evidence count: {carried['audited_local_persistence_evidence']}",
            f"- Carried local persistence boundary audit row count: {carried['local_persistence_boundary_rows']}",
            ...
            f"- Carried previously resolved gap retained count: {carried['previously_resolved_gap_retained']}",
            ...
            "",
            "This artifact advances the third conditional-hold gap by creating recurrence witness evidence only. SPCHG-003 should require a later boundary audit plus resolution decision before the unresolved-gap count can fall again.",
            "",
            "## Warnings",
            "",
            "- This milestone executes recurrence witness evidence only.",
            "- The targeted gap receives evidence support, but no gap resolution decision is recorded yet.",
            "- Five conditional-hold gaps remain unresolved and blocking.",
...
def run():
    ...
    if not source_exists:
        errors.append("Missing required v8.91 local persistence resolution decision source artifact.")

    expected_carried = {
        "local_persistence_boundary_audit": 1,
        "audited_local_persistence_evidence": 1,
        "local_persistence_boundary_rows": 5,
        "accepted_claim": 5,
        "blocked_claim": 5,
        "gap_resolution_decision": 1,
        "targeted_gap_resolution_decision": 1,
        "evidence_supported_gap": 1,
        "previously_resolved_gap_retained": 1,
        "resolved_gap": 2,
    ...
    return SimpleNamespace(
        title="Recurrence Witness Evidence Execution v8.92",
        output_path=OUTPUT_PATH,
        source_path=SOURCE_PATH,
        source_artifact_count=1 if source_exists else 0,
        missing_source_artifact_count=0 if source_exists else 1,
        recurrence_witness_evidence_execution_count=1,
        recurrence_witness_evidence_row_count=5,
        recurrence_witness_scope_rule_count=3,
        recurrence_witness_acceptance_test_count=5,
        recurrence_witness_blocked_overreach_count=5,
        targeted_gap_count=1,
        targeted_gap_id_count=1,
        targeted_criterion_count=1,
        evidence_support_count=1,
        prior_resolved_gaps_retained_count=2,
        gap_resolution_decision_count=0,
        targeted_gap_resolution_decision_count=0,
        newly_resolved_gap_count=0,
        resolved_gap_count=2,
        unresolved_gap_count=5,
        remaining_blocking_gap_count=5,
        ...
        carried_local_persistence_evidence_boundary_audit_count=carried["local_persistence_boundary_audit"],
        carried_audited_local_persistence_evidence_count=carried["audited_local_persistence_evidence"],
        carried_local_persistence_boundary_audit_row_count=carried["local_persistence_boundary_rows"],
        ...
        carried_previously_resolved_gap_retained_count=carried["previously_resolved_gap_retained"],
