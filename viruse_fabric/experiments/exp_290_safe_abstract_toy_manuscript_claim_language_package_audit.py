...
from viruse_fabric.writing.safe_abstract_toy_manuscript_claim_language_package_audit import (
    SafeAbstractToyManuscriptClaimLanguagePackageAuditBuilder,
)


def main() -> None:
    builder = SafeAbstractToyManuscriptClaimLanguagePackageAuditBuilder()
    report = builder.run()
    counters = report["counters"]

    assert report["passed"] is True
    assert report["scope"] == "claim-language-package-audit-only"
    assert report["audit_phrase"] == "safe_claim_language_audited_but_no_readiness_or_real_bio_claims"
    ...
    assert counters["Toy manuscript claim language package audit item count"] == 10
    assert counters["Toy manuscript claim language package audit pass count"] == 10
    assert counters["Toy manuscript claim language package audit failure count"] == 0
