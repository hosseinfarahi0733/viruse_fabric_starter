...
from viruse_fabric.writing.sigma_a_refinement_execution_plan_boundary_audit import (
    SigmaARefinementExecutionPlanBoundaryAuditBuilder,
)


def main() -> int:
    builder = SigmaARefinementExecutionPlanBoundaryAuditBuilder()
    report = builder.run()

    output = Path(report.output_path)

    print("Experiment 215: Sigma_A Refinement Execution Plan Boundary Audit")
    print(
        "Question: Can Viruse Fabric audit the Sigma_A refinement execution plan while keeping actual Sigma_A refinement, new Sigma_A refinement, "
    ...
    print(f"Sigma_A refinement execution plan boundary audit count: {report.sigma_a_refinement_execution_plan_boundary_audit_count}")
    print(f"Sigma_A refinement plan boundary audit count: {report.sigma_a_refinement_plan_boundary_audit_count}")
    print(f"Draft Sigma_A refinement execution plan boundary audit count: {report.draft_sigma_a_refinement_execution_plan_boundary_audit_count}")
    print(f"Sigma_A refinement plan boundary audit row count: {report.sigma_a_refinement_plan_boundary_audit_row_count}")
    print(f"Sigma_A refinement plan boundary preserved count: {report.sigma_a_refinement_plan_boundary_preserved_count}")
    print(f"Sigma_A refinement plan boundary audit finding count: {report.sigma_a_refinement_plan_boundary_audit_finding_count}")
    print(f"Planned Sigma_A refinement scope audited count: {report.planned_sigma_a_refinement_scope_audited_count}")
    print(f"Planned time-index layer integration audited count: {report.planned_time_index_layer_integration_audited_count}")
    print(f"Planned draft shell refinement link audited count: {report.planned_draft_shell_refinement_link_audited_count}")
    print(f"Planned carrier clause preservation audited count: {report.planned_carrier_clause_preservation_audited_count}")
    print(f"Planned dependent object integration schedule audited count: {report.planned_dependent_object_integration_schedule_audited_count}")
    print(f"Sigma_A refinement execution blocker count: {report.sigma_a_refinement_execution_blocker_count}")
    print(f"New time-index refinement execution blocker count: {report.new_time_index_refinement_execution_blocker_count}")
    print(f"New T_A refinement execution blocker count: {report.new_t_a_refinement_execution_blocker_count}")
    ...
    print(f"Carried Sigma_A refinement execution plan count: {report.carried_sigma_a_refinement_execution_plan_count}")
    print(f"Carried Sigma_A refinement plan count: {report.carried_sigma_a_refinement_plan_count}")
    print(f"Carried Draft Sigma_A refinement execution plan count: {report.carried_draft_sigma_a_refinement_execution_plan_count}")
    print(f"Carried Sigma_A refinement plan row count: {report.carried_sigma_a_refinement_plan_row_count}")
    print(f"Carried Sigma_A refinement execution gate count: {report.carried_sigma_a_refinement_execution_gate_count}")
    print(f"Carried planned Sigma_A refinement scope count: {report.carried_planned_sigma_a_refinement_scope_count}")
    print(f"Carried planned time-index layer integration count: {report.carried_planned_time_index_layer_integration_count}")
    print(f"Carried planned draft shell refinement link count: {report.carried_planned_draft_shell_refinement_link_count}")
    print(f"Carried planned carrier clause preservation count: {report.carried_planned_carrier_clause_preservation_count}")
    print(f"Carried planned dependent object integration schedule count: {report.carried_planned_dependent_object_integration_schedule_count}")
    print(f"Carried planned Sigma_A boundary guard count: {report.carried_planned_sigma_a_boundary_guard_count}")
    print(f"Carried planned Sigma_A blocked-overreach count: {report.carried_planned_sigma_a_blocked_overreach_count}")
    ...
    required_phrases = [
        ...
        "Sigma_A refinement execution plan boundary audit only",
    ...
    print(
        "Interpretation: The v8.135 artifact audits the Sigma_A refinement execution plan while preserving zero counts for actual Sigma_A refinement, "
        "new Sigma_A refinement, new time-index refinement, new T_A refinement, definition execution, Sigma_A completion, theorem candidate planning, theorem proof, "
        "proof assistant verification, validation, readiness, and citation claims."
    )
    print("Experiment 215 completed successfully.")
    print("V8_135_OFFICIAL_EXPERIMENT_OK")
    return 0
