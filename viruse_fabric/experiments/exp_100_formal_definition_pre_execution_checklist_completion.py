...
from viruse_fabric.writing.formal_definition_pre_execution_checklist_completion import (
    BOUNDARY_PHRASES,
...
TITLE = "Experiment 100: Formal Definition Pre-Execution Checklist Completion"
QUESTION = (
    "Can Viruse Fabric complete the formal definition pre-execution checklist while "
    "keeping checklist completion approval, completed formal definitions, definition "
...
REQUIRED_REPORT_PHRASES = [
    "Formal Definition Pre-Execution Checklist Completion v8.20",
    ...
    "pre_execution_checklist_item_completed_not_approved",
...
def _count_completion_rows(text: str) -> int:
    row_ids = re.findall(
        r"^\s*###\s+(FDPECC-ROW-\d{4})\s*$",
        text,
        flags=re.MULTILINE,
    )
...
def run() -> dict[str, object]:
    ...
    definition_pre_execution_checklist_completed_count = 1 if (
        ...
        and "definition_pre_execution_checklist_completed: yes" in report
    ) else 0
    definition_pre_execution_checklist_completed_row_count = _count_completion_rows(report)
    checklist_completion_decision_execution_source_row_count = len(
        {row["source_decision_execution"] for row in CHECKLIST_COMPLETION_ROWS}
    )
    ...
    if SOURCE_ARTIFACT_COUNT != 50:
        errors.append(f"Expected source artifact count 50, got {SOURCE_ARTIFACT_COUNT}")
    ...
    if definition_pre_execution_checklist_completed_row_count != len(CHECKLIST_COMPLETION_ROWS):
        errors.append(
            f"Expected {len(CHECKLIST_COMPLETION_ROWS)} definition pre-execution checklist completed rows, got "
            f"{definition_pre_execution_checklist_completed_row_count}"
        )

    if checklist_completion_decision_execution_source_row_count != 4:
        errors.append(
            "Expected checklist completion decision execution source row count 4, got "
            f"{checklist_completion_decision_execution_source_row_count}"
        )
    ...
    return {
        ...
        "definition_pre_execution_checklist_completed_row_count": definition_pre_execution_checklist_completed_row_count,
        "checklist_completion_decision_execution_source_row_count": checklist_completion_decision_execution_source_row_count,
        "checklist_completion_approved_count": checklist_completion_approved_count,
        "formal_definition_completed_count": formal_definition_completed_count,
        "formal_definition_execution_count": formal_definition_execution_count,
        "definition_execution_approved_count": definition_execution_approved_count,
        ...
        "interpretation": (
            "The v8.20 artifact completes the formal definition pre-execution checklist "
            "while keeping checklist completion approval, formal definition execution, "
...
def main() -> None:
    ...
    print("Title: Formal Definition Pre-Execution Checklist Completion v8.20")
    ...
    print(
        "Definition pre-execution checklist completed row count: "
        f"{result['definition_pre_execution_checklist_completed_row_count']}"
    )
    print(
        "Checklist completion decision execution source row count: "
        f"{result['checklist_completion_decision_execution_source_row_count']}"
    )
    ...
    print("Experiment 100 completed successfully.")
