...
from viruse_fabric.writing.formal_definition_execution_readiness_gate_plan import (
    BOUNDARY_PHRASES,
    GATE_ROWS,
    HARD_ZERO_FIELDS,
    NEXT_STEPS,
...
TITLE = "Experiment 90: Formal Definition Execution Readiness Gate Plan"
QUESTION = (
    "Can Viruse Fabric plan formal definition execution readiness gates while "
    "keeping completed formal definitions, definition execution approval, definition "
    "execution, proof execution, formal proof, theorem proof, lemma proof, "
    "formalization completion, submission readiness, independent experiment, "
    "external validation, and new citation additions at zero?"
)

REQUIRED_REPORT_PHRASES = [
    "Formal Definition Execution Readiness Gate Plan v8.10",
...
def _count_gate_rows(text: str) -> int:
    row_ids = re.findall(
        r"^\s*###\s+(FDERG-ROW-\d{4})\s*$",
        text,
        flags=re.MULTILINE,
    )
...
def run() -> dict[str, object]:
    ...
    formal_definition_execution_readiness_gate_plan_count = report.count(
        "formal_definition_execution_readiness_gate_plan_count: 1"
    )
    definition_execution_readiness_gate_row_count = _count_gate_rows(report)
    definition_completion_criteria_source_row_count = len(
        {row["source_criteria"] for row in GATE_ROWS}
    )
    ...
    if SOURCE_ARTIFACT_COUNT != 40:
        errors.append(f"Expected source artifact count 40, got {SOURCE_ARTIFACT_COUNT}")

    if formal_definition_execution_readiness_gate_plan_count != 1:
        errors.append(
            "Expected formal definition execution readiness gate plan count 1, got "
            f"{formal_definition_execution_readiness_gate_plan_count}"
        )

    if definition_execution_readiness_gate_row_count != len(GATE_ROWS):
        errors.append(
            f"Expected {len(GATE_ROWS)} formal definition execution readiness gate rows, got "
            f"{definition_execution_readiness_gate_row_count}"
        )

    if definition_completion_criteria_source_row_count != 4:
        errors.append(
            "Expected definition completion criteria source row count 4, got "
            f"{definition_completion_criteria_source_row_count}"
        )
    ...
    return {
        ...
        "formal_definition_execution_readiness_gate_plan_count": formal_definition_execution_readiness_gate_plan_count,
        "definition_execution_readiness_gate_row_count": definition_execution_readiness_gate_row_count,
        "definition_completion_criteria_source_row_count": definition_completion_criteria_source_row_count,
        "formal_definition_completed_count": formal_definition_completed_count,
        "formal_definition_execution_count": formal_definition_execution_count,
        "definition_execution_approved_count": definition_execution_approved_count,
        "formal_mathematical_proof_count": formal_mathematical_proof_count,
        ...
        "interpretation": (
            "The v8.10 artifact plans four formal definition execution readiness "
            "gate rows while keeping completed formal definitions, definition "
            "execution approval, definition execution, proof execution, formal "
...
def main() -> None:
    ...
    print("Title: Formal Definition Execution Readiness Gate Plan v8.10")
    print(f"Output path: {result['output_path']}")
    print(f"Source artifact count: {result['source_artifact_count']}")
    print(f"Missing source artifact count: {result['missing_source_artifact_count']}")
    print(
        "Formal definition execution readiness gate plan count: "
        f"{result['formal_definition_execution_readiness_gate_plan_count']}"
    )
    print(
        "Definition execution readiness gate row count: "
        f"{result['definition_execution_readiness_gate_row_count']}"
    )
    print(
        "Definition completion criteria source row count: "
        f"{result['definition_completion_criteria_source_row_count']}"
    )
    ...
    print("Experiment 90 completed successfully.")
