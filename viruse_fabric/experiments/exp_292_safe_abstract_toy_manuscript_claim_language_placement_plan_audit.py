...
from viruse_fabric.writing.safe_abstract_toy_manuscript_claim_language_placement_plan_audit import (
    SafeAbstractToyManuscriptClaimLanguagePlacementPlanAuditBuilder,
)


def main() -> None:
    builder = SafeAbstractToyManuscriptClaimLanguagePlacementPlanAuditBuilder()
    report = builder.run()
    counters = report["counters"]

    assert report["passed"] is True
    assert report["scope"] == "claim-language-placement-plan-audit-only"
    assert report["audit_phrase"] == "safe_claim_language_placement_plan_audited_but_no_manuscript_patch_applied"
    ...
    assert counters["Toy manuscript claim language placement plan audit item count"] == 10
    assert counters["Toy manuscript claim language placement plan audit pass count"] == 10
    assert counters["Toy manuscript claim language placement plan audit failure count"] == 0
