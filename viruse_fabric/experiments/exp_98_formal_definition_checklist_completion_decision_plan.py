"""Experiment 98: checklist completion decision plan."""
...
from viruse_fabric.writing.formal_definition_checklist_completion_decision_plan import (
    BOUNDARY_PHRASES,
    DECISION_PLAN_ROWS,
    HARD_ZERO_FIELDS,
    NEXT_STEPS,
...
TITLE = "Experiment 98: Formal Definition Checklist Completion Decision Plan"
QUESTION = (
    "Can Viruse Fabric plan a checklist completion decision from audited candidate rows "
    "while keeping checklist completion decision execution, checklist completion, "
    "checklist completion approval, completed formal definitions, definition execution "
    "approval, definition execution, proof execution, formal proof, theorem proof, "
    "lemma proof, formalization completion, submission readiness, independent experiment, "
    "external validation, and new citation additions at zero?"
)

REQUIRED_REPORT_PHRASES = [
    "Formal Definition Checklist Completion Decision Plan v8.18",
    ...
    "FDCCCA-ROW-0003",
    "FDCCCA-ROW-0004",
    "eligible_for_future_completion_decision_execution_not_completed",
...
def run() -> dict[str, object]:
    ...
    checklist_completion_decision_plan_count = 1 if (
        "checklist_completion_decision_plan_count: 1" in report
    ) else 0
    checklist_completion_decision_plan_row_count = _count_decision_plan_rows(report)
    candidate_audit_source_row_count = len(
        {row["source_candidate_audit"] for row in DECISION_PLAN_ROWS}
    )
    ...
    if SOURCE_ARTIFACT_COUNT != 48:
        errors.append(f"Expected source artifact count 48, got {SOURCE_ARTIFACT_COUNT}")

    if checklist_completion_decision_plan_count != 1:
        errors.append(
            "Expected checklist completion decision plan count 1, got "
            f"{checklist_completion_decision_plan_count}"
        )
    ...
    if candidate_audit_source_row_count != 4:
        errors.append(
            "Expected candidate audit source row count 4, got "
            f"{candidate_audit_source_row_count}"
        )
    ...
    return {
        ...
        "checklist_completion_decision_plan_count": checklist_completion_decision_plan_count,
        "checklist_completion_decision_plan_row_count": checklist_completion_decision_plan_row_count,
        "candidate_audit_source_row_count": candidate_audit_source_row_count,
        "checklist_completion_decision_execution_count": checklist_completion_decision_execution_count,
        "definition_pre_execution_checklist_completed_count": definition_pre_execution_checklist_completed_count,
        "checklist_completion_approved_count": checklist_completion_approved_count,
        "formal_definition_completed_count": formal_definition_completed_count,
        ...
        "interpretation": (
            "The v8.18 artifact plans a checklist completion decision from four "
            "audited candidate rows while keeping decision execution, checklist "
...
def main() -> None:
    ...
    print("Title: Formal Definition Checklist Completion Decision Plan v8.18")
    print(f"Output path: {result['output_path']}")
    print(f"Source artifact count: {result['source_artifact_count']}")
    print(f"Missing source artifact count: {result['missing_source_artifact_count']}")
    print(f"Checklist completion decision plan count: {result['checklist_completion_decision_plan_count']}")
    print(
        "Checklist completion decision plan row count: "
        f"{result['checklist_completion_decision_plan_row_count']}"
    )
    print(f"Candidate audit source row count: {result['candidate_audit_source_row_count']}")
    print(
    ...
    print("Experiment 98 completed successfully.")
