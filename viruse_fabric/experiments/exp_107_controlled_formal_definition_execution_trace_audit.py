...
from viruse_fabric.writing.controlled_formal_definition_execution_trace_audit import (
    BOUNDARY_PHRASES,
    HARD_ZERO_FIELDS,
    NEXT_STEPS,
    OUTPUT_PATH,
...
TITLE = "Experiment 107: Controlled Formal Definition Execution Trace Audit"
QUESTION = (
    "Can Viruse Fabric audit controlled formal definition execution traces while "
...
REQUIRED_REPORT_PHRASES = [
    "Controlled Formal Definition Execution Trace Audit v8.27",
    ...
    "CFDE-ROW-0003",
    "CFDE-ROW-0004",
    "formal_definition_execution_trace_audited_not_completed",
...
def _count_trace_audit_rows(text: str) -> int:
    row_ids = re.findall(
        r"^\s*###\s+(CFDETA-ROW-\d{4})\s*$",
        text,
        flags=re.MULTILINE,
    )
...
def run() -> dict[str, object]:
    ...
    controlled_formal_definition_execution_trace_audit_count = 1 if (
        "controlled_formal_definition_execution_trace_audit_count: 1" in report
        and "controlled_formal_definition_execution_trace_audited: yes" in report
    ) else 0
    controlled_formal_definition_execution_trace_audit_row_count = _count_trace_audit_rows(report)
    controlled_formal_definition_execution_source_row_count = len(
        {row["source_controlled_formal_definition_execution"] for row in TRACE_AUDIT_ROWS}
    )
    ...
    if SOURCE_ARTIFACT_COUNT != 57:
        errors.append(f"Expected source artifact count 57, got {SOURCE_ARTIFACT_COUNT}")

    if controlled_formal_definition_execution_trace_audit_count != 1:
        errors.append(
            "Expected controlled formal definition execution trace audit count 1, got "
            f"{controlled_formal_definition_execution_trace_audit_count}"
        )

    if controlled_formal_definition_execution_trace_audit_row_count != len(TRACE_AUDIT_ROWS):
        errors.append(
            f"Expected {len(TRACE_AUDIT_ROWS)} controlled formal definition execution trace audit rows, got "
            f"{controlled_formal_definition_execution_trace_audit_row_count}"
        )

    if controlled_formal_definition_execution_source_row_count != 4:
        errors.append(
            "Expected controlled formal definition execution source row count 4, got "
            f"{controlled_formal_definition_execution_source_row_count}"
        )
    ...
    return {
        ...
        "controlled_formal_definition_execution_trace_audit_count": controlled_formal_definition_execution_trace_audit_count,
        "controlled_formal_definition_execution_trace_audit_row_count": controlled_formal_definition_execution_trace_audit_row_count,
        "controlled_formal_definition_execution_source_row_count": controlled_formal_definition_execution_source_row_count,
        ...
        "interpretation": (
            "The v8.27 artifact audits controlled formal definition execution traces "
...
def main() -> None:
    ...
    print("Title: Controlled Formal Definition Execution Trace Audit v8.27")
    print(f"Output path: {result['output_path']}")
    print(f"Source artifact count: {result['source_artifact_count']}")
    print(f"Missing source artifact count: {result['missing_source_artifact_count']}")
    print(
        "Controlled formal definition execution trace audit count: "
        f"{result['controlled_formal_definition_execution_trace_audit_count']}"
    )
    print(
        "Controlled formal definition execution trace audit row count: "
        f"{result['controlled_formal_definition_execution_trace_audit_row_count']}"
    )
    print(
        "Controlled formal definition execution source row count: "
        f"{result['controlled_formal_definition_execution_source_row_count']}"
    )
    ...
    print("Experiment 107 completed successfully.")
