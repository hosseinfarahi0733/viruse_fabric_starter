"""Experiment 97: controlled checklist completion candidate audit."""
...
from viruse_fabric.writing.formal_definition_controlled_checklist_completion_candidate_audit import (
    BOUNDARY_PHRASES,
...
TITLE = "Experiment 97: Formal Definition Controlled Checklist Completion Candidate Audit"
QUESTION = (
    "Can Viruse Fabric audit controlled checklist completion candidate rows while keeping "
    "checklist completion, checklist completion approval, completed formal definitions, "
    "definition execution approval, definition execution, proof execution, formal proof, "
...
REQUIRED_REPORT_PHRASES = [
    "Formal Definition Controlled Checklist Completion Candidate Audit v8.17",
    ...
    "FDCCCAND-ROW-0003",
    "FDCCCAND-ROW-0004",
    "candidate_sufficient_for_completion_decision_planning_not_completion",
...
def _count_candidate_audit_rows(text: str) -> int:
    row_ids = re.findall(
        r"^\s*###\s+(FDCCCA-ROW-\d{4})\s*$",
        text,
        flags=re.MULTILINE,
    )
...
def run() -> dict[str, object]:
    ...
    controlled_checklist_completion_candidate_audit_count = 1 if (
        "controlled_checklist_completion_candidate_audit_count: 1" in report
    ) else 0
    controlled_checklist_completion_candidate_audit_row_count = _count_candidate_audit_rows(report)
    checklist_completion_candidate_source_row_count = len(
        {row["source_candidate"] for row in CANDIDATE_AUDIT_ROWS}
    )
    ...
    if SOURCE_ARTIFACT_COUNT != 47:
        errors.append(f"Expected source artifact count 47, got {SOURCE_ARTIFACT_COUNT}")

    if controlled_checklist_completion_candidate_audit_count != 1:
        errors.append(
            "Expected controlled checklist completion candidate audit count 1, got "
            f"{controlled_checklist_completion_candidate_audit_count}"
        )

    if controlled_checklist_completion_candidate_audit_row_count != len(CANDIDATE_AUDIT_ROWS):
        errors.append(
            f"Expected {len(CANDIDATE_AUDIT_ROWS)} controlled checklist completion candidate audit rows, got "
            f"{controlled_checklist_completion_candidate_audit_row_count}"
        )

    if checklist_completion_candidate_source_row_count != 4:
        errors.append(
            "Expected checklist completion candidate source row count 4, got "
            f"{checklist_completion_candidate_source_row_count}"
        )
    ...
    return {
        ...
        "controlled_checklist_completion_candidate_audit_count": controlled_checklist_completion_candidate_audit_count,
        "controlled_checklist_completion_candidate_audit_row_count": controlled_checklist_completion_candidate_audit_row_count,
        "checklist_completion_candidate_source_row_count": checklist_completion_candidate_source_row_count,
        "definition_pre_execution_checklist_completed_count": definition_pre_execution_checklist_completed_count,
        "checklist_completion_approved_count": checklist_completion_approved_count,
        "formal_definition_completed_count": formal_definition_completed_count,
        "formal_definition_execution_count": formal_definition_execution_count,
        ...
        "interpretation": (
            "The v8.17 artifact audits four controlled checklist completion candidate "
            "rows while keeping checklist completion, checklist completion approval, "
            "formal definition execution, proof execution, formal mathematical proof, "
            "theorem proof, lemma proof, formalization completion, submission readiness, "
...
def main() -> None:
    ...
    print("Title: Formal Definition Controlled Checklist Completion Candidate Audit v8.17")
    print(f"Output path: {result['output_path']}")
    print(f"Source artifact count: {result['source_artifact_count']}")
    print(f"Missing source artifact count: {result['missing_source_artifact_count']}")
    print(
        "Controlled checklist completion candidate audit count: "
        f"{result['controlled_checklist_completion_candidate_audit_count']}"
    )
    print(
        "Controlled checklist completion candidate audit row count: "
        f"{result['controlled_checklist_completion_candidate_audit_row_count']}"
    )
    print(
        "Checklist completion candidate source row count: "
        f"{result['checklist_completion_candidate_source_row_count']}"
    )
    print(
    ...
    print("Experiment 97 completed successfully.")
