...
from viruse_fabric.writing.whole_sigma_a_draft_assembly_boundary_audit import (
    WholeSigmaADraftAssemblyBoundaryAuditBuilder,
)


def main() -> int:
    builder = WholeSigmaADraftAssemblyBoundaryAuditBuilder()
    report = builder.run()

    output = Path(report.output_path)

    print("Experiment 209: Whole Sigma_A Draft Assembly Boundary Audit")
    print(
        "Question: Can Viruse Fabric audit whole Sigma_A draft assembly boundaries while keeping new whole Sigma_A draft assembly, "
    ...
    print(f"Whole Sigma_A draft assembly boundary audit count: {report.whole_sigma_a_draft_assembly_boundary_audit_count}")
    print(f"Sigma_A whole draft assembly boundary audit count: {report.sigma_a_whole_draft_assembly_boundary_audit_count}")
    print(f"Sigma_A draft assembly boundary audit count: {report.sigma_a_draft_assembly_boundary_audit_count}")
    print(f"Whole Sigma_A draft assembly boundary audit row count: {report.whole_sigma_a_draft_assembly_boundary_audit_row_count}")
    print(f"Whole Sigma_A draft assembly boundary preserved count: {report.whole_sigma_a_draft_assembly_boundary_preserved_count}")
    print(f"Whole Sigma_A draft assembly boundary audit finding count: {report.whole_sigma_a_draft_assembly_boundary_audit_finding_count}")
    print(f"Assembled whole Sigma_A draft audited count: {report.assembled_whole_sigma_a_draft_audited_count}")
    print(f"Assembled whole Sigma_A draft shell audited count: {report.assembled_whole_sigma_a_draft_shell_audited_count}")
    print(f"Imported carrier-slot clause audited count: {report.imported_carrier_slot_clause_audited_count}")
    print(f"Dependent object slot deferral preserved count: {report.dependent_object_slot_deferral_preserved_count}")
    print(f"Time-index slot deferral preserved count: {report.time_index_slot_deferral_preserved_count}")
    print(f"Audit traceability audited count: {report.audit_traceability_audited_count}")
    print(f"New whole Sigma_A draft assembly blocker count: {report.new_whole_sigma_a_draft_assembly_blocker_count}")
    ...
    print(f"Carried whole Sigma_A draft assembly execution count: {report.carried_whole_sigma_a_draft_assembly_execution_count}")
    print(f"Carried Sigma_A whole draft assembly execution count: {report.carried_sigma_a_whole_draft_assembly_execution_count}")
    print(f"Carried Sigma_A draft assembly execution count: {report.carried_sigma_a_draft_assembly_execution_count}")
    print(f"Carried assembled whole Sigma_A draft count: {report.carried_assembled_whole_sigma_a_draft_count}")
    print(f"Carried assembled whole Sigma_A draft shell count: {report.carried_assembled_whole_sigma_a_draft_shell_count}")
    print(f"Carried imported carrier-slot clause count: {report.carried_imported_carrier_slot_clause_count}")
    print(f"Carried carrier-slot clause import count: {report.carried_carrier_slot_clause_import_count}")
    print(f"Carried dependent object slot count: {report.carried_dependent_object_slot_count}")
    print(f"Carried time-index slot deferral count: {report.carried_time_index_slot_deferral_count}")
    print(f"Carried audit traceability carried count: {report.carried_audit_traceability_carried_count}")
    print(f"Carried whole Sigma_A draft assembly row count: {report.carried_whole_sigma_a_draft_assembly_row_count}")
    print(f"Carried whole Sigma_A draft assembly check count: {report.carried_whole_sigma_a_draft_assembly_check_count}")
    print(f"Carried whole Sigma_A draft boundary preserved count: {report.carried_whole_sigma_a_draft_boundary_preserved_count}")
    ...
    required_phrases = [
        "Whole Sigma_A Draft Assembly Boundary Audit v8.129",
        "Whole Sigma_A draft assembly boundary audit only",
    ...
    print(
        "Interpretation: The v8.129 artifact audits the whole Draft Sigma_A shell assembled in v8.128 while preserving zero counts for new whole Sigma_A draft assembly, "
        "new Sigma_A draft assembly, new Sigma_A draft clause creation, time-index refinement, Sigma_A refinement, definition execution, "
    ...
    print("Experiment 209 completed successfully.")
    print("V8_129_OFFICIAL_EXPERIMENT_OK")
    return 0
