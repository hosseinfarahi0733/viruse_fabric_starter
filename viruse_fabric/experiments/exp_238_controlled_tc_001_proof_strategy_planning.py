...
from viruse_fabric.writing.controlled_tc_001_proof_strategy_planning import (
    REQUIRED_REPORT_PHRASES,
    write_report,
)


SOURCE_PATH = Path("outputs/controlled_theorem_candidate_planning_over_dependency_closed_bundle_v8_157.md")
OUTPUT_PATH = Path("outputs/controlled_tc_001_proof_strategy_planning_v8_158.md")
...
"""
    ...
    if "## v8.158 — Controlled TC-001 Proof Strategy Planning" not in existing:
        NOTE_PATH.write_text(existing.rstrip() + "\n\n" + note.strip() + "\n", encoding="utf-8")


def main() -> int:
    print("Experiment 238: Controlled TC-001 Proof Strategy Planning")
    print("Question: Can Viruse Fabric select TC-001 and plan a controlled proof strategy while preserving zero claims for theorem proof execution, proof assistant verification, validation, readiness, and citations?")
    print("Title: Controlled TC-001 Proof Strategy Planning v8.158")
    ...
    print("Passed: True")
    print(
        "Interpretation: The v8.158 artifact selects TC-001 and plans a controlled proof strategy with six proof obligations while preserving zero counts for theorem proof execution, TC-001 proof execution, proof assistant verification, validation, readiness approval, and citation claims."
    )
    print("Experiment 238 completed successfully.")
    print("V8_158_CONTROLLED_TC_001_PROOF_STRATEGY_PLANNING_OK")
    return 0
