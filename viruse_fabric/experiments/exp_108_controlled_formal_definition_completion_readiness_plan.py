...
from viruse_fabric.writing.controlled_formal_definition_completion_readiness_plan import (
    BOUNDARY_PHRASES,
...
TITLE = "Experiment 108: Controlled Formal Definition Completion Readiness Plan"
QUESTION = (
    "Can Viruse Fabric plan controlled formal definition completion readiness while "
    "keeping completed formal definitions, proof execution, formal proof, theorem "
...
REQUIRED_REPORT_PHRASES = [
    "Controlled Formal Definition Completion Readiness Plan v8.28",
    ...
    "CFDETA-ROW-0003",
    "CFDETA-ROW-0004",
    "formal_definition_completion_readiness_planned_not_completed",
...
def _count_completion_readiness_plan_rows(text: str) -> int:
    row_ids = re.findall(
        r"^\s*###\s+(CFDCRP-ROW-\d{4})\s*$",
        text,
        flags=re.MULTILINE,
    )
...
def run() -> dict[str, object]:
    ...
    controlled_formal_definition_completion_readiness_plan_count = 1 if (
        "controlled_formal_definition_completion_readiness_plan_count: 1" in report
        and "controlled_formal_definition_completion_readiness_planned: yes" in report
    ) else 0
    controlled_formal_definition_completion_readiness_plan_row_count = (
        _count_completion_readiness_plan_rows(report)
    )
    controlled_formal_definition_execution_trace_audit_source_row_count = len(
        {row["source_trace_audit"] for row in COMPLETION_READINESS_PLAN_ROWS}
    )
    ...
    if SOURCE_ARTIFACT_COUNT != 58:
        errors.append(f"Expected source artifact count 58, got {SOURCE_ARTIFACT_COUNT}")

    if controlled_formal_definition_completion_readiness_plan_count != 1:
        errors.append(
            "Expected controlled formal definition completion readiness plan count 1, got "
            f"{controlled_formal_definition_completion_readiness_plan_count}"
        )

    if controlled_formal_definition_completion_readiness_plan_row_count != len(COMPLETION_READINESS_PLAN_ROWS):
        errors.append(
            f"Expected {len(COMPLETION_READINESS_PLAN_ROWS)} controlled formal definition completion readiness plan rows, got "
            f"{controlled_formal_definition_completion_readiness_plan_row_count}"
        )

    if controlled_formal_definition_execution_trace_audit_source_row_count != 4:
        errors.append(
            "Expected controlled formal definition execution trace audit source row count 4, got "
            f"{controlled_formal_definition_execution_trace_audit_source_row_count}"
        )
    ...
    return {
        ...
        "controlled_formal_definition_completion_readiness_plan_count": controlled_formal_definition_completion_readiness_plan_count,
        "controlled_formal_definition_completion_readiness_plan_row_count": controlled_formal_definition_completion_readiness_plan_row_count,
        "controlled_formal_definition_execution_trace_audit_source_row_count": controlled_formal_definition_execution_trace_audit_source_row_count,
        ...
        "interpretation": (
            "The v8.28 artifact plans controlled formal definition completion readiness "
            "while keeping completed formal definitions, proof execution, formal "
            "mathematical proof, theorem proof, lemma proof, formalization completion, "
...
def main() -> None:
    ...
    print("Title: Controlled Formal Definition Completion Readiness Plan v8.28")
    print(f"Output path: {result['output_path']}")
    print(f"Source artifact count: {result['source_artifact_count']}")
    print(f"Missing source artifact count: {result['missing_source_artifact_count']}")
    print(
        "Controlled formal definition completion readiness plan count: "
        f"{result['controlled_formal_definition_completion_readiness_plan_count']}"
    )
    print(
        "Controlled formal definition completion readiness plan row count: "
        f"{result['controlled_formal_definition_completion_readiness_plan_row_count']}"
    )
    print(
        "Controlled formal definition execution trace audit source row count: "
        f"{result['controlled_formal_definition_execution_trace_audit_source_row_count']}"
    )
    ...
    print("Experiment 108 completed successfully.")
