...
from viruse_fabric.writing.controlled_formal_definition_completion_readiness_decision_plan import (
    BOUNDARY_PHRASES,
...
TITLE = "Experiment 109: Controlled Formal Definition Completion Readiness Decision Plan"
QUESTION = (
    "Can Viruse Fabric plan controlled formal definition completion readiness "
...
REQUIRED_REPORT_PHRASES = [
    "Controlled Formal Definition Completion Readiness Decision Plan v8.29",
    ...
    "CFDCRP-ROW-0003",
    "CFDCRP-ROW-0004",
    "completion_readiness_decision_planned_not_executed",
...
OVERCLAIM_PATTERNS = [
    r"\bcontrolled_formal_definition_completion_readiness_decision_executed:\s*yes\b",
...
def _count_decision_plan_rows(text: str) -> int:
    row_ids = re.findall(
        r"^\s*###\s+(CFDCRDP-ROW-\d{4})\s*$",
        text,
        flags=re.MULTILINE,
    )
...
def run() -> dict[str, object]:
    ...
    controlled_formal_definition_completion_readiness_decision_plan_count = 1 if (
        "controlled_formal_definition_completion_readiness_decision_plan_count: 1" in report
        and "controlled_formal_definition_completion_readiness_decision_planned: yes" in report
    ) else 0
    controlled_formal_definition_completion_readiness_decision_plan_row_count = (
        _count_decision_plan_rows(report)
    )
    controlled_formal_definition_completion_readiness_plan_source_row_count = len(
        {row["source_completion_readiness_plan"] for row in COMPLETION_READINESS_DECISION_PLAN_ROWS}
    )
    ...
    if SOURCE_ARTIFACT_COUNT != 59:
        errors.append(f"Expected source artifact count 59, got {SOURCE_ARTIFACT_COUNT}")

    if controlled_formal_definition_completion_readiness_decision_plan_count != 1:
        errors.append(
            "Expected controlled formal definition completion readiness decision plan count 1, got "
            f"{controlled_formal_definition_completion_readiness_decision_plan_count}"
        )

    if controlled_formal_definition_completion_readiness_decision_plan_row_count != len(
        COMPLETION_READINESS_DECISION_PLAN_ROWS
    ):
        errors.append(
            f"Expected {len(COMPLETION_READINESS_DECISION_PLAN_ROWS)} controlled formal definition completion readiness decision plan rows, got "
            f"{controlled_formal_definition_completion_readiness_decision_plan_row_count}"
        )

    if controlled_formal_definition_completion_readiness_plan_source_row_count != 4:
        errors.append(
            "Expected controlled formal definition completion readiness plan source row count 4, got "
            f"{controlled_formal_definition_completion_readiness_plan_source_row_count}"
        )
    ...
    return {
        ...
        "controlled_formal_definition_completion_readiness_decision_plan_count": controlled_formal_definition_completion_readiness_decision_plan_count,
        "controlled_formal_definition_completion_readiness_decision_plan_row_count": controlled_formal_definition_completion_readiness_decision_plan_row_count,
        "controlled_formal_definition_completion_readiness_plan_source_row_count": controlled_formal_definition_completion_readiness_plan_source_row_count,
        ...
        "interpretation": (
            "The v8.29 artifact plans controlled formal definition completion readiness "
            "decision work while keeping completed formal definitions, proof execution, "
...
def main() -> None:
    ...
    print("Title: Controlled Formal Definition Completion Readiness Decision Plan v8.29")
    print(f"Output path: {result['output_path']}")
    print(f"Source artifact count: {result['source_artifact_count']}")
    print(f"Missing source artifact count: {result['missing_source_artifact_count']}")
    print(
        "Controlled formal definition completion readiness decision plan count: "
        f"{result['controlled_formal_definition_completion_readiness_decision_plan_count']}"
    )
    print(
        "Controlled formal definition completion readiness decision plan row count: "
        f"{result['controlled_formal_definition_completion_readiness_decision_plan_row_count']}"
    )
    print(
        "Controlled formal definition completion readiness plan source row count: "
        f"{result['controlled_formal_definition_completion_readiness_plan_source_row_count']}"
    )
    ...
    print("Experiment 109 completed successfully.")
