...
from viruse_fabric.writing.safe_abstract_toy_manuscript_patch_application_decision_ledger_audit_finding_register_consistency_audit import (
    SafeAbstractToyManuscriptPatchApplicationDecisionLedgerAuditFindingRegisterConsistencyAuditBuilder,
)


def main() -> None:
    builder = SafeAbstractToyManuscriptPatchApplicationDecisionLedgerAuditFindingRegisterConsistencyAuditBuilder()
    report = builder.run()
    counters = report["counters"]

    assert report["passed"] is True
    assert report["scope"] == "consistency-audit-only"
    ...
    assert counters["Toy manuscript patch application decision ledger audit finding register consistency audit item count"] == 9
    assert counters["Toy manuscript patch application decision ledger audit finding register consistency audit pass count"] == 9
    assert counters["Toy manuscript patch application decision ledger audit finding register consistency audit failure count"] == 0
