...
from viruse_fabric.writing.safe_abstract_toy_manuscript_section_draft_package import (
    SafeAbstractToyManuscriptSectionDraftPackageBuilder,
)


def main() -> None:
    builder = SafeAbstractToyManuscriptSectionDraftPackageBuilder()
    report = builder.run()
    counters = report["counters"]

    assert report["passed"] is True
    assert report["scope"] == "section-draft-package-only"
    assert report["package_phrase"] == "safe_section_drafts_packaged_but_no_manuscript_patch_applied"
    ...
    assert counters["Toy manuscript section draft package item count"] == 9
    assert counters["Toy manuscript section draft package source audit pass count"] == 10
    assert counters["Toy manuscript section draft package source audit failure count"] == 0
