...
from viruse_fabric.writing.time_index_refinement_execution_plan import (
    TimeIndexRefinementExecutionPlanBuilder,
)


def main() -> int:
    builder = TimeIndexRefinementExecutionPlanBuilder()
    report = builder.run()

    output = Path(report.output_path)

    print("Experiment 210: Time-index Refinement Execution Plan")
    print(
        "Question: Can Viruse Fabric plan time-index refinement while keeping actual time-index refinement, Sigma_A refinement, "
    ...
    print(f"Time-index refinement execution plan count: {report.time_index_refinement_execution_plan_count}")
    print(f"Sigma_A time-index refinement execution plan count: {report.sigma_a_time_index_refinement_execution_plan_count}")
    print(f"T_A refinement execution plan count: {report.t_a_refinement_execution_plan_count}")
    print(f"Time-index refinement plan row count: {report.time_index_refinement_plan_row_count}")
    print(f"Time-index refinement execution gate count: {report.time_index_refinement_execution_gate_count}")
    print(f"Planned T_A refinement scope count: {report.planned_t_a_refinement_scope_count}")
    print(f"Planned draft shell time-index link count: {report.planned_draft_shell_time_index_link_count}")
    print(f"Planned three-time structure count: {report.planned_three_time_structure_count}")
    print(f"Planned time-index boundary guard count: {report.planned_time_index_boundary_guard_count}")
    print(f"Planned time-index blocked-overreach count: {report.planned_time_index_blocked_overreach_count}")
    ...
    print(f"Carried whole Sigma_A draft assembly boundary audit row count: {report.carried_whole_sigma_a_draft_assembly_boundary_audit_row_count}")
    print(f"Carried whole Sigma_A draft assembly boundary preserved count: {report.carried_whole_sigma_a_draft_assembly_boundary_preserved_count}")
    print(f"Carried whole Sigma_A draft assembly boundary audit finding count: {report.carried_whole_sigma_a_draft_assembly_boundary_audit_finding_count}")
    print(f"Carried assembled whole Sigma_A draft audited count: {report.carried_assembled_whole_sigma_a_draft_audited_count}")
    ...
    print(f"Carried dependent object slot deferral preserved count: {report.carried_dependent_object_slot_deferral_preserved_count}")
    print(f"Carried time-index slot deferral preserved count: {report.carried_time_index_slot_deferral_preserved_count}")
    print(f"Carried new whole Sigma_A draft assembly blocker count: {report.carried_new_whole_sigma_a_draft_assembly_blocker_count}")
    print(f"Carried new Sigma_A draft clause blocker count: {report.carried_new_sigma_a_draft_clause_blocker_count}")
    print(f"Carried time-index refinement blocker count: {report.carried_time_index_refinement_blocker_count}")
    ...
    required_phrases = [
        "Time-index Refinement Execution Plan v8.130",
        "Time-index refinement execution plan only",
    ...
    print(
        "Interpretation: The v8.130 artifact plans time-index refinement around T_A while preserving zero counts for actual time-index refinement, "
        "Sigma_A refinement, definition execution, Sigma_A completion, theorem candidate planning, theorem proof, proof assistant verification, validation, readiness, and citation claims."
    )
    print("Experiment 210 completed successfully.")
    print("V8_130_OFFICIAL_EXPERIMENT_OK")
    return 0
