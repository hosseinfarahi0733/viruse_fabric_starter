...
from viruse_fabric.writing.safe_abstract_toy_manuscript_claim_language_placement_plan import (
    SafeAbstractToyManuscriptClaimLanguagePlacementPlanBuilder,
)


def main() -> None:
    builder = SafeAbstractToyManuscriptClaimLanguagePlacementPlanBuilder()
    report = builder.run()
    counters = report["counters"]

    assert report["passed"] is True
    assert report["scope"] == "claim-language-placement-plan-only"
    assert report["plan_phrase"] == "safe_claim_language_placement_planned_but_no_manuscript_patch_applied"
    ...
    assert counters["Toy manuscript claim language placement plan item count"] == 9
    assert counters["Toy manuscript claim language placement plan source audit pass count"] == 10
    assert counters["Toy manuscript claim language placement plan source audit failure count"] == 0
