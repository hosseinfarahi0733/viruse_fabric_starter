...
from viruse_fabric.writing.formal_definition_completion_criteria_plan import (
    BOUNDARY_PHRASES,
    CRITERIA_ROWS,
    HARD_ZERO_FIELDS,
    NEXT_STEPS,
...
TITLE = "Experiment 89: Formal Definition Completion Criteria Plan"
QUESTION = (
    "Can Viruse Fabric plan formal definition completion criteria while keeping "
    "completed formal definitions, definition execution, proof execution, formal "
...
REQUIRED_REPORT_PHRASES = [
    "Formal Definition Completion Criteria Plan v8.9",
    ...
    "path_compatibility",
    "observer_projection",
    "criteria_planned_not_completed",
...
def _count_criteria_rows(text: str) -> int:
    row_ids = re.findall(
        r"^\s*###\s+(FDCCP-ROW-\d{4})\s*$",
        text,
        flags=re.MULTILINE,
    )
...
def run() -> dict[str, object]:
    ...
    formal_definition_completion_criteria_plan_count = report.count(
        "formal_definition_completion_criteria_plan_count: 1"
    )
    definition_completion_criteria_row_count = _count_criteria_rows(report)
    definition_skeleton_audit_source_row_count = len(
        {row["source_audit"] for row in CRITERIA_ROWS}
    )
    ...
    if SOURCE_ARTIFACT_COUNT != 39:
        errors.append(f"Expected source artifact count 39, got {SOURCE_ARTIFACT_COUNT}")

    if formal_definition_completion_criteria_plan_count != 1:
        errors.append(
            "Expected formal definition completion criteria plan count 1, got "
            f"{formal_definition_completion_criteria_plan_count}"
        )

    if definition_completion_criteria_row_count != len(CRITERIA_ROWS):
        errors.append(
            f"Expected {len(CRITERIA_ROWS)} formal definition completion criteria rows, got "
            f"{definition_completion_criteria_row_count}"
        )

    if definition_skeleton_audit_source_row_count != 4:
        errors.append(
            "Expected definition skeleton audit source row count 4, got "
            f"{definition_skeleton_audit_source_row_count}"
        )
    ...
    return {
        ...
        "formal_definition_completion_criteria_plan_count": formal_definition_completion_criteria_plan_count,
        "definition_completion_criteria_row_count": definition_completion_criteria_row_count,
        "definition_skeleton_audit_source_row_count": definition_skeleton_audit_source_row_count,
        "formal_definition_completed_count": formal_definition_completed_count,
        "formal_definition_execution_count": formal_definition_execution_count,
        "formal_mathematical_proof_count": formal_mathematical_proof_count,
        "proof_execution_count": proof_execution_count,
        ...
        "interpretation": (
            "The v8.9 artifact plans four formal definition completion criteria rows "
            "while keeping completed formal definitions, definition execution, proof "
...
def main() -> None:
    ...
    print("Title: Formal Definition Completion Criteria Plan v8.9")
    print(f"Output path: {result['output_path']}")
    print(f"Source artifact count: {result['source_artifact_count']}")
    print(f"Missing source artifact count: {result['missing_source_artifact_count']}")
    print(
        "Formal definition completion criteria plan count: "
        f"{result['formal_definition_completion_criteria_plan_count']}"
    )
    print(
        "Definition completion criteria row count: "
        f"{result['definition_completion_criteria_row_count']}"
    )
    print(
        "Definition skeleton audit source row count: "
        f"{result['definition_skeleton_audit_source_row_count']}"
    )
    ...
    print("Experiment 89 completed successfully.")
