...
from viruse_fabric.writing.controlled_formal_definition_execution_plan import (
    BOUNDARY_PHRASES,
    EXECUTION_PLAN_ROWS,
    HARD_ZERO_FIELDS,
    NEXT_STEPS,
...
TITLE = "Experiment 105: Controlled Formal Definition Execution Plan"
QUESTION = (
    "Can Viruse Fabric plan controlled formal definition execution while keeping "
    "formal definition execution, completed formal definitions, proof execution, "
    "formal proof, theorem proof, lemma proof, formalization completion, submission "
    "readiness, independent experiment, external validation, and new citation additions "
    "at zero?"
)

REQUIRED_REPORT_PHRASES = [
    "Controlled Formal Definition Execution Plan v8.25",
    ...
    "FDERAE-ROW-0003",
    "FDERAE-ROW-0004",
    "controlled_formal_definition_execution_planned_not_executed",
...
def _count_execution_plan_rows(text: str) -> int:
    row_ids = re.findall(
        r"^\s*###\s+(CFDEP-ROW-\d{4})\s*$",
        text,
        flags=re.MULTILINE,
    )
...
def run() -> dict[str, object]:
    ...
    controlled_formal_definition_execution_plan_count = 1 if (
        "controlled_formal_definition_execution_plan_count: 1" in report
    ) else 0
    controlled_formal_definition_execution_plan_row_count = _count_execution_plan_rows(report)
    definition_execution_approval_source_row_count = len(
        {row["source_definition_execution_approval"] for row in EXECUTION_PLAN_ROWS}
    )
    ...
    if SOURCE_ARTIFACT_COUNT != 55:
        errors.append(f"Expected source artifact count 55, got {SOURCE_ARTIFACT_COUNT}")

    if controlled_formal_definition_execution_plan_count != 1:
        errors.append(
            "Expected controlled formal definition execution plan count 1, got "
            f"{controlled_formal_definition_execution_plan_count}"
        )

    if controlled_formal_definition_execution_plan_row_count != len(EXECUTION_PLAN_ROWS):
        errors.append(
            f"Expected {len(EXECUTION_PLAN_ROWS)} controlled formal definition execution plan rows, got "
            f"{controlled_formal_definition_execution_plan_row_count}"
        )

    if definition_execution_approval_source_row_count != 4:
        errors.append(
            "Expected definition execution approval source row count 4, got "
            f"{definition_execution_approval_source_row_count}"
        )
    ...
    return {
        ...
        "controlled_formal_definition_execution_plan_count": controlled_formal_definition_execution_plan_count,
        "controlled_formal_definition_execution_plan_row_count": controlled_formal_definition_execution_plan_row_count,
        "definition_execution_approval_source_row_count": definition_execution_approval_source_row_count,
        ...
        "interpretation": (
            "The v8.25 artifact plans controlled formal definition execution while "
            "keeping formal definition execution, completed formal definitions, proof "
...
def main() -> None:
    ...
    print("Title: Controlled Formal Definition Execution Plan v8.25")
    print(f"Output path: {result['output_path']}")
    print(f"Source artifact count: {result['source_artifact_count']}")
    print(f"Missing source artifact count: {result['missing_source_artifact_count']}")
    print(
        "Controlled formal definition execution plan count: "
        f"{result['controlled_formal_definition_execution_plan_count']}"
    )
    print(
        "Controlled formal definition execution plan row count: "
        f"{result['controlled_formal_definition_execution_plan_row_count']}"
    )
    print(
        "Definition execution approval source row count: "
        f"{result['definition_execution_approval_source_row_count']}"
    )
    print(
    ...
    print("Experiment 105 completed successfully.")
