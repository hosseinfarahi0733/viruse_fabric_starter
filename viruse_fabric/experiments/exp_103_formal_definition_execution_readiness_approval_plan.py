...
from viruse_fabric.writing.formal_definition_execution_readiness_approval_plan import (
    APPROVAL_PLAN_ROWS,
    BOUNDARY_PHRASES,
    HARD_ZERO_FIELDS,
    NEXT_STEPS,
...
TITLE = "Experiment 103: Formal Definition Execution Readiness Approval Plan"
QUESTION = (
    "Can Viruse Fabric plan formal definition execution readiness approval while keeping "
    "definition execution approval, completed formal definitions, definition execution, "
    "proof execution, formal proof, theorem proof, lemma proof, formalization completion, "
    "submission readiness, independent experiment, external validation, and new citation "
    "additions at zero?"
)

REQUIRED_REPORT_PHRASES = [
    "Formal Definition Execution Readiness Approval Plan v8.23",
    ...
    "FDCCAE-ROW-0003",
    "FDCCAE-ROW-0004",
    "eligible_for_future_definition_execution_approval_not_approved",
...
def _count_approval_plan_rows(text: str) -> int:
    row_ids = re.findall(
        r"^\s*###\s+(FDERAP-ROW-\d{4})\s*$",
        text,
        flags=re.MULTILINE,
    )
...
def run() -> dict[str, object]:
    ...
    definition_execution_approval_plan_count = 1 if (
        "definition_execution_approval_plan_count: 1" in report
    ) else 0
    definition_execution_approval_plan_row_count = _count_approval_plan_rows(report)
    checklist_completion_approval_execution_source_row_count = len(
        {row["source_checklist_approval_execution"] for row in APPROVAL_PLAN_ROWS}
    )
    ...
    if SOURCE_ARTIFACT_COUNT != 53:
        errors.append(f"Expected source artifact count 53, got {SOURCE_ARTIFACT_COUNT}")

    if definition_execution_approval_plan_count != 1:
        errors.append(
            "Expected definition execution approval plan count 1, got "
            f"{definition_execution_approval_plan_count}"
        )

    if definition_execution_approval_plan_row_count != len(APPROVAL_PLAN_ROWS):
        errors.append(
            f"Expected {len(APPROVAL_PLAN_ROWS)} definition execution approval plan rows, got "
            f"{definition_execution_approval_plan_row_count}"
        )

    if checklist_completion_approval_execution_source_row_count != 4:
        errors.append(
            "Expected checklist completion approval execution source row count 4, got "
            f"{checklist_completion_approval_execution_source_row_count}"
        )
    ...
    return {
        ...
        "definition_execution_approval_plan_count": definition_execution_approval_plan_count,
        "definition_execution_approval_plan_row_count": definition_execution_approval_plan_row_count,
        "checklist_completion_approval_execution_source_row_count": checklist_completion_approval_execution_source_row_count,
        ...
        "interpretation": (
            "The v8.23 artifact plans formal definition execution readiness approval "
            "while keeping definition execution approval, formal definition execution, "
...
def main() -> None:
    ...
    print("Title: Formal Definition Execution Readiness Approval Plan v8.23")
    print(f"Output path: {result['output_path']}")
    print(f"Source artifact count: {result['source_artifact_count']}")
    print(f"Missing source artifact count: {result['missing_source_artifact_count']}")
    print(f"Definition execution approval plan count: {result['definition_execution_approval_plan_count']}")
    print(
        "Definition execution approval plan row count: "
        f"{result['definition_execution_approval_plan_row_count']}"
    )
    print(
        "Checklist completion approval execution source row count: "
        f"{result['checklist_completion_approval_execution_source_row_count']}"
    )
    print(
    ...
    print("Experiment 103 completed successfully.")
