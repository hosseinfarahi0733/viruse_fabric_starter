...
from viruse_fabric.writing.sigma_a_typed_product_carrier_refinement_boundary_audit import (
    SigmaATypedProductCarrierRefinementBoundaryAuditBuilder,
)


def main() -> int:
    builder = SigmaATypedProductCarrierRefinementBoundaryAuditBuilder()
    report = builder.run()

    output = Path(report.output_path)

    print("Experiment 200: Sigma_A Typed-Product Carrier Refinement Boundary Audit")
    print(
        "Question: Can Viruse Fabric audit typed-product carrier refinement boundaries while keeping new typed-product carrier refinement execution, "
    ...
    print(f"Sigma_A typed-product carrier refinement boundary audit count: {report.sigma_a_typed_product_carrier_refinement_boundary_audit_count}")
    print(f"Typed-product carrier refinement boundary audit count: {report.typed_product_carrier_refinement_boundary_audit_count}")
    print(f"Full typed-product carrier refinement boundary audit count: {report.full_typed_product_carrier_refinement_boundary_audit_count}")
    print(f"Typed-product carrier refinement boundary audit row count: {report.typed_product_carrier_refinement_boundary_audit_row_count}")
    print(f"Typed-product carrier refinement boundary preserved count: {report.typed_product_carrier_refinement_boundary_preserved_count}")
    print(f"Refined typed-product carrier candidate audited count: {report.refined_typed_product_carrier_candidate_audited_count}")
    print(f"Dependent object boundary preserved count: {report.dependent_object_boundary_preserved_count}")
    print(f"Open dependency preserved count: {report.open_dependency_preserved_count}")
    print(f"Carrier refinement boundary blocker count: {report.carrier_refinement_boundary_blocker_count}")
    print(f"Typed-product carrier refinement boundary audit finding count: {report.typed_product_carrier_refinement_boundary_audit_finding_count}")
    ...
    print(f"Carried typed-product carrier refinement row count: {report.carried_typed_product_carrier_refinement_row_count}")
    print(f"Carried typed-product carrier refinement check count: {report.carried_typed_product_carrier_refinement_check_count}")
    ...
    print(f"New full typed-product carrier refinement execution count: {report.new_full_typed_product_carrier_refinement_execution_count}")
    print(f"New Sigma_A typed-product carrier refinement execution count: {report.new_sigma_a_typed_product_carrier_refinement_execution_count}")
    ...
    required_phrases = [
        "Sigma_A Typed-Product Carrier Refinement Boundary Audit v8.120",
        "Sigma_A typed-product carrier refinement boundary audit only",
    ...
    print(
        "Interpretation: The v8.120 artifact audits typed-product carrier refinement boundaries while preserving zero counts for new typed-product carrier refinement execution, "
        "generic carrier refinement, carrier-type refinement, time-index refinement, Sigma_A refinement, new draft execution, definition execution, Sigma_A completion, "
        "theorem candidate planning, theorem proof, proof assistant verification, validation, readiness, and citation claims."
    )
    print("Experiment 200 completed successfully.")
    print("V8_120_OFFICIAL_EXPERIMENT_OK")
    return 0
