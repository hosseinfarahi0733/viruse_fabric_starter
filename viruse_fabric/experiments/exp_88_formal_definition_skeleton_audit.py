...
from viruse_fabric.writing.formal_definition_skeleton_audit import (
    AUDIT_ROWS,
    BOUNDARY_PHRASES,
    HARD_ZERO_FIELDS,
    NEXT_STEPS,
...
TITLE = "Experiment 88: Formal Definition Skeleton Audit"
QUESTION = (
    "Can Viruse Fabric audit formal definition skeleton targets while keeping "
...
REQUIRED_REPORT_PHRASES = [
    "Formal Definition Skeleton Audit v8.8",
...
def run() -> dict[str, object]:
    ...
    formal_definition_skeleton_audit_count = report.count("formal_definition_skeleton_audit_count: 1")
    definition_skeleton_audit_row_count = _count_audit_rows(report)
    proof_skeleton_source_component_count = report.count("proof_skeleton_source_component_count: 5")
    ...
    if SOURCE_ARTIFACT_COUNT != 38:
        errors.append(f"Expected source artifact count 38, got {SOURCE_ARTIFACT_COUNT}")

    if formal_definition_skeleton_audit_count != 1:
        errors.append(
            "Expected formal definition skeleton audit count 1, got "
            f"{formal_definition_skeleton_audit_count}"
        )

    if definition_skeleton_audit_row_count != len(AUDIT_ROWS):
        errors.append(
            f"Expected {len(AUDIT_ROWS)} formal definition skeleton audit rows, got "
            f"{definition_skeleton_audit_row_count}"
        )

    if proof_skeleton_source_component_count != 1:
        errors.append(
            "Expected one proof skeleton source component count marker, got "
            f"{proof_skeleton_source_component_count}"
        )
    ...
    return {
        ...
        "formal_definition_skeleton_audit_count": formal_definition_skeleton_audit_count,
        "definition_skeleton_audit_row_count": definition_skeleton_audit_row_count,
        "proof_skeleton_source_component_count": proof_skeleton_source_component_count,
        "formal_definition_completed_count": formal_definition_completed_count,
        "formal_mathematical_proof_count": formal_mathematical_proof_count,
        "proof_execution_count": proof_execution_count,
        "theorem_proven_count": theorem_proven_count,
        ...
        "interpretation": (
            "The v8.8 artifact audits four formal definition skeleton targets while "
...
def main() -> None:
    ...
    print("Title: Formal Definition Skeleton Audit v8.8")
    print(f"Output path: {result['output_path']}")
    print(f"Source artifact count: {result['source_artifact_count']}")
    print(f"Missing source artifact count: {result['missing_source_artifact_count']}")
    print(
        "Formal definition skeleton audit count: "
        f"{result['formal_definition_skeleton_audit_count']}"
    )
    print(
        "Definition skeleton audit row count: "
        f"{result['definition_skeleton_audit_row_count']}"
    )
    print(
        "Proof skeleton source component count: "
        f"{result['proof_skeleton_source_component_count']}"
    )
    ...
    print("Experiment 88 completed successfully.")
