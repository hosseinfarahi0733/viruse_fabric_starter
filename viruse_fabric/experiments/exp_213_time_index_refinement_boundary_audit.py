...
from viruse_fabric.writing.time_index_refinement_boundary_audit import (
    TimeIndexRefinementBoundaryAuditBuilder,
)


def main() -> int:
    builder = TimeIndexRefinementBoundaryAuditBuilder()
    report = builder.run()

    output = Path(report.output_path)

    print("Experiment 213: Time-index Refinement Boundary Audit")
    print(
        "Question: Can Viruse Fabric audit executed bounded time-index refinement while keeping new time-index refinement, new T_A refinement, "
    ...
    print(f"Time-index refinement boundary audit count: {report.time_index_refinement_boundary_audit_count}")
    print(f"T_A refinement boundary audit count: {report.t_a_refinement_boundary_audit_count}")
    print(f"Sigma_A time-index refinement boundary audit count: {report.sigma_a_time_index_refinement_boundary_audit_count}")
    print(f"Time-index refinement boundary audit row count: {report.time_index_refinement_boundary_audit_row_count}")
    print(f"Time-index refinement boundary preserved count: {report.time_index_refinement_boundary_preserved_count}")
    print(f"Time-index refinement boundary audit finding count: {report.time_index_refinement_boundary_audit_finding_count}")
    print(f"Executed time-index refinement audited count: {report.executed_time_index_refinement_audited_count}")
    print(f"Executed T_A refinement audited count: {report.executed_t_a_refinement_audited_count}")
    print(f"Three-time structure audited count: {report.three_time_structure_audited_count}")
    print(f"Draft shell time-index link audited count: {report.draft_shell_time_index_link_audited_count}")
    print(f"Carrier clause preservation audited count: {report.carrier_clause_preservation_audited_count}")
    ...
    print(f"Carried draft shell time-index link executed count: {report.carried_draft_shell_time_index_link_executed_count}")
    print(f"Carried carrier clause preserved count: {report.carried_carrier_clause_preserved_count}")
    ...
    print(f"Carried time-index refinement execution row count: {report.carried_time_index_refinement_execution_row_count}")
    print(f"Carried time-index refinement execution check count: {report.carried_time_index_refinement_execution_check_count}")
    ...
    required_phrases = [
        ...
        "Time-index refinement boundary audit only",
    ...
    print(
        "Interpretation: The v8.133 artifact audits the executed bounded time-index refinement while preserving zero counts for new time-index refinement, "
        "new T_A refinement, Sigma_A refinement, definition execution, Sigma_A completion, theorem candidate planning, theorem proof, proof assistant verification, "
        "validation, readiness, and citation claims."
    )
    print("Experiment 213 completed successfully.")
    print("V8_133_OFFICIAL_EXPERIMENT_OK")
    return 0
