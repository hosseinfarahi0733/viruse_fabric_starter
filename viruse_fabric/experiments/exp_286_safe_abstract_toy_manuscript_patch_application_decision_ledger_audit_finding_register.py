...
from viruse_fabric.writing.safe_abstract_toy_manuscript_patch_application_decision_ledger_audit_finding_register import (
    SafeAbstractToyManuscriptPatchApplicationDecisionLedgerAuditFindingRegisterBuilder,
)


def main() -> None:
    builder = SafeAbstractToyManuscriptPatchApplicationDecisionLedgerAuditFindingRegisterBuilder()
    report = builder.run()
    counters = report["counters"]

    assert report["passed"] is True
    assert report["scope"] == "finding-register-only"
    ...
    assert counters["Toy manuscript patch application decision ledger audit finding register item count"] == 9
    assert counters["Toy manuscript patch application decision ledger audit finding register registered pass count"] == 9
    assert counters["Toy manuscript patch application decision ledger audit finding register failure count"] == 0
