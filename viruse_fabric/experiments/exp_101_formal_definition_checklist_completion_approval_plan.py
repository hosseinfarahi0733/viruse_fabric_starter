...
from viruse_fabric.writing.formal_definition_checklist_completion_approval_plan import (
    APPROVAL_PLAN_ROWS,
    BOUNDARY_PHRASES,
    HARD_ZERO_FIELDS,
    NEXT_STEPS,
...
TITLE = "Experiment 101: Formal Definition Checklist Completion Approval Plan"
QUESTION = (
    "Can Viruse Fabric plan checklist completion approval while keeping checklist "
...
REQUIRED_REPORT_PHRASES = [
    "Formal Definition Checklist Completion Approval Plan v8.21",
    ...
    "FDPECC-ROW-0003",
    "FDPECC-ROW-0004",
    "eligible_for_future_checklist_completion_approval_execution_not_approved",
...
def run() -> dict[str, object]:
    ...
    checklist_completion_approval_plan_count = 1 if (
        "checklist_completion_approval_plan_count: 1" in report
    ) else 0
    checklist_completion_approval_plan_row_count = _count_approval_plan_rows(report)
    pre_execution_checklist_completion_source_row_count = len(
        {row["source_checklist_completion"] for row in APPROVAL_PLAN_ROWS}
    )
    ...
    if SOURCE_ARTIFACT_COUNT != 51:
        errors.append(f"Expected source artifact count 51, got {SOURCE_ARTIFACT_COUNT}")

    if checklist_completion_approval_plan_count != 1:
        errors.append(
            "Expected checklist completion approval plan count 1, got "
            f"{checklist_completion_approval_plan_count}"
        )

    if checklist_completion_approval_plan_row_count != len(APPROVAL_PLAN_ROWS):
        errors.append(
            f"Expected {len(APPROVAL_PLAN_ROWS)} checklist completion approval plan rows, got "
            f"{checklist_completion_approval_plan_row_count}"
        )

    if pre_execution_checklist_completion_source_row_count != 4:
        errors.append(
            "Expected pre-execution checklist completion source row count 4, got "
            f"{pre_execution_checklist_completion_source_row_count}"
        )
    ...
    return {
        ...
        "checklist_completion_approval_plan_count": checklist_completion_approval_plan_count,
        "checklist_completion_approval_plan_row_count": checklist_completion_approval_plan_row_count,
        "pre_execution_checklist_completion_source_row_count": pre_execution_checklist_completion_source_row_count,
        ...
        "interpretation": (
            "The v8.21 artifact plans checklist completion approval while keeping "
...
def main() -> None:
    ...
    print("Title: Formal Definition Checklist Completion Approval Plan v8.21")
    print(f"Output path: {result['output_path']}")
    print(f"Source artifact count: {result['source_artifact_count']}")
    print(f"Missing source artifact count: {result['missing_source_artifact_count']}")
    print(f"Checklist completion approval plan count: {result['checklist_completion_approval_plan_count']}")
    print(
        "Checklist completion approval plan row count: "
        f"{result['checklist_completion_approval_plan_row_count']}"
    )
    print(
        "Pre-execution checklist completion source row count: "
        f"{result['pre_execution_checklist_completion_source_row_count']}"
    )
    print(
    ...
    print("Experiment 101 completed successfully.")
