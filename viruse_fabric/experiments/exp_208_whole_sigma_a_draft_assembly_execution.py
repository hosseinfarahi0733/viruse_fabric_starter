...
from viruse_fabric.writing.whole_sigma_a_draft_assembly_execution import (
    WholeSigmaADraftAssemblyExecutionBuilder,
)


def main() -> int:
    builder = WholeSigmaADraftAssemblyExecutionBuilder()
    report = builder.run()

    output = Path(report.output_path)

    print("Experiment 208: Whole Sigma_A Draft Assembly Execution")
    print(
        "Question: Can Viruse Fabric execute whole Sigma_A draft assembly while keeping new Sigma_A draft clause creation, "
    ...
    print(f"Sigma_A whole draft assembly execution count: {report.sigma_a_whole_draft_assembly_execution_count}")
    print(f"Sigma_A draft assembly execution count: {report.sigma_a_draft_assembly_execution_count}")
    print(f"Assembled whole Sigma_A draft count: {report.assembled_whole_sigma_a_draft_count}")
    print(f"Assembled whole Sigma_A draft shell count: {report.assembled_whole_sigma_a_draft_shell_count}")
    print(f"Imported carrier-slot clause count: {report.imported_carrier_slot_clause_count}")
    print(f"Carried carrier-slot clause import count: {report.carried_carrier_slot_clause_import_count}")
    print(f"Dependent object slot count: {report.dependent_object_slot_count}")
    print(f"Time-index slot deferral count: {report.time_index_slot_deferral_count}")
    print(f"Audit traceability carried count: {report.audit_traceability_carried_count}")
    print(f"Whole Sigma_A draft assembly row count: {report.whole_sigma_a_draft_assembly_row_count}")
    print(f"Whole Sigma_A draft assembly check count: {report.whole_sigma_a_draft_assembly_check_count}")
    print(f"Whole Sigma_A draft boundary preserved count: {report.whole_sigma_a_draft_boundary_preserved_count}")
    print(f"Carried whole Sigma_A draft assembly execution plan count: {report.carried_whole_sigma_a_draft_assembly_execution_plan_count}")
    print(f"Carried Sigma_A whole draft assembly execution plan count: {report.carried_sigma_a_whole_draft_assembly_execution_plan_count}")
    print(f"Carried Sigma_A draft assembly execution plan count: {report.carried_sigma_a_draft_assembly_execution_plan_count}")
    print(f"Carried whole Sigma_A draft assembly plan row count: {report.carried_whole_sigma_a_draft_assembly_plan_row_count}")
    print(f"Carried whole Sigma_A draft assembly execution gate count: {report.carried_whole_sigma_a_draft_assembly_execution_gate_count}")
    print(f"Carried planned whole Sigma_A draft shell count: {report.carried_planned_whole_sigma_a_draft_shell_count}")
    print(f"Carried planned carried carrier-slot clause import count: {report.carried_planned_carried_carrier_slot_clause_import_count}")
    print(f"Carried planned dependent object slot count: {report.carried_planned_dependent_object_slot_count}")
    print(f"Carried planned time-index slot deferral count: {report.carried_planned_time_index_slot_deferral_count}")
    ...
    required_phrases = [
        ...
        "Whole Sigma_A draft assembly execution only",
    ...
    print(
        "Interpretation: The v8.128 artifact assembles exactly one whole Draft Sigma_A shell while preserving zero counts for new Sigma_A draft clause creation, "
    ...
    print("Experiment 208 completed successfully.")
    print("V8_128_OFFICIAL_EXPERIMENT_OK")
    return 0
