...
from viruse_fabric.writing.residual_ambiguity_condition_evidence_execution import (
    ResidualAmbiguityConditionEvidenceExecutionBuilder,
)


def main() -> int:
    builder = ResidualAmbiguityConditionEvidenceExecutionBuilder()
    report = builder.run()

    output = Path(report.output_path)

    print("Experiment 178: Residual Ambiguity Condition Evidence Execution")
    print(
        "Question: Can Viruse Fabric produce residual ambiguity condition evidence for SPCHG-006 / SPCC-006 "
    ...
    print(f"Residual ambiguity condition evidence execution count: {report.residual_ambiguity_condition_evidence_execution_count}")
    print(f"Residual ambiguity evidence row count: {report.residual_ambiguity_evidence_row_count}")
    print(f"Residual ambiguity scope rule count: {report.residual_ambiguity_scope_rule_count}")
    print(f"Residual ambiguity acceptance test count: {report.residual_ambiguity_acceptance_test_count}")
    print(f"Residual ambiguity blocked overreach count: {report.residual_ambiguity_blocked_overreach_count}")
    ...
    print(f"Carried minimal adequacy boundary audit count: {report.carried_minimal_adequacy_boundary_audit_count}")
    ...
    required_phrases = [
        "Residual Ambiguity Condition Evidence Execution v8.98",
        "SPCHG-006",
        "SPCC-006",
    ...
    print(
        "Interpretation: The v8.98 artifact produces residual ambiguity condition evidence for SPCHG-006 / SPCC-006, "
        "does not resolve SPCHG-006, preserves five resolved gaps and two unresolved blocking gaps, and keeps completion "
    ...
    print("Experiment 178 completed successfully.")
    print("V8_98_OFFICIAL_EXPERIMENT_OK")
    return 0
