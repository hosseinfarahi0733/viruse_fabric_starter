...
from viruse_fabric.writing.safe_abstract_toy_citation_slot_integration_plan import (
    SafeAbstractToyCitationSlotIntegrationPlanBuilder,
)


def main() -> None:
    builder = SafeAbstractToyCitationSlotIntegrationPlanBuilder()
    report = builder.run()
    counters = report["counters"]

    assert report["passed"] is True
    assert report["scope"] == "citation-slot-integration-plan-only"
    assert report["plan_phrase"] == "citation_slots_planned_but_no_citations_added"

    assert report["citation_slot_plan_completed"] is True
    ...
    assert counters["Toy citation slot count"] == 12
    assert counters["Toy citation unresolved slot count"] == 12
    assert counters["Toy citation slot group count"] == 4
    assert counters["Toy citation actual citation count"] == 0
    assert counters["Toy citation verified source count"] == 0
    assert counters["Toy citation fabricated reference count"] == 0
    assert counters["Toy citation source retrieval count"] == 0
    assert counters["Toy citation integration completion count"] == 0
    assert counters["Toy citation added to manuscript count"] == 0

    assert counters["Toy citation slot source assembly section count"] == 9
    assert counters["Toy citation slot source gap item count"] == 12
    assert counters["Toy citation slot source P0 gap count"] == 6
    assert counters["Toy citation slot source evidence upgrade completed count"] == 0
    assert counters["Toy citation slot source evaluation design module count"] == 10
    assert counters["Toy citation slot source actual evaluation run count"] == 0
    assert counters["Toy citation slot source validation claim count"] == 0
    assert counters["Toy citation slot source coherence improvement item count"] == 10
    assert counters["Toy citation slot source coherence rewrite application count"] == 0
    ...
    for phrase in [
        ...
        assert phrase in combined_text, f"Missing citation-slot phrase: {phrase}"
    ...
    print(f"Source evidence upgrade completed count: {counters['Toy citation slot source evidence upgrade completed count']}")
    print(f"Source evaluation design module count: {counters['Toy citation slot source evaluation design module count']}")
    print(f"Source actual evaluation run count: {counters['Toy citation slot source actual evaluation run count']}")
    print(f"Source validation claim count: {counters['Toy citation slot source validation claim count']}")
