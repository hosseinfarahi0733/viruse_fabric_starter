...
from viruse_fabric.writing.final_residual_condition_evidence_execution import (
    FinalResidualConditionEvidenceExecutionBuilder,
)


def main() -> int:
    builder = FinalResidualConditionEvidenceExecutionBuilder()
    report = builder.run()

    output = Path(report.output_path)

    print("Experiment 180: Final Residual Condition Evidence Execution")
    print(
        "Question: Can Viruse Fabric produce final residual condition evidence for SPCHG-007 / SPCC-007 "
        "while keeping gap resolution decision, completion decision, completion execution, definition execution, "
        "predicate definition completion, theorem proof, proof execution, proof assistant verification, completed "
        "formalization, framework-level proof, external validation, independent experiment, submission readiness, "
    ...
    print(f"Final residual condition evidence execution count: {report.final_residual_condition_evidence_execution_count}")
    print(f"Final residual evidence row count: {report.final_residual_evidence_row_count}")
    print(f"Final residual scope rule count: {report.final_residual_scope_rule_count}")
    print(f"Final residual acceptance test count: {report.final_residual_acceptance_test_count}")
    print(f"Final residual blocked overreach count: {report.final_residual_blocked_overreach_count}")
    print(f"Target gap count: {report.target_gap_count}")
    print(f"Target gap ID count: {report.target_gap_id_count}")
    print(f"Target criterion count: {report.target_criterion_count}")
    print(f"Evidence support count: {report.evidence_support_count}")
    ...
    print(f"Carried residual ambiguity evidence boundary audit count: {report.carried_residual_ambiguity_evidence_boundary_audit_count}")
    ...
    required_phrases = [
        "Final Residual Condition Evidence Execution v8.100",
        "SPCHG-007",
        "SPCC-007",
    ...
    print(
        "Interpretation: The v8.100 artifact produces final residual condition evidence for SPCHG-007 / SPCC-007, "
        "does not resolve SPCHG-007, preserves six resolved gaps and one unresolved blocking gap, and keeps gap resolution "
        "decision, completion decision, completion execution, definition execution, predicate definition completion, theorem proof, "
        "proof execution, proof assistant verification, completed formalization, framework-level proof, external validation, "
        "independent experiment, submission readiness, readiness approval, and new citation additions at zero."
    )
    print("Experiment 180 completed successfully.")
    print("V8_100_OFFICIAL_EXPERIMENT_OK")
    return 0
