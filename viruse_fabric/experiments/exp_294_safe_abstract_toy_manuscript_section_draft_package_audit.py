...
from viruse_fabric.writing.safe_abstract_toy_manuscript_section_draft_package_audit import (
    SafeAbstractToyManuscriptSectionDraftPackageAuditBuilder,
)


def main() -> None:
    builder = SafeAbstractToyManuscriptSectionDraftPackageAuditBuilder()
    report = builder.run()
    counters = report["counters"]

    assert report["passed"] is True
    assert report["scope"] == "section-draft-package-audit-only"
    assert report["audit_phrase"] == "safe_section_drafts_audited_but_no_manuscript_patch_applied"
    ...
    assert counters["Toy manuscript section draft package audit item count"] == 11
    assert counters["Toy manuscript section draft package audit pass count"] == 11
    assert counters["Toy manuscript section draft package audit failure count"] == 0
