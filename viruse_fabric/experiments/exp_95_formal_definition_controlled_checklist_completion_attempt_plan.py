"""Experiment 95: controlled checklist completion attempt plan."""
...
from viruse_fabric.writing.formal_definition_controlled_checklist_completion_attempt_plan import (
    ATTEMPT_PLAN_ROWS,
    BOUNDARY_PHRASES,
    HARD_ZERO_FIELDS,
    NEXT_STEPS,
...
TITLE = "Experiment 95: Formal Definition Controlled Checklist Completion Attempt Plan"
QUESTION = (
    "Can Viruse Fabric plan a controlled checklist completion attempt while keeping "
    "attempt execution, checklist completion, checklist completion approval, completed "
    "formal definitions, definition execution approval, definition execution, proof "
...
REQUIRED_REPORT_PHRASES = [
    "Formal Definition Controlled Checklist Completion Attempt Plan v8.15",
    ...
    "path_compatibility",
    "observer_projection",
    "completion_attempt_planned_not_executed",
...
OVERCLAIM_PATTERNS = [
    r"\bcontrolled_completion_attempt_executed:\s*yes\b",
    r"\bcontrolled_checklist_completion_attempt_execution_count:\s*[1-9][0-9]*\b",
...
def _count_attempt_rows(text: str) -> int:
    row_ids = re.findall(
        r"^\s*###\s+(FDCCAP-ROW-\d{4})\s*$",
        text,
        flags=re.MULTILINE,
    )
...
def run() -> dict[str, object]:
    ...
    controlled_checklist_completion_attempt_plan_count = report.count(
        "controlled_checklist_completion_attempt_plan_count: 1"
    )
    controlled_checklist_completion_attempt_plan_row_count = _count_attempt_rows(report)
    checklist_completion_readiness_decision_source_row_count = len(
        {row["source_readiness_decision"] for row in ATTEMPT_PLAN_ROWS}
    )

    controlled_checklist_completion_attempt_execution_count = report.count(
    ...
    if SOURCE_ARTIFACT_COUNT != 45:
        errors.append(f"Expected source artifact count 45, got {SOURCE_ARTIFACT_COUNT}")

    if controlled_checklist_completion_attempt_plan_count != 1:
        errors.append(
            "Expected controlled checklist completion attempt plan count 1, got "
            f"{controlled_checklist_completion_attempt_plan_count}"
        )

    if controlled_checklist_completion_attempt_plan_row_count != len(ATTEMPT_PLAN_ROWS):
        errors.append(
            f"Expected {len(ATTEMPT_PLAN_ROWS)} controlled checklist completion attempt plan rows, got "
            f"{controlled_checklist_completion_attempt_plan_row_count}"
        )

    if checklist_completion_readiness_decision_source_row_count != 2:
        errors.append(
            "Expected checklist completion readiness decision source row count 2, got "
            f"{checklist_completion_readiness_decision_source_row_count}"
        )
    ...
    return {
        ...
        "controlled_checklist_completion_attempt_plan_count": controlled_checklist_completion_attempt_plan_count,
        "controlled_checklist_completion_attempt_plan_row_count": controlled_checklist_completion_attempt_plan_row_count,
        "checklist_completion_readiness_decision_source_row_count": checklist_completion_readiness_decision_source_row_count,
        "controlled_checklist_completion_attempt_execution_count": controlled_checklist_completion_attempt_execution_count,
        "definition_pre_execution_checklist_completed_count": definition_pre_execution_checklist_completed_count,
        "checklist_completion_approved_count": checklist_completion_approved_count,
        "formal_definition_completed_count": formal_definition_completed_count,
        ...
        "interpretation": (
            "The v8.15 artifact plans a controlled checklist completion attempt for "
            "four checklist rows while keeping attempt execution, checklist completion, "
            "checklist completion approval, formal definition execution, proof execution, "
            "formal mathematical proof, theorem proof, lemma proof, formalization completion, "
...
def main() -> None:
    ...
    print("Title: Formal Definition Controlled Checklist Completion Attempt Plan v8.15")
    print(f"Output path: {result['output_path']}")
    print(f"Source artifact count: {result['source_artifact_count']}")
    print(f"Missing source artifact count: {result['missing_source_artifact_count']}")
    print(
        "Controlled checklist completion attempt plan count: "
        f"{result['controlled_checklist_completion_attempt_plan_count']}"
    )
    print(
        "Controlled checklist completion attempt plan row count: "
        f"{result['controlled_checklist_completion_attempt_plan_row_count']}"
    )
    print(
        "Checklist completion readiness decision source row count: "
        f"{result['checklist_completion_readiness_decision_source_row_count']}"
    )
    print(
    ...
    print("Experiment 95 completed successfully.")
