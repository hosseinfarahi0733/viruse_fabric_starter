...
from viruse_fabric.writing.controlled_formal_definition_completion_attempt_plan import (
    BOUNDARY_PHRASES,
...
TITLE = "Experiment 111: Controlled Formal Definition Completion Attempt Plan"
QUESTION = (
    "Can Viruse Fabric plan controlled formal definition completion attempts while "
    "keeping completion attempt execution, completed formal definitions, proof "
    "execution, formal proof, theorem proof, lemma proof, formalization completion, "
    "submission readiness, independent experiment, external validation, and new "
    "citation additions at zero?"
)

REQUIRED_REPORT_PHRASES = [
    "Controlled Formal Definition Completion Attempt Plan v8.31",
    ...
    "CFDCRDE-ROW-0003",
    "CFDCRDE-ROW-0004",
    "controlled_formal_definition_completion_attempt_planned_not_executed",
...
OVERCLAIM_PATTERNS = [
    r"\bcontrolled_formal_definition_completion_attempt_executed:\s*yes\b",
    r"\bcontrolled_formal_definition_completion_attempt_execution_count:\s*[1-9][0-9]*\b",
...
def _count_completion_attempt_plan_rows(text: str) -> int:
    row_ids = re.findall(
        r"^\s*###\s+(CFDCAP-ROW-\d{4})\s*$",
        text,
        flags=re.MULTILINE,
    )
...
def run() -> dict[str, object]:
    ...
    controlled_formal_definition_completion_attempt_plan_count = 1 if (
        ...
        and "controlled_formal_definition_completion_attempt_planned: yes" in report
    ) else 0
    controlled_formal_definition_completion_attempt_plan_row_count = (
        _count_completion_attempt_plan_rows(report)
    )
    controlled_formal_definition_completion_readiness_decision_execution_source_row_count = len(
        {
            row["source_completion_readiness_decision_execution"]
            for row in COMPLETION_ATTEMPT_PLAN_ROWS
        }
    )
    controlled_formal_definition_completion_readiness_decision_executed_count = 1 if (
        "controlled_formal_definition_completion_readiness_decision_executed_count: 1" in report
        and "controlled_formal_definition_completion_readiness_decision_executed_source: yes" in report
    ) else 0
    ...
    controlled_formal_definition_completion_attempt_execution_count = report.count(
    ...
    if SOURCE_ARTIFACT_COUNT != 61:
        errors.append(f"Expected source artifact count 61, got {SOURCE_ARTIFACT_COUNT}")
    ...
    if controlled_formal_definition_completion_attempt_plan_row_count != len(COMPLETION_ATTEMPT_PLAN_ROWS):
        errors.append(
            f"Expected {len(COMPLETION_ATTEMPT_PLAN_ROWS)} controlled formal definition completion attempt plan rows, got "
            f"{controlled_formal_definition_completion_attempt_plan_row_count}"
        )

    if controlled_formal_definition_completion_readiness_decision_execution_source_row_count != 4:
        errors.append(
            "Expected controlled formal definition completion readiness decision execution source row count 4, got "
            f"{controlled_formal_definition_completion_readiness_decision_execution_source_row_count}"
        )

    if controlled_formal_definition_completion_readiness_decision_executed_count != 1:
        errors.append(
            "Expected controlled formal definition completion readiness decision executed count 1, got "
            f"{controlled_formal_definition_completion_readiness_decision_executed_count}"
        )
    ...
    return {
        ...
        "controlled_formal_definition_completion_attempt_plan_row_count": controlled_formal_definition_completion_attempt_plan_row_count,
        "controlled_formal_definition_completion_readiness_decision_execution_source_row_count": controlled_formal_definition_completion_readiness_decision_execution_source_row_count,
        "controlled_formal_definition_completion_readiness_decision_executed_count": controlled_formal_definition_completion_readiness_decision_executed_count,
        ...
        "interpretation": (
            "The v8.31 artifact plans controlled formal definition completion attempts "
            "while keeping completion attempt execution, completed formal definitions, "
...
def main() -> None:
    ...
    print("Title: Controlled Formal Definition Completion Attempt Plan v8.31")
    ...
    print(
        "Controlled formal definition completion attempt plan row count: "
        f"{result['controlled_formal_definition_completion_attempt_plan_row_count']}"
    )
    print(
        "Controlled formal definition completion readiness decision execution source row count: "
        f"{result['controlled_formal_definition_completion_readiness_decision_execution_source_row_count']}"
    )
    print(
        "Controlled formal definition completion readiness decision executed count: "
        f"{result['controlled_formal_definition_completion_readiness_decision_executed_count']}"
    )
    ...
    print("Experiment 111 completed successfully.")
