...
from viruse_fabric.writing.controlled_formal_definition_completion_approval_plan import (
    BOUNDARY_PHRASES,
    COMPLETION_APPROVAL_PLAN_ROWS,
    HARD_ZERO_FIELDS,
    NEXT_STEPS,
    OUTPUT_PATH,
...
TITLE = "Experiment 113: Controlled Formal Definition Completion Approval Plan"
QUESTION = (
    "Can Viruse Fabric plan controlled formal definition completion approval while "
    "keeping completion approval execution, formal definition completion approval, "
    "completed formal definitions, proof execution, formal proof, theorem proof, "
...
REQUIRED_REPORT_PHRASES = [
    "Controlled Formal Definition Completion Approval Plan v8.33",
    "controlled_formal_definition_completion_approval_plan_count: 1",
    "controlled_formal_definition_completion_approval_plan_row_count: 4",
    "controlled_formal_definition_completion_attempt_execution_source_row_count: 4",
    ...
    "CFDCAE-ROW-0003",
    "CFDCAE-ROW-0004",
    "formal_definition_completion_approval_planned_not_executed",
...
def _count_completion_approval_plan_rows(text: str) -> int:
    row_ids = re.findall(
        r"^\s*###\s+(CFDCAPPR-ROW-\d{4})\s*$",
        text,
        flags=re.MULTILINE,
    )
...
def run() -> dict[str, object]:
    ...
    controlled_formal_definition_completion_approval_plan_count = 1 if (
        ...
        and "formal_definition_completion_approval_planned: yes" in report
    ) else 0
    controlled_formal_definition_completion_approval_plan_row_count = (
        _count_completion_approval_plan_rows(report)
    )
    controlled_formal_definition_completion_attempt_execution_source_row_count = len(
        {
            row["source_completion_attempt_execution"]
            for row in COMPLETION_APPROVAL_PLAN_ROWS
        }
    )
    ...
    if SOURCE_ARTIFACT_COUNT != 63:
        errors.append(f"Expected source artifact count 63, got {SOURCE_ARTIFACT_COUNT}")
    ...
    if controlled_formal_definition_completion_approval_plan_row_count != len(COMPLETION_APPROVAL_PLAN_ROWS):
        errors.append(
            f"Expected {len(COMPLETION_APPROVAL_PLAN_ROWS)} controlled formal definition completion approval plan rows, got "
            f"{controlled_formal_definition_completion_approval_plan_row_count}"
        )

    if controlled_formal_definition_completion_attempt_execution_source_row_count != 4:
        errors.append(
            "Expected controlled formal definition completion attempt execution source row count 4, got "
            f"{controlled_formal_definition_completion_attempt_execution_source_row_count}"
        )
    ...
    return {
        ...
        "controlled_formal_definition_completion_approval_plan_row_count": controlled_formal_definition_completion_approval_plan_row_count,
        "controlled_formal_definition_completion_attempt_execution_source_row_count": controlled_formal_definition_completion_attempt_execution_source_row_count,
        ...
        "interpretation": (
            ...
            "while keeping approval execution, formal definition completion approval, "
            "completed formal definitions, proof execution, formal mathematical proof, "
            "theorem proof, lemma proof, formalization completion, submission readiness, "
...
def main() -> None:
    ...
    print("Title: Controlled Formal Definition Completion Approval Plan v8.33")
    ...
    print(
        "Controlled formal definition completion approval plan row count: "
        f"{result['controlled_formal_definition_completion_approval_plan_row_count']}"
    )
    print(
        "Controlled formal definition completion attempt execution source row count: "
        f"{result['controlled_formal_definition_completion_attempt_execution_source_row_count']}"
    )
    print(
    ...
    print("Experiment 113 completed successfully.")
