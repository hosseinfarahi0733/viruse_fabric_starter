...
from viruse_fabric.writing.sigma_a_carrier_level_draft_assembly_boundary_audit import (
    SigmaACarrierLevelDraftAssemblyBoundaryAuditBuilder,
)


def main() -> int:
    builder = SigmaACarrierLevelDraftAssemblyBoundaryAuditBuilder()
    report = builder.run()

    output = Path(report.output_path)

    print("Experiment 203: Sigma_A Carrier-Level Draft Assembly Boundary Audit")
    print(
        "Question: Can Viruse Fabric audit Sigma_A carrier-level draft assembly boundaries while keeping new carrier-level draft assembly execution, "
    ...
    print(f"Sigma_A carrier-level draft assembly boundary audit count: {report.sigma_a_carrier_level_draft_assembly_boundary_audit_count}")
    print(f"Carrier-level draft assembly boundary audit count: {report.carrier_level_draft_assembly_boundary_audit_count}")
    print(f"Carrier-level draft assembly boundary audit row count: {report.carrier_level_draft_assembly_boundary_audit_row_count}")
    print(f"Carrier-level draft assembly boundary preserved count: {report.carrier_level_draft_assembly_boundary_preserved_count}")
    print(f"Carrier-level draft assembly boundary audit finding count: {report.carrier_level_draft_assembly_boundary_audit_finding_count}")
    print(f"Assembled carrier candidate audited count: {report.assembled_carrier_candidate_audited_count}")
    print(f"Carrier slot placement audited count: {report.carrier_slot_placement_audited_count}")
    print(f"Carrier candidate import boundary preserved count: {report.carrier_candidate_import_boundary_preserved_count}")
    ...
    print(f"Carried carrier-level draft assembly row count: {report.carried_carrier_level_draft_assembly_row_count}")
    print(f"Carried carrier-level draft assembly check count: {report.carried_carrier_level_draft_assembly_check_count}")
    print(f"Carried assembled carrier candidate count: {report.carried_assembled_carrier_candidate_count}")
    print(f"Carried carrier slot placement count: {report.carried_carrier_slot_placement_count}")
    print(f"Carried carrier candidate import count: {report.carried_carrier_candidate_import_count}")
    ...
    required_phrases = [
        ...
        "Sigma_A carrier-level draft assembly boundary audit only",
    ...
    print(
        "Interpretation: The v8.123 artifact audits Sigma_A carrier-level draft assembly boundaries while preserving zero counts for new carrier-level draft assembly execution, "
        "whole Sigma_A draft assembly execution, new Sigma_A draft clause creation, time-index refinement, Sigma_A refinement, definition execution, Sigma_A completion, "
        "theorem candidate planning, theorem proof, proof assistant verification, validation, readiness, and citation claims."
    )
    print("Experiment 203 completed successfully.")
    print("V8_123_OFFICIAL_EXPERIMENT_OK")
    return 0
