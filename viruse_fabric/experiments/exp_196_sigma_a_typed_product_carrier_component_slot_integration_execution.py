...
from viruse_fabric.writing.sigma_a_typed_product_carrier_component_slot_integration_execution import (
    SigmaATypedProductCarrierComponentSlotIntegrationExecutionBuilder,
)


def main() -> int:
    builder = SigmaATypedProductCarrierComponentSlotIntegrationExecutionBuilder()
    report = builder.run()

    output = Path(report.output_path)

    print("Experiment 196: Sigma_A Typed-Product Carrier Component-Slot Integration Execution")
    print(
        "Question: Can Viruse Fabric execute component-slot integration while keeping full typed-product carrier refinement, carrier refinement, "
        "time-index refinement, Sigma_A refinement, new draft execution, definition execution, Sigma_A definition completion, theorem candidate planning, "
    ...
    print(f"Sigma_A typed-product carrier component-slot integration execution count: {report.sigma_a_typed_product_carrier_component_slot_integration_execution_count}")
    print(f"Component-slot integration execution count: {report.component_slot_integration_execution_count}")
    print(f"Component-slot integration row count: {report.component_slot_integration_row_count}")
    print(f"Integrated component slot count: {report.integrated_component_slot_count}")
    print(f"Integrated coordinate count: {report.integrated_coordinate_count}")
    print(f"Integration boundary check count: {report.integration_boundary_check_count}")
    print(f"Component-slot integration status count: {report.component_slot_integration_status_count}")
    ...
    print(f"Carried component-slot integration gate count: {report.carried_component_slot_integration_gate_count}")
    print(f"Carried planned integration coordinate count: {report.carried_planned_integration_coordinate_count}")
    ...
    print(f"Carried component-slot refinement boundary audit row count: {report.carried_component_slot_refinement_boundary_audit_row_count}")
    print(f"Carried refined component slot audited count: {report.carried_refined_component_slot_audited_count}")
    print(f"Carried local boundary preserved count: {report.carried_local_boundary_preserved_count}")
    print(f"Carried integration blocker count: {report.carried_integration_blocker_count}")
    ...
    required_phrases = [
        "Sigma_A Typed-Product Carrier Component-Slot Integration Execution v8.116",
        "Sigma_A typed-product carrier component-slot integration execution only",
    ...
    print(
        "Interpretation: The v8.116 artifact executes component-slot integration while preserving zero counts for full typed-product carrier refinement, "
        "carrier refinement, time-index refinement, Sigma_A refinement, new draft execution, definition execution, Sigma_A completion, theorem candidate planning, "
        "theorem proof, proof assistant verification, validation, readiness, and citation claims."
    )
    print("Experiment 196 completed successfully.")
    print("V8_116_OFFICIAL_EXPERIMENT_OK")
    return 0
