...
from viruse_fabric.writing.formal_definition_pre_execution_checklist_plan import (
    BOUNDARY_PHRASES,
    CHECKLIST_ROWS,
    HARD_ZERO_FIELDS,
    NEXT_STEPS,
...
TITLE = "Experiment 91: Formal Definition Pre-Execution Checklist Plan"
QUESTION = (
    "Can Viruse Fabric plan formal definition pre-execution checklist rows while "
...
REQUIRED_REPORT_PHRASES = [
    "Formal Definition Pre-Execution Checklist Plan v8.11",
    ...
    "path_compatibility",
    "observer_projection",
    "checklist_planned_not_completed",
...
def _count_checklist_rows(text: str) -> int:
    row_ids = re.findall(
        r"^\s*###\s+(FDECP-ROW-\d{4})\s*$",
        text,
        flags=re.MULTILINE,
    )
...
def run() -> dict[str, object]:
    ...
    formal_definition_pre_execution_checklist_plan_count = report.count(
        "formal_definition_pre_execution_checklist_plan_count: 1"
    )
    definition_pre_execution_checklist_row_count = _count_checklist_rows(report)
    definition_execution_readiness_gate_source_row_count = len(
        {row["source_gate"] for row in CHECKLIST_ROWS}
    )
    ...
    if SOURCE_ARTIFACT_COUNT != 41:
        errors.append(f"Expected source artifact count 41, got {SOURCE_ARTIFACT_COUNT}")

    if formal_definition_pre_execution_checklist_plan_count != 1:
        errors.append(
            "Expected formal definition pre-execution checklist plan count 1, got "
            f"{formal_definition_pre_execution_checklist_plan_count}"
        )

    if definition_pre_execution_checklist_row_count != len(CHECKLIST_ROWS):
        errors.append(
            f"Expected {len(CHECKLIST_ROWS)} formal definition pre-execution checklist rows, got "
            f"{definition_pre_execution_checklist_row_count}"
        )

    if definition_execution_readiness_gate_source_row_count != 4:
        errors.append(
            "Expected definition execution readiness gate source row count 4, got "
            f"{definition_execution_readiness_gate_source_row_count}"
        )
    ...
    return {
        ...
        "formal_definition_pre_execution_checklist_plan_count": formal_definition_pre_execution_checklist_plan_count,
        "definition_pre_execution_checklist_row_count": definition_pre_execution_checklist_row_count,
        "definition_execution_readiness_gate_source_row_count": definition_execution_readiness_gate_source_row_count,
        "definition_pre_execution_checklist_completed_count": definition_pre_execution_checklist_completed_count,
        "formal_definition_completed_count": formal_definition_completed_count,
        "formal_definition_execution_count": formal_definition_execution_count,
        "definition_execution_approved_count": definition_execution_approved_count,
        ...
        "interpretation": (
            "The v8.11 artifact plans four formal definition pre-execution checklist "
...
def main() -> None:
    ...
    print("Title: Formal Definition Pre-Execution Checklist Plan v8.11")
    print(f"Output path: {result['output_path']}")
    print(f"Source artifact count: {result['source_artifact_count']}")
    print(f"Missing source artifact count: {result['missing_source_artifact_count']}")
    print(
        "Formal definition pre-execution checklist plan count: "
        f"{result['formal_definition_pre_execution_checklist_plan_count']}"
    )
    print(
        "Definition pre-execution checklist row count: "
        f"{result['definition_pre_execution_checklist_row_count']}"
    )
    print(
        "Definition execution readiness gate source row count: "
        f"{result['definition_execution_readiness_gate_source_row_count']}"
    )
    print(
    ...
    print("Experiment 91 completed successfully.")
