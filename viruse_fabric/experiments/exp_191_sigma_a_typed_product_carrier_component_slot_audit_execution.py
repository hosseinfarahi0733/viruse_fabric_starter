...
from viruse_fabric.writing.sigma_a_typed_product_carrier_component_slot_audit_execution import (
    SigmaATypedProductCarrierComponentSlotAuditExecutionBuilder,
)


def main() -> int:
    builder = SigmaATypedProductCarrierComponentSlotAuditExecutionBuilder()
    report = builder.run()

    output = Path(report.output_path)

    print("Experiment 191: Sigma_A Typed-Product Carrier Component-Slot Audit Execution")
    print(
        "Question: Can Viruse Fabric audit typed-product carrier component slots after typed-product carrier selection while keeping "
        "new carrier selection, carrier refinement execution, Sigma_A refinement execution, new draft execution, definition execution, "
    ...
    print(f"Sigma_A typed-product carrier component-slot audit execution count: {report.sigma_a_typed_product_carrier_component_slot_audit_execution_count}")
    print(f"Typed-product carrier component slot audited count: {report.typed_product_carrier_component_slot_audited_count}")
    print(f"Component-slot compatibility check count: {report.component_slot_compatibility_check_count}")
    print(f"Component-slot unresolved status count: {report.component_slot_unresolved_status_count}")
    print(f"Selected typed-product carrier carried count: {report.selected_typed_product_carrier_carried_count}")
    ...
    print(f"Carried selected carrier type count: {report.carried_selected_carrier_type_count}")
    print(f"Carried selected typed-product carrier count: {report.carried_selected_typed_product_carrier_count}")
    print(f"Carried auxiliary structured-schema representation count: {report.carried_auxiliary_structured_schema_representation_count}")
    ...
    required_phrases = [
        "Sigma_A Typed-Product Carrier Component-Slot Audit Execution v8.111",
        "Sigma_A typed-product carrier component-slot audit execution only",
    ...
    print(
        "Interpretation: The v8.111 artifact audits typed-product carrier component slots while preserving zero counts for new carrier selection, "
    ...
    print("Experiment 191 completed successfully.")
    print("V8_111_OFFICIAL_EXPERIMENT_OK")
    return 0
