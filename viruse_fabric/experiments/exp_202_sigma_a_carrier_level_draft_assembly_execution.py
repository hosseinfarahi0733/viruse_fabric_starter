...
from viruse_fabric.writing.sigma_a_carrier_level_draft_assembly_execution import (
    SigmaACarrierLevelDraftAssemblyExecutionBuilder,
)


def main() -> int:
    builder = SigmaACarrierLevelDraftAssemblyExecutionBuilder()
    report = builder.run()

    output = Path(report.output_path)

    print("Experiment 202: Sigma_A Carrier-Level Draft Assembly Execution")
    print(
        "Question: Can Viruse Fabric execute Sigma_A carrier-level draft assembly while keeping whole Sigma_A draft assembly execution, "
        "new Sigma_A draft clause creation, time-index refinement, Sigma_A refinement, definition execution, Sigma_A definition completion, "
    ...
    print(f"Carrier-level draft assembly row count: {report.carrier_level_draft_assembly_row_count}")
    print(f"Carrier-level draft assembly check count: {report.carrier_level_draft_assembly_check_count}")
    print(f"Assembled carrier candidate count: {report.assembled_carrier_candidate_count}")
    print(f"Carrier slot placement count: {report.carrier_slot_placement_count}")
    print(f"Carrier candidate import count: {report.carrier_candidate_import_count}")
    ...
    print(f"Carried Sigma_A carrier-level draft assembly execution plan count: {report.carried_sigma_a_carrier_level_draft_assembly_execution_plan_count}")
    print(f"Carried carrier-level draft assembly execution plan count: {report.carried_carrier_level_draft_assembly_execution_plan_count}")
    print(f"Carried Sigma_A draft assembly execution plan count: {report.carried_sigma_a_draft_assembly_execution_plan_count}")
    print(f"Carried carrier-level draft assembly plan row count: {report.carried_carrier_level_draft_assembly_plan_row_count}")
    print(f"Carried carrier-level draft assembly execution gate count: {report.carried_carrier_level_draft_assembly_execution_gate_count}")
    print(f"Carried planned carrier-level draft mapping count: {report.carried_planned_carrier_level_draft_mapping_count}")
    print(f"Carried planned carrier candidate import count: {report.carried_planned_carrier_candidate_import_count}")
    ...
    required_phrases = [
        "Sigma_A Carrier-Level Draft Assembly Execution v8.122",
        "Sigma_A carrier-level draft assembly execution only",
    ...
    print(
        "Interpretation: The v8.122 artifact executes Sigma_A carrier-level draft assembly while preserving zero counts for whole Sigma_A draft assembly, "
    ...
    print("Experiment 202 completed successfully.")
    print("V8_122_OFFICIAL_EXPERIMENT_OK")
    return 0
