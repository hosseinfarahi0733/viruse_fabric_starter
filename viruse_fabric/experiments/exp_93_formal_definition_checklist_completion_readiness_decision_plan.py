"""Experiment 93: checklist completion readiness decision plan."""
...
from viruse_fabric.writing.formal_definition_checklist_completion_readiness_decision_plan import (
    BOUNDARY_PHRASES,
    DECISION_PLAN_ROWS,
    HARD_ZERO_FIELDS,
    NEXT_STEPS,
...
TITLE = "Experiment 93: Formal Definition Checklist Completion Readiness Decision Plan"
QUESTION = (
    "Can Viruse Fabric plan a future checklist completion readiness decision while "
    "keeping checklist completion, decision execution, checklist completion approval, "
    "completed formal definitions, definition execution approval, definition execution, "
...
REQUIRED_REPORT_PHRASES = [
    "Formal Definition Checklist Completion Readiness Decision Plan v8.13",
    ...
    "FDCCRDP-ROW-0001",
    "FDCCRDP-ROW-0002",
    "checklist_completion_readiness",
    "checklist_completion_hold_or_advance",
    "decision_planned_not_executed",
...
OVERCLAIM_PATTERNS = [
    ...
    r"\bchecklist_completion_decision_executed:\s*yes\b",
    r"\bchecklist_completion_decision_execution_count:\s*[1-9][0-9]*\b",
...
def run() -> dict[str, object]:
    ...
    checklist_completion_readiness_decision_plan_count = report.count(
        "checklist_completion_readiness_decision_plan_count: 1"
    )
    checklist_completion_decision_plan_row_count = _count_decision_plan_rows(report)
    checklist_audit_source_row_count = 4 if "FDECA-ROW-0001..FDECA-ROW-0004" in report else 0

    definition_pre_execution_checklist_completed_count = report.count(
        "pre_execution_checklist_completed: yes"
    )
    checklist_completion_decision_execution_count = report.count(
    ...
    if SOURCE_ARTIFACT_COUNT != 43:
        errors.append(f"Expected source artifact count 43, got {SOURCE_ARTIFACT_COUNT}")

    if checklist_completion_readiness_decision_plan_count != 1:
        errors.append(
            "Expected checklist completion readiness decision plan count 1, got "
            f"{checklist_completion_readiness_decision_plan_count}"
        )

    if checklist_completion_decision_plan_row_count != len(DECISION_PLAN_ROWS):
        errors.append(
            f"Expected {len(DECISION_PLAN_ROWS)} checklist completion decision plan rows, got "
            f"{checklist_completion_decision_plan_row_count}"
        )

    if checklist_audit_source_row_count != 4:
        errors.append(
            "Expected checklist audit source row count 4, got "
            f"{checklist_audit_source_row_count}"
        )

    zero_checks = {
        "definition_pre_execution_checklist_completed_count": definition_pre_execution_checklist_completed_count,
        "checklist_completion_decision_execution_count": checklist_completion_decision_execution_count,
    ...
    return {
        ...
        "checklist_completion_readiness_decision_plan_count": checklist_completion_readiness_decision_plan_count,
        "checklist_completion_decision_plan_row_count": checklist_completion_decision_plan_row_count,
        "checklist_audit_source_row_count": checklist_audit_source_row_count,
        "definition_pre_execution_checklist_completed_count": definition_pre_execution_checklist_completed_count,
        "checklist_completion_decision_execution_count": checklist_completion_decision_execution_count,
        "checklist_completion_approved_count": checklist_completion_approved_count,
        "formal_definition_completed_count": formal_definition_completed_count,
        ...
        "interpretation": (
            "The v8.13 artifact plans two checklist completion readiness decision "
            "rows while keeping checklist completion, decision execution, checklist "
            "completion approval, formal definition execution, proof execution, formal "
...
def main() -> None:
    ...
    print("Title: Formal Definition Checklist Completion Readiness Decision Plan v8.13")
    print(f"Output path: {result['output_path']}")
    print(f"Source artifact count: {result['source_artifact_count']}")
    print(f"Missing source artifact count: {result['missing_source_artifact_count']}")
    print(
        "Checklist completion readiness decision plan count: "
        f"{result['checklist_completion_readiness_decision_plan_count']}"
    )
    print(
        "Checklist completion decision plan row count: "
        f"{result['checklist_completion_decision_plan_row_count']}"
    )
    print(f"Checklist audit source row count: {result['checklist_audit_source_row_count']}")
    print(
        "Definition pre-execution checklist completed count: "
        f"{result['definition_pre_execution_checklist_completed_count']}"
    )
    print(
        "Checklist completion decision execution count: "
        f"{result['checklist_completion_decision_execution_count']}"
    )
    ...
    print("Experiment 93 completed successfully.")
