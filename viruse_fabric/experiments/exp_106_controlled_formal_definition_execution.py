...
from viruse_fabric.writing.controlled_formal_definition_execution import (
    BOUNDARY_PHRASES,
    EXECUTION_ROWS,
    HARD_ZERO_FIELDS,
    NEXT_STEPS,
...
TITLE = "Experiment 106: Controlled Formal Definition Execution"
QUESTION = (
    "Can Viruse Fabric execute controlled formal definition traces while keeping "
...
REQUIRED_REPORT_PHRASES = [
    ...
    "CFDEP-ROW-0003",
    "CFDEP-ROW-0004",
    "formal_definition_execution_trace_created_not_completed",
...
def run() -> dict[str, object]:
    ...
    controlled_formal_definition_execution_count = 1 if (
        "controlled_formal_definition_execution_count: 1" in report
        and "controlled_formal_definition_execution_executed: yes" in report
    ) else 0
    formal_definition_execution_count = 1 if (
        "formal_definition_execution_count: 1" in report
        and "formal_definition_executed: yes" in report
    ) else 0
    formal_definition_execution_row_count = _count_execution_rows(report)
    controlled_formal_definition_execution_plan_source_row_count = len(
        {row["source_execution_plan"] for row in EXECUTION_ROWS}
    )
    ...
    if SOURCE_ARTIFACT_COUNT != 56:
        errors.append(f"Expected source artifact count 56, got {SOURCE_ARTIFACT_COUNT}")

    if controlled_formal_definition_execution_count != 1:
        errors.append(
            "Expected controlled formal definition execution count 1, got "
            f"{controlled_formal_definition_execution_count}"
        )
    ...
    if formal_definition_execution_row_count != len(EXECUTION_ROWS):
        errors.append(
            f"Expected {len(EXECUTION_ROWS)} formal definition execution rows, got "
            f"{formal_definition_execution_row_count}"
        )

    if controlled_formal_definition_execution_plan_source_row_count != 4:
        errors.append(
            "Expected controlled formal definition execution plan source row count 4, got "
            f"{controlled_formal_definition_execution_plan_source_row_count}"
        )
    ...
    return {
        ...
        "controlled_formal_definition_execution_count": controlled_formal_definition_execution_count,
        "formal_definition_execution_count": formal_definition_execution_count,
        "formal_definition_execution_row_count": formal_definition_execution_row_count,
        "controlled_formal_definition_execution_plan_source_row_count": controlled_formal_definition_execution_plan_source_row_count,
        ...
        "interpretation": (
            "The v8.26 artifact executes controlled formal definition traces while "
            "keeping completed formal definitions, proof execution, formal mathematical "
...
def main() -> None:
    ...
    print("Title: Controlled Formal Definition Execution v8.26")
    print(f"Output path: {result['output_path']}")
    print(f"Source artifact count: {result['source_artifact_count']}")
    print(f"Missing source artifact count: {result['missing_source_artifact_count']}")
    print(
        "Controlled formal definition execution count: "
        f"{result['controlled_formal_definition_execution_count']}"
    )
    print(f"Formal definition execution count: {result['formal_definition_execution_count']}")
    print(
        "Formal definition execution row count: "
        f"{result['formal_definition_execution_row_count']}"
    )
    print(
        "Controlled formal definition execution plan source row count: "
        f"{result['controlled_formal_definition_execution_plan_source_row_count']}"
    )
    print(
    ...
    print("Experiment 106 completed successfully.")
